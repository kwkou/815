...
单击几下按钮，就可以为 WEB 应用启用身份验证和授权。每个 Azure 中的复选框样式配置都提供了业务线 WEB 应用的基本访问控制。它在授予用户对 WEB 应用内容的访问权限之前，会强制对所选的 Azure AD 租户进行 HTTPS 通信和身份验证。有关详细信息，请参阅 [ WEB 应用身份验证/授权](http://azure.microsoft.com/zh-cn/blog/2014/11/13/azure-websites-authentication-authorization/)。
