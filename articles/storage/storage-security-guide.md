...
SSE 可让你请求存储服务在将数据写入 Azure 存储空间时自动加密数据。当你从 Azure 存储空间读取数据时，存储服务将在返回数据之前将其解密。这样，你就可以保护数据，而无需修改代码或将代码添加到任何应用程序。
...
SSE 由 Azure 存储管理。使用 SSE 不是针对传输中数据安全性提供的，但它将在数据写入 Azure 存储空间时加密数据。使用此功能时不会对性能造成任何影响。
...
-   [Monitor a Storage Account in the Azure portal（在 Azure 门户预览中监视存储帐户）](/documentation/articles/storage-monitor-storage-account/)
