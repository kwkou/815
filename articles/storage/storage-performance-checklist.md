...
||所有服务|	可伸缩性目标|[你的命名约定是否旨在实现更好的负载均衡？](#subheading47)
...
Azure 存储空间使用基于范围的分区方案来对系统进行缩放和负载均衡。分区键用于将数据分区成多个范围，这些范围在整个系统上经过负载均衡。这意味着，命名约定，如词汇顺序（例如 msftpayroll、msftperformance、msftemployees 等等）或使用时间戳（log20160101、log20160102、log20160102 等等）将本身出租给可能共置于同一分区服务器中的分区，直到负载均衡操作将它们拆分成较小的范围。例如，容器中的所有 Bob 接受单个服务器的服务，直到这些 Blob 的负载需要进一步重新平衡分区范围。同样，一组名称按词汇顺序排列的少量负载帐户可以接受单个服务器的服务，直到其中一个或所有帐户的负载请求它们拆分到多个分区服务器。每个负载均衡操作可能在操作期间影响存储调用的延迟。系统处理某个分区流量激增的能力受到单个分区服务器可缩放性的限制，直到负载均衡操作着手重新平衡分区键范围。  
