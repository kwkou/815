...
**应该在什么时候使用 Azure 导入/导出服务？** 
如果在网络上进行的上传或下载根据估计大约会超过 7 天，则应考虑使用 Azure 导入导出。可以使用任何联机计算器或通过[下载](https://github.com/Azure-Samples/storage-dotnet-import-export-job-management/archive/master.zip)位于我们 Azure 示例存储库的 Azure 导入导出 REST API 示例中[数据传输速度计算器](https://github.com/Azure-Samples/storage-dotnet-import-export-job-management/blob/master/DataTransferSpeedCalculator.html)处的计算器，来计算所需时间。这并不是精确计算，只是粗略指示。
