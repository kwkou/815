...
例如，考虑名为 _photos_ 的容器中包含的下面一组块 Blob：
...
在 _photos_ 容器上调用 **ListBlobs**（如上面的示例所示）时，返回一个层次结构列表。它包含 **CloudBlobDirectory** 和 **CloudBlockBlob** 对象，分别表示容器中的目录和 Blob。生成的输出如下所示：
...
此示例演示平面 Blob 列表，但你也可以执行分层列表，只需将 **ListBlobsSegmentedAsync** 方法的 _useFlatBlobListing_ 参数设置为 _false_ 即可。
