<properties
	pageTitle="Azure 虚拟机备份疑难解答 | Windows Azure"
...
| 备份操作 | 错误详细信息 | 解决方法 |
| -------- | -------- | -------|
| 发现 | 无法发现新项 - Windows Azure 备份发生内部错误。等候几分钟时间，然后重试操作。 | 在 15 分钟后重试发现过程。
...
| 注册 | Windows Azure 备份遇到内部错误 - 等候几分钟，然后重试操作。如果问题持续出现，请联系 Windows Azure 支持。 | 可能因为不支持以下其中一项配置而发生此错误：<ul><li>Premium LRS </ul> |
...
| 备份操作 | 错误详细信息 | 解决方法 |
| -------- | -------- | -------|
| 备份 | 从备份保管库复制 VHD 超时 - 请在几分钟后重试操作。如果问题持续出现，请联系 Windows Azure 支持。 | 要复制的数据太多时会发生此问题。请检查你的数据磁盘是否少于 16 个。 |
...
| 备份 | 发生内部错误，备份失败 - 请在几分钟后重试操作。如果问题持续出现，请联系 Windows Azure 支持 | 发生此错误的原因有 2 个：<ol><li>要复制的数据太多。<li>已删除原始 VM，因此无法进行备份。若要保留已删除 VM 的备份数据但要防止备份错误，请取消保护 VM 并选择保留数据。这样即可停止备份计划以及重复出现的错误消息。 |
...
| 备份 | 扩展安装失败，出现错误“COM+ 无法与 Windows Azure 分布式事务处理协调器通信”。 | 这通常意味着到 COM+ 服务未运行。请与 Windows Azure 支持部门联系，以获取解决此问题所需的帮助。 |
