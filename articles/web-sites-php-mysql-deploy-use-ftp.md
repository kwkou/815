...
1. 创建名为 `registration` 的 MySQL 数据库。您可以在 MySQL 命令提示符中使用此命令执行此操作：
	...
	<pre><code>mysql> create database registration;</code></pre>

2. 在 Web 服务器的根目录中，创建一个名为 `registration` 的文件夹，并在其中创建两个文件 - 一个名为 <pre><code>createtable.php</code></pre>，另一个名为 `index.php`。
