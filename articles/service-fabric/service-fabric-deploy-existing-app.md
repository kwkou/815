...
这是前端应用程序（不包括 REST 终结点）的有用配置，因为客户端应用程序只需“连接到”群集中的任何节点以便使用该终结点。例如，当 Service Fabric 群集的所有节点都连接到负载均衡器时，还可以使用此配置，以便客户端流量可以分布在在群集中的所有节点上运行的服务上。
...
##<a name="using-visual-studio-to-package-an-existing-application"></a> 使用 Visual Studio 打包现有应用程序
