<properties 
	...
	description="了解如何使用 Azure 托管托管缓存服务提高 Azure 应用程序的性能" 
	services="cache" 
	documentationCenter="" 
	authors="steved0x" 
	manager="dwrede" 
...
# 如何使用 Azure 托管缓存服务（预览版）

本指南演示如何开始使用 **Azure 托管缓存服务（预览版）**。相关示例用 C# 代码编写且使用 .NET API。涉及的应用场景包括**创建和配置缓存**、**配置缓存客户端**、**在缓存中添加和删除对象、在缓存中存储 ASP.NET 会话状态**以及**使用缓存启用 ASP.NET 页面输出缓存**。有关使用 Azure Cache 的详细信息，请参阅[后续步骤][]部分。
...
Azure 托管缓存服务（预览版）是内存中的可扩展分布式解决方案，利用该解决方案，可通过对数据进行超高速访问来生成高度可扩展且高度可响应的应用程序。

Azure 托管缓存服务（预览版）包含以下功能：
...
托管缓存服务（预览版）提供对 Microsoft 所管理安全专用缓存的访问权限。使用托管缓存服务（预览版）创建的缓存可从在 Azure 网站、Web 角色和辅助角色以及虚拟机中运行的 Azure 内的应用程序进行访问。
...
本指南提供了托管缓存服务（预览版）的入门概述。有关超出本入门指南范围的那些功能的更多详细信息，请参阅 [Azure 托管缓存服务（预览版）概述][]。

<a name="getting-started-cache-service"></a>
## 缓存服务（预览版）入门

托管缓存服务（预览版）入门相当容易。若要开始使用，需要首先设置和配置缓存。接下来，配置缓存客户端，以便它们可以访问缓存。在配置了缓存客户端后，就可以开始使用它们。
...
使用托管缓存服务（预览版）创建的缓存可从 Azure 网站、Web 角色、辅助角色和虚拟机中运行的 Azure 应用程序进行访问。提供了 NuGet 包，它可用于简化缓存客户端应用程序的配置。

若要使用缓存 NuGet 包配置客户端应用程序，请在“解决方案资源管理器”中右键单击项目，然后选择“管理 NuGet 包”。

![NuGet 包菜单][NuGetPackageMenu]

在“联机搜索”文本框中键入 **WindowsAzure.Caching**，然后从结果中选择“Microsoft Azure Cache”。单击“安装”，然后单击“我接受”。
...
用于 Azure Cache 的会话状态提供程序是用于 ASP.NET 应用程序的进程外存储机制。此提供程序允许你将会话状态存储在 Azure 缓存中而非内存或 SQL Server 数据库中。若要使用缓存会话状态提供程序，请首先配置缓存群集，然后使用缓存 NuGet 包配置 ASP.NET 应用程序的缓存，如[托管缓存服务（预览版）入门][]中所述。在安装缓存 NuGet 包时，它会在 web.config 中添加一个包含 ASP.NET 应用程序所需配置的注释掉的节，以使用用于 Azure Cache 的会话状态提供程序。
...
用于 Azure Cache 的输出缓存提供程序是用于输出缓存数据的进程外存储机制。此数据专门用于完整 HTTP 响应（页面输出缓存）。此提供程序会插入 ASP.NET 4 中引入的新输出缓存提供程序扩展点。若要使用该输出缓存提供程序，请首先配置缓存群集，然后使用缓存 NuGet 包配置 ASP.NET 应用程序的缓存，如[托管缓存服务（预览版）入门][]中所述。在安装 Caching NuGet 包时，它会在 web.config 中添加以下包含 ASP.NET 应用程序所需配置的注释掉的节，以使用用于 Azure Caching 的输出缓存提供程序。
...
现在，你已了解托管缓存服务（预览版）的基础知识，请单击下面的链接了解如何执行更复杂的缓存任务。

-   参阅 MSDN 参考：[托管缓存服务（预览版）][]
-	了解如何迁移到托管缓存服务（预览版）：[迁移到托管缓存服务（预览版）][]
-   查看示例：[托管缓存服务（预览版）示例][]
...
[Which type of caching : #choosing-cache
[Prepare Your Visual Studio Project to Use Azure Caching]: #prepare-vs
[Configure Your Application to Use Caching]: #configure-app
[托管缓存服务（预览版）入门]: #getting-started-cache-service
[创建缓存]: #create-cache
[配置缓存]: #enable-caching
[配置缓存客户端]: #NuGet
...
[Azure 托管缓存服务（预览版）概述]: http://go.microsoft.com/fwlink/?LinkId=320830
[托管缓存服务（预览版）]: https://msdn.microsoft.com/zh-cn/library/azure/dn690523.aspx
...
[迁移到托管缓存服务（预览版）]: https://msdn.microsoft.com/zh-cn/library/azure/dn386108.aspx
