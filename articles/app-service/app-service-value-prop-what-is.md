...
## App Service 计划
[应用服务计划](/documentation/articles/azure-web-sites-web-hosting-plans-in-depth-overview/)指定运行应用的计算资源的类型。如果流量负载预计较轻，则可使用共享 VM（**免费**和**共享**定价层）。负载较高时，可以从不同大小的专用 VM（**基本**、**标准**和**高级**层）中进行选择。多个应用服务应用可共享同一个计划，并可随计划一起增加和减少定价层。
