...
### <a name="configure-local-cache-by-using-the-azure-portal-preview"></a>通过 Azure 门户预览配置本地缓存
...
本地缓存大小默认为 **300 MB**， 其中包括从内容存储复制过来的 /site 和 /siteextensions 文件夹，以及任何本地创建的日志和数据文件夹。 若要增加此限制，请使用应用设置 `WEBSITE_LOCAL_CACHE_SIZEINMB`。 最高可将此大小增加到每个 Web 应用 **2 GB** (2000 MB)。
...
* 针对“过渡”槽来测试站点。  
* 准备就绪以后，在过渡槽和生产槽之间执行[交换操作](/documentation/articles/web-sites-staged-publishing/#Swap)。  
...
### <a name="how-can-i-tell-if-local-cache-applies-to-my-web-app"></a>如何确定本地缓存是否适用于 Web 应用？
...
### <a name="i-just-published-new-changes-but-my-web-app-does-not-seem-to-have-them-why"></a>刚发布的新更改内容似乎并没有在 Web 应用上显示出来， 为什么？
...
<!--Update_Description: add a question "Does local cache exclude any directories from being copied to the faster local drive?"-->