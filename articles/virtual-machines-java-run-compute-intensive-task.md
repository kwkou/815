...
借助 Azure，您可以使用虚拟机处理需要进行大量计算的任务。例如，虚拟机可以处理任务并将结果传送给客户端计算机或移动应用程序。完成本指南后，您将了解如何创建运行可由其他 Java 应用程序监视的、需要进行大量计算的 Java 应用程序的虚拟机。
...
请注意，Azure 服务总线功能需要将 Baltimore CyberTrust 根证书作为您的 JRE 的 **cacerts** 存储的一部分安装。此证书将自动包含在本教程使用的 JRE 中。如果您的 JRE **cacerts** 存储中没有此证书，请参阅[将证书添加到 Java CA 证书存储][add_ca_cert]，以获取有关添加该证书的信息（以及有关查看您的 cacerts 存储中的证书的信息）。

## 如何创建服务总线命名空间

若要开始在 Azure 中使用服务总线队列，必须首先创建一个服务命名空间。服务命名空间提供了用于对应用程序中的服务总线资源进行寻址的范围容器。
...
4.  在"创建新的服务命名空间"****对话框中，输入"命名空间"****，然后为了确保它是唯一的，请单击"检查可用性"****按钮。  
    ![Create a New Namespace screenshot][create_namespace]
5.  在确保命名空间名称可用后，选择应该托管您的命名空间的国家或地区，然后单击"创建命名空间"****按钮。  
...
4.  "默认密钥"****处于隐藏状态。单击"查看"****按钮以显示
    安全凭据：   
    ![Default Key screenshot][default_key]
5.  记下"默认颁发者"****和"默认密钥"****，因为您将在下面使用此信息对命名空间执行操作。 
...
1. 在开发计算机上，使用本节末尾的示例代码创建 Java 控制台应用程序。在本教程中，我们将使用 **TSPClient.java** 作为 Java 文件名。如上所述，将 **your\_service\_bus\_namespace**、**your\_service\_bus\_owner** 和 **your\_service\_bus\_key** 占位符分别修改为使用您的服务总线"命名空间"****、"默认颁发者"****和"默认密钥"****值。
...
7. 您将需要在运行 TSP 解算器排列之前创建服务总线队列。运行以下命令以创建服务总线队列：
...
6. 运行以下命令：
    ...
    或者，也可以通过传入命令行参数来指定检查队列之间休眠的分钟数。默认休眠期为检查队列为 3 分钟，这将使用没有命令行参数传递给**TSPClient**。如果您要使用不同的休眠时间间隔值（如一分钟），请运行：
