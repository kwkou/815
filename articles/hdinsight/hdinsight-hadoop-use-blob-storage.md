...
* **数据重用和共享：**HDFS 中的数据位于计算群集内。仅有权访问计算群集的应用程序才能通过 HDFS API 使用数据。可通过 HDFS API 或 [Blob 存储 REST API][blob-storage-restAPI] 访问 Azure 存储中的数据。因此，可使用大量应用程序（包括其他 HDInsight 群集）和工具来生成和使用此类数据。
...
> [AZURE.NOTE]
大多数 HDFS 命令（例如 <b>ls</b>、<b>copyFromLocal</b> 和 <b>mkdir</b>）仍按预期工作。只有特定于本机 HDFS 实现（称作 DFS）的命令才会在 Azure 存储上显示不同的行为，例如 <b>fschk</b> 和 <b>dfsadmin</b>。
> 
> 
...
&lt;BlobStorageContainerName&gt; 标识 Azure 存储中 Blob 容器的名称。&lt;StorageAccountName&gt; 标识 Azure 存储帐户名。完全限定域名 (FQDN) 是必需的。
...
&lt;path&gt; 是文件或目录 HDFS 路径名。由于 Azure 存储中的容器只是键值存储，因此没有真正的分层文件系统。Blob 键中的斜杠字符 (/) 解释为目录分隔符。例如，*hadoop-mapreduce-examples.jar* 的 Blob 名称是：
