...
在群集创建过程中，该脚本将枚举文件、将这些文件复制到头节点和工作节点上的 `/usr/lib/customhivelibs/` 目录，然后将它们添加到 `core-site.xml` 文件中的 `hive.aux.jars.path` 
...
1. 使用[预配 Windows 上的 HDInsight 群集](/documentation/articles/hdinsight-provision-clusters/)中的步骤开始预配群集，但不要完成预配。
2. 在“可选配置”边栏选项卡上，选择“脚本操作”，并提供如下所示信息：

    * **名称**：输入脚本操作的友好名称。
    * **脚本 URI**：https://hdiconfigactions.blob.core.windows.net/setupcustomhivelibsv01/setup-customhivelibs-v01.ps1
    * **头节点**：选中此选项
    * **工作节点**：选中此选项。
    * **ZOOKEEPER**：将此项留空。
...
6. 继续按[预配 Windows 上的 HDInsight 群集](/documentation/articles/hdinsight-provision-clusters/)中所述预配群集。
