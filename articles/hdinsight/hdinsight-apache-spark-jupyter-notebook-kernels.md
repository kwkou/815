<properties
    pageTitle="在 Azure Spark 群集上将不同内核与 Jupyter 笔记本配合使用 | Azure"
    description="了解 PySpark 和 Spark 内核，这两种内核可与 HDInsight Linux 上 Spark 群集随附的 Jupyter 笔记本配合使用。"
    services="hdinsight"
    documentationcenter=""
    author="nitinme"
    manager="jhubbard"
...
HDInsight Spark 群集提供了可在 Jupyter 笔记本中使用的两个内核，以便测试 Spark 应用程序。内核是可以运行和解释代码的程序。这两个内核是：
...
3. 单击“新建”，然后单击“Pyspark”或“Spark”创建新的笔记本。Spark 内核适用于 Scala 应用程序，PySpark 内核适用于 Python 应用程序。
   
    ![创建新 Jupyter 笔记本](./media/hdinsight-apache-spark-jupyter-notebook-kernels/jupyter-kernels.png "创建新的 Jupyter 笔记本")

4. 此时将打开具有所选内核的新笔记本。

## <a name="choose-between-the-kernels"></a>在内核之间进行选择
下面是使用新内核的几项好处。

- **预设上下文**。使用 **PySpark** 或 **Spark** 内核，不需要先显式设置 Spark 或 Hive 上下文，即可开始使用应用程序；这些上下文默认可供使用。这些上下文包括：
