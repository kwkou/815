<properties
	pageTitle="在 Azure 网站中监视网站"
	description="了解如何使用管理门户在 Azure 网站中监视网站。"
...
#<a name="howtomonitor"></a>在 Azure 网站中监视网站

[Azure 网站](/documentation/services/web-sites/)通过“监视”管理页对标准 App Service 提供监视功能。“监视”管理页按如下所述提供网站的性能统计信息。
## 目录 ##
...
1. 在 [Azure 管理门户](https://manage.windowsazure.cn)中，从网站页面上单击“监视”选项卡，以显示“监视”管理页。默认情况下，“监视”页上的图表显示的度量值与“仪表板”页上图表所显示的度量值相同。
...
在“标准”网站模式中，可以收到基于网站监视度量值的警报。该警报功能要求你首先配置用于监视的 Web 终结点，你可以在“配置”页的“监视”部分中进行此配置。你还可以选择当所选度量值达到指定的值时发送电子邮件。

##<a name="howtoviewusage"></a>如何：查看网站的使用率配额

从 [Azure 管理门户](https://manage.windowsazure.cn)中网站的“缩放”管理页可将网站配置为以“共享”或“标准”网站模式运行。每个 Azure 订阅均有权访问为在“共享”模式中每个区域运行最多 100 个网站所提供的资源池。对于为此目的而向每个网站订阅提供的资源池可由同一地理区域中配置为在“共享”模式中运行的其他网站共享。由于共享这些资源是为了供其他网站使用，因此所有订阅对这些资源的使用是受限的。订阅使用这些资源存在限制，该限制以各网站“仪表板”管理页的使用率概述部分下列出的使用率配额形式表示。

>[AZURE.NOTE]当网站配置为以“标准”模式运行时，会被分配专用资源，资源大小等同于 Azure 的[虚拟机](/documentation/articles/virtual-machines-size-specs)和[云服务](/documentation/articles/cloud-services-sizes-specs)大小表中的“小型”（默认值）、“中型”或“大型”虚拟机大小。对于可用于在“标准”模式下运行网站的订阅，没有针对资源的限制。但是，每个区域可创建的“标准”模式网站的数量是 500。

### 如何：查看配置为共享模式的网站的使用率配额 ###
若要确定网站对资源使用率配额的影响程度，请执行下列步骤：

1. 在 [Azure 管理门户](https://manage.windowsazure.cn)中打开网站的“仪表板”管理页。
2. “使用概览”部分显示了相应 [Azure 网站](/documentation/services/web-sites/)的使用率配额，它是以下内容的一个子集：
	-	**输出的数据**、**CPU 时间**和**内存** - 当超过配额时，Azure 将会停止网站，以留出当前配额间隔的剩余部分。Azure 将在下一配额间隔开始时启动该网站。
	-	**文件系统存储** - 当达到配额时，文件系统存储仍然可以访问以执行读取操作，但所有其他写入操作（包括正常网站活动所需的写入操作）将会被阻止。文件使用量降低或将该网站移入配额更高的 App Service 计划时，写入操作将恢复。
...
配额不是性能或成本问题，它是 Azure 通过防止租户过度使用共享资源，以在多租户环境下管理资源使用情况的方法。由于网站超出配额意味着停机或功能减少，因此如果你希望在即将达到配额时保持站点应用正常运行，请考虑以下几点：

- 将网站移到级别更高的 App Service 计划以充分利用更高的配额。例如，**基本**和**标准**计划的唯一配额是文件系统存储。
- 随着网站实例的数量增加，超过共享资源配额的可能性将随之增加。在适当情况下，请考虑在即将达到共享资源配额时回缩网站的其他实例。

##<a name="howtoconfigdiagnostics"></a>如何：为网站配置诊断和下载日志

[Azure 管理门户](https://manage.windowsazure.cn)中网站的“配置”管理页面上启用了诊断。有两种类型的诊断：“应用程序诊断”和“站点诊断”。
...
>
> - **.NET** - [使用 Visual Studio 对 Azure 网站中的网站进行故障排除](/documentation/articles/web-sites-dotnet-troubleshoot-visual-studio)
...
> [AZURE.IMPORTANT]“详细错误消息”和“失败请求跟踪”会对网站提出诸多要求。建议在重现所排查的问题后关闭这些功能。
...
1. 在 [Azure 管理门户](https://manage.windowsazure.cn)k 打开网站的“仪表板”管理页，然后记下在“诊断日志”下列出的 FTP 站点以及在“部署用户”下列出的帐户。FTP 站点是日志文件所在的位置，而“部署用户”下方列出的帐户可用于对 FTP 站点进行身份验证。
...
2. 从 Azure PowerShell 提示符处，使用以下命令下载日志文件：
	...
	这将下载 **webappname** 指定的网站的日志文件，并将这些文件保存到当前目录的 **log.zip** 文件中。
...
- 位置：/LogFiles/http/RawLogs。使用 [W3C 扩展日志格式](http://www.w3.org/TR/WD-logfile.html)设置文件中存储的信息的格式。Azure 网站不使用 s-computername、s-ip 和 cs-version 字段。
...
1.	打开“Web Apps”。单击要配置的网站的名称。
2.	单击“配置”选项卡。
3.     转到“监视”部分以便输入你的终结点设置。
4.	输入终结点的名称。
5.	输入你想要监视的网站一部分的 URL。例如，http://contoso.chinacloudsites.cn/archive。
6.	从列表中选择一个或多个地理位置。
...
9.	在最左侧的服务栏中单击“管理服务”。
10.	单击底部的“添加规则”。
11.	在“服务类型”中选择“Web Apps”，然后选择你之前配置了终结点监视的网站。单击**“下一步”**。
