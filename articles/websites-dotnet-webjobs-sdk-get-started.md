<properties
	pageTitle="在 Azure 网站中创建 .NET Web 作业 | Windows Azure"
	description="使用 ASP.NET MVC 和 Azure 创建多层应用。前端在 Azure 网站上的网站中运行，后端以 Web 作业的形式运行。应用程序使用实体框架、SQL 数据库和 Azure 存储队列和 Blob。"
...
* 将包含 Web 作业后端的应用程序发布到 Azure 网站中的网站。
...
2. 右键单击“Azure”节点，然后单击“连接到 Windows Azure”。![连接到 Azure](./media/websites-dotnet-webjobs-sdk-get-started/connaz.png)

3. 使用你的 Azure 凭据登录。

5. 在 Azure 节点下右键单击“存储”，然后单击“创建存储帐户”。![创建存储帐户](./media/websites-dotnet-webjobs-sdk-get-started/createstor.png)
...
5. 将“区域或地缘组”设置为离你最近的区域。
	...
	此设置指定哪个 Azure 数据中心将托管你的存储帐户。对于本教程，你做的选择不会带来明显的差异。但是，对于生产网站，你希望 Web 服务器和存储帐户处于同一区域，以最大程度地减少延迟和数据传出费用。（稍后要创建的）网站数据中心应尽可能靠近访问网站的浏览器，以最大程度地减少延迟。
...
* 部署到网站。Visual Studio 将自动在 Azure 网站中创建一个新的网站，并创建一个 SQL 数据库实例。
...
17. 单击“发布”。
	...
	只有在学习下一部分时在 Azure 环境中设置了连接字符串后，你才可以使用该网站。你将会看到错误页或主页，具体取决于你在前面选择的网站和数据库创建选项。
...
7. 在“服务器资源管理器”中，右键单击“Azure”>“{你的资源组}”下的网站，然后单击“查看设置”。

	“Azure 网站”窗口将在“配置”选项卡上打开。

9. 将 DefaultConnection 连接字符串的名称更改为 ContosoAdsContext。

	当你使用关联的数据库创建网站时，Azure 已自动创建此连接字符串，因此它已具有正确的连接字符串值。你只需将名称更改为代码将要查找的值。
...
10. 在“服务器资源管理器”中右键单击该网站，然后单击“停止”。

12. 网站停止后，请再次右键单击该网站，然后单击“启动”。

	Web 作业在你发布时会自动启动，但在你进行配置更改时会停止。若要重新启动它，可以重新启动网站或者在 [Azure 管理门户](https://manage.windowsazure.cn/)中重新启动 Web 作业。一般而言，建议你在进行配置更改后重新启动网站。

9. 刷新地址栏中包含网站 URL 的浏览器窗口。
...
11.	几秒钟后请刷新页面，随后将会显示缩略图。
	...
	如果未显示缩略图，你可能需要等待一分钟左右，让 Web 作业重新启动。如果经过一段时间后刷新页面时仍未显示缩略图，原因可能是 Web 作业未自动启动。在此情况下，转到 [Azure 管理门户](https://manage.windowsazure.cn)页中网站的“Web 作业”选项卡，然后单击“启动”。

### “Web 作业”选项卡

1. 在 [Azure 管理门户](https://manage.windowsazure.cn)中选择你的网站。
...
>[AZURE.NOTE]完成测试后，请删除网站和 SQL 数据库实例。网站是免费的，但 SQL 数据库实例和存储帐户是计费的（由于较小，因此费用很低）。此外，如果保持网站运行，则找到你的 URL 的任何人都可以创建和查看广告。在 Azure 管理门户中，转到网站的“仪表板”选项卡，然后单击页面底部的“删除”按钮。然后，你可以选中用于同时删除 SQL 数据库实例的复选框。如果你只是想要暂时防止其他人访问网站，请单击“停止”。在这种情况下，SQL 数据库和存储帐户会继续计费。当你不再需要 SQL 数据库和存储帐户时，可以遵循类似的过程将其删除。
...
2. 在“新建项目”对话框中，选择“Visual C#”>“Web”>“ASP.NET网站”。
...
类似代码获取对 *blobnamerequest* 队列的引用并创建一个新队列。这种情况不需要权限更改。本教程稍后的 [ResolveBlobName](#resolveblobname) 部分将说明为何只使用网站所写入到的队列来获取 Blob 名称而不生成缩略图。
...
> [AZURE.NOTE]
>
> * 如果在多个 VM 上运行网站，多个 Web 作业将同时运行，在某些情况下，这可能会导致相同的数据被处理多次。如果使用内置队列、Blob 和服务总线触发器，则这不会造成问题。SDK 可确保只会针对每个消息或 Blob 处理函数一次。
>
...
Web 作业在网站的上下文中运行，并且不可单独缩放。例如，如果你有一个标准网站实例，只运行了后台进程的一个实例，并且该实例正在使用某些服务器资源（CPU、内存等），而这些资源也可用于提供 Web 内容。

如果流量根据一天的时间或者星期变化，并且你需要执行的后端处理可以等待，则你可以将 Web 作业安排为在低流量期间运行。如果该解决方案的负载仍然太高，可以在针对该用途专用的网站中以 Web 作业形式运行后端。然后，可以独立于前端网站缩放你的后端网站。

有关详细信息，请参阅[缩放 Web 作业](/documentation/articles/websites-webjobs-resources#scale)。

### 避免因网站超时而导致其关闭

若要确保 Web 作业始终在你的网站的所有实例上运行，你必须启用 [AlwaysOn](http://weblogs.asp.net/scottgu/archive/2014/01/16/windows-azure-staging-publishing-support-for-web-sites-monitoring-improvements-hyper-v-recovery-manager-ga-and-pci-compliance.aspx) 功能。

### 在 Web 作业的外部使用 WebJobs SDK

使用 WebJobs SDK 的程序无需在 Azure 中的 Web 作业内运行。它可以在本地运行，也可以在其他环境（例如云服务辅助角色或 Windows 服务）中运行。但是，你只能通过 Azure 网站访问 WebJobs SDK 仪表板。若要使用仪表板，必须通过在 Azure 管理门户的“配置”选项卡上设置 AzureWebJobsDashboard 连接字符串，将网站连接到你所用的存储帐户。然后，可以使用以下 URL 访问仪表板：
