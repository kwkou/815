...
如果要练习创建此配置，请在指定 DNS 服务器时引用这些[值](#values)。
...
某些配置的网关子网前缀需是 /28 或更大的子网才能容纳池中所需的 IP 地址数。这意味着网关子网前缀必须是 /28、/27、/26 等。可以在此处创建较大的子网，适应将来可能的配置添加。
...
*局域网网关* 指本地位置。为本地网络网关命名，Azure 可通过该名称引用该网关。

如果要练习创建此配置，请在添加本地站点时引用这些[值](#values)。
