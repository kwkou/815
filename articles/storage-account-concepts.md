<properties pageTitle="存储帐户概念 | Azure" metaKeywords="Azure storage, storage service, service, storage account, account, create storage account, create account" description="了解有关存储帐户的概念。" linkid="manage-windows-how-to-guide-storage-accounts" urlDisplayName="How to: storage accounts"  title="Storage Account Concepts" services="storage" authors="tamram" manager="mbaldwin" editor="cgronlun"/>  

<tags ms.service="storage" ms.date="10/30/2014" wacn.date="04/11/2015"/>



# 关于 Azure 存储帐户

你的 Azure 存储帐户是可以通过其访问 Azure 存储服务（Blob、队列、表和文件服务）的唯一安全帐户。你的数据在你的存储帐户级别受到保护，且在默认情况下，它仅提供给你，即帐户所有者。 
...
[存储定价详细信息](/home/features/storage/#price) 页面提供了存储容量、复制和事务的详细定价信息。[数据传输定价详细信息](/zh-cn/pricing/details/data-transfers) 提供了针对数据流出量的详细定价信息。

本文介绍如何创建存储帐户，以及当你创建它时需要考虑的一些决策事项。它还介绍如何管理你的存储帐户访问密钥，以及如何删除存储帐户。
...
此外还可以配置用于存储帐户的自定义域名称。请参阅[为存储帐户中的 Blob 数据配置自定义域名](/zh-cn/documentation/articles/storage-custom-domain-name) 以了解详细信息。
