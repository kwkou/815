<properties
	...
	description="如果你无法连接基于 Windows 的 Azure 虚拟机，可以使用这些诊断方法和步骤来查出问题的起源。"
...
如果你无法连接到基于 Windows 的 Azure 虚拟机，你可以参考本文中的方法，有条不紊地判断和更正远程桌面连接问题的根本原因。
...
有关详细信息，请参阅 [Windows Azure IaaS (Windows) 诊断程序包知识库文章](http://support.microsoft.com/kb/2976864)。
...
根据你要尝试连接时看到的错误消息使用以下部分。

### 远程桌面连接消息窗口：由于没有可用于提供许可证的远程桌面许可证服务器，远程会话已断开连接。

原因：用于远程桌面服务器角色的 120 天许可宽限期已过期，你需要安装许可证。

一种临时解决方法是，从 Azure 门户保存 RDP 文件的本地副本，然后在 Windows PowerShell 命令提示符下运行此命令以进行连接。
...
- 如果你使用的是本地存储的 RDP 文件，请尝试使用 Azure 门户生成的 RDP 文件，以确保为虚拟机或云服务使用正确的 DNS 名称和正确的虚拟机终结点端口。

### 远程桌面连接消息窗口：发生身份验证错误。无法联系本地安全机构。

原因：要连接到的虚拟机找不到凭据的用户名部分中指示的安全机构。

如果用户名格式为“安全机构\\用户名”（例如：CORP\\User1），则 *SecurityAuthority* 部分将是虚拟机的计算机名（表示本地安全机构）或 Active Directory 域名。

此问题的可能解决方案：

- 如果该用户帐户位于虚拟机本地，请检查虚拟机名称的拼写是否正确。
- 如果该用户帐户是 Active Directory 域帐户，请检查域名的拼写是否正确。
- 如果该用户帐户是 Active Directory 域帐户且域名的拼写正确，请验证你是否可以使用 Active Directory 域的域控制器。在包含域控制器且某个域控制器计算机未启动的 Azure 虚拟网络中，这是一个常见的问题。暂时的解决办法是使用本地管理员帐户，而不要使用域帐户。
...
对于提升为新的 Active Directory 林中的域控制器的计算机，你执行提升时登录到的本地管理员帐户将转换为新的林和域中具有相同密码的等效帐户。以前的本地管理员帐户将被删除。例如，如果你使用本地管理员帐户 DC1\\DCAdmin 登录并将虚拟机提升为 corp.contoso.com 域的新林中的域控制器，则将删除 DC1\\DCAdmin 本地帐户并使用同一密码创建新的域帐户 (CORP\\DCAdmin)。
...
每台基于 Windows 的计算机都有一个远程桌面用户本地组，其中包含有权使用远程桌面连接登录的帐户和组。本地 Administrators 组的成员也具有访问权限，即使这些帐户未作为 Remote Desktop Users 本地组的成员列出也是如此。对于已加入域的计算机，本地管理员组还包含该域的域管理员。

确保你用于启动连接的帐户具有远程桌面登录权限。使用域管理员或本地管理员帐户作为创建远程桌面连接的暂时解决方法，并使用“计算机管理”管理单元将所需帐户添加到远程桌面用户本地组（“系统工具”>“本地用户和组”>“组”>“远程桌面用户”）中。
...
如果同一云服务或虚拟网络中没有其他虚拟机，可以轻松创建一个新虚拟机。有关详细信息，请参阅[在 Azure 中创建运行 Windows 的虚拟机](/documentation/articles/virtual-machines-windows-tutorial)。测试完成后，请删除多余的虚拟机。
...
若要更正使用服务管理 API 创建的虚拟机可能存在的这些问题，可以使用 Azure 虚拟机的远程 Azure PowerShell 会话。首先，必须安装虚拟机托管云服务的证书。转到[为 Azure 虚拟机配置安全远程 PowerShell 访问](http://gallery.technet.microsoft.com/scriptcenter/Configures-Secure-Remote-b137f2fe)，并将 **InstallWinRMCertAzureVM.ps1** 脚本文件下载到本地计算机上的文件夹中。
...
若要提出事件，请转到 [Azure 支持站点](/support/)，然后单击“获取帮助”。
