...
-	**高可用性/SLA**：Azure Redis 缓存可确保在至少 99.9% 的时间内提供标准/高级（在预览期结束之前，SLA 不适用于高级级别）缓存。若要了解有关 SLA 的详细信息，请参阅 [Azure Redis 缓存定价](/home/features/redis-cache/#price)。SLA 仅涉及与缓存终结点的连接。SLA 不涉及对数据丢失的防护。
