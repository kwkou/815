...
<strong>Web 套接字</strong>。设为“打开”以启用 WebSocket 协议；例如，如果你的 WEB 应用使用 [ASP.NET SignalR](http://www.asp.net/signalr) 或 [socket.io](/documentation/articles/web-sites-nodejs-chat-app-socketio/)。

<strong>始终打开</strong>。默认情况下， WEB 应用如果已处于空闲状态相当一段时间，则是未加载的状态。这样可以让系统节省资源。在“基本”或“标准”模式下，你可以启用“始终打开”<strong></strong>，以始终保持加载站点。如果你的站点运行连续的 Web 作业，则应启用“始终打开”，否则这些 Web 作业可能无法可靠运行
...
- <strong>文件系统</strong>。将日志写入 WEB 应用的文件系统。文件系统日志记录将持续 12 小时。你可以从 WEB 应用的 FTP 共享访问这些日志。（参阅 [FTP 凭据](/documentation/articles/web-sites-manage#ftp-credentials)）。
...
有关 New Relic 与 Azure WEB 应用配合使用的详细信息，请参阅 <a href="/documentation/articles/store-new-relic-web-sites-dotnet-application-performance-management/">Azure WEB 应用上的 New Relic 应用程序性能管理</a>。
...
1. 在[管理门户](http://manage.windowsazure.cn)中，选择“ WEB 应用”****显示当前登录的帐户创建的 WEB 应用的列表。
...
##<a name="howtoconfigMySQL"></a>如何配置 WEB 应用以使用 MySQL 数据库##
若要配置 WEB 应用以使用 MySQL 数据库，请遵循与使用 SQL 数据库相同的步骤，但是在“链接资源”向导中，请选择“MySQL 数据库”，而不是“SQL 数据库”。

或者，你也可以选择使用“自定义创建”选项来创建 WEB 应用。在“数据库”下拉列表中，选择“新建 MySQL 数据库”或“使用现有的 MySQL 数据库”。-->
