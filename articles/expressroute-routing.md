...
a.b.c.d/29 拆分成 a.b.c.d/30 和 a.b.c.d+4/30 并通过预配 API 一路传递到 Microsoft Azure。你将使用 a.b.c.d+1 作为主要 PE 的 VRF IP，而 Microsoft Azure 将使用 a.b.c.d+2 作为主要 MSEE 的 VRF IP。你将使用 a.b.c.d+5 作为辅助 PE 的 VRF IP，而 Microsoft Azure 将使用 a.b.c.d+6 作为辅助 MSEE 的 VRF IP。
...
可以针对每个地缘政治区域购买多个 ExpressRoute 线路。如果拥有多个连接，则可以从异地冗余中获得明显的高可用性优势。如果你有多条 ExpressRoute 线路，将从 Microsoft Azure 收到同一组公共对等互连路径的前缀。这意味着你可以使用多个路径从你的网络接入 Microsoft Azure。这可能会导致在网络中做出欠佳的路由决策。因此，你可能会在不同的服务上遇到欠佳的连接体验。
