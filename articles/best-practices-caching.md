...
如果要构建通过 HTTP 连接提供数据的网站，可以隐式强制 Web 客户端（例如浏览器或 Web 代理）在资源通过更改该资源的 URI 更新时提取最新的信息。Web 客户端通常使用资源的 URI 作为客户端缓存中的键，因此更改 URI 会导致 Web 客户端忽略任何先前缓存的资源版本，并改为提取新的版本。
...
如果你要构建通过使用 Azure Web 角色运行的 ASP.NET网站，可以将会话状态信息和 HTML 输出保存在 Azure Redis 缓存中。Azure Redis 缓存的会话状态提供程序可让你在 ASP.NET网站的不同实例之间共享会话信息，在无法建立客户端与服务器之间的关联性并且内存中缓存会话数据并不适当的 Web 场中非常有用。

配合 Azure Redis 缓存使用会话状态提供程序可带来几个好处，包括：

- 可以在 ASP.NET网站的大量实例之间共享会话状态，并提供更高的可缩放性，
...
同样地，Azure Redis 缓存的输出缓存提供程序可让你保存 ASP.NET网站生成的 HTTP 响应。配合 Azure Redis 缓存使用输出缓存提供程序可以针对呈现复杂 HTML 输出的应用程序改善响应时间；生成类似响应的应用程序实例可以使用缓存中的共享输出段，而不用重新生成此 HTML 输出。有关详细信息，请访问 Microsoft 网站上的 [Azure Redis 缓存的 ASP.NET 输出缓存提供程序](http://msdn.microsoft.com/zh-cn/library/azure/dn798898.aspx)页。
...
- Microsoft 网站上的 [Windows Azure 缓存](http://msdn.microsoft.com/zh-cn/library/windowsazure/gg278356.aspx)页。
