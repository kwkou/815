<properties 
	pageTitle="安装弹性数据库作业 | Windows Azure" 
...
1. 启动 Windows Azure PowerShell 命令窗口，并导航到 NuGet 命令行实用程序 (nuget.exe) 所下载到的目录。
...
1.	启动 Windows Azure PowerShell 命令窗口，并导航到 Microsoft.Azure.SqlDatabase.Jobs.x.x.xxx.x 文件夹下的 \\tools 子目录：键入 cd \\tools
