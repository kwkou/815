...
<tags ms.service="virtual-machines" ms.date="05/29/2015" wacn.date="06/26/2015"/>
...
当管理员创建虚拟机时，Windows Azure 将要求他们提供虚拟机名称。Windows Azure 将使用虚拟机名称作为 Azure 虚拟机资源名称。Azure 将使用同一名称作为虚拟机上安装的操作系统的计算机名称。但是，这些名称可能并非始终相同。

在基于已包含操作系统的 .VHD 文件创建虚拟机的情况下，Windows Azure 中的虚拟机名称可能不同于虚拟机的 OS 计算机名称。这种情况可能会增加虚拟机管理难度，我们不建议这样做。始终确保 Azure 虚拟机资源名称与分配给该虚拟机的操作系统的计算机名称相同。
...
存储帐户将绑定到可伸缩性目标。要熟悉当前 Azure 存储空间限制，请参阅 [Windows Azure 订阅和服务限制、配额和约束](azure-subscription-service-limits#storage-limits)。另请参阅 [Azure 存储空间可伸缩性和性能目标](storage-scalability-targets)。
...
Windows Azure 将公开云服务名称，因为这些名称会关联到域“chinacloudapp.cn”中 VIP。为了改善应用程序的用户体验，应根据需要配置虚名称来替换完全限定的云服务名称。这通常使用公共 DNS 中将资源的公共 DNS 名称（例如，www.contoso.com）映射到托管资源的云服务（例如，托管 www.contoso.com 的 Web 服务器的云服务）的 DNS 名称的 CNAME 记录完成。
...
[Windows Azure 订阅和服务限制、配额和约束](azure-subscription-service-limits#storage-limits)
