...
## 为何使用 Azure IoT 中心
除丰富的设备到云和云到设备通信选项（包括消息传送、文件传输和请求-答复方法）外，Azure IoT 中心还通过以下方式解决设备连接问题：

- **设备克隆**。借助[设备克隆][lnk-twins]，可以存储、同步和查询设备元数据及状态信息。设备克隆是存储设备状态信息（元数据、配置和条件）的 JSON 文档。IoT 中心为连接到 IoT 中心的每台设备保留一个设备克隆。
