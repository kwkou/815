...
> [AZURE.NOTE] [IoT Hub SDKs][lnk-hub-sdks] 支持将大量结果分页：

IoT 中心允许使用任意条件检索克隆筛选结果。例如，

        SELECT * FROM devices
        WHERE tags.location.region = 'CN'

检索 **location.region** 标记设置为 **CN** 的克隆。
也支持布尔运算符和算术比较，例如
...
检索位于美国、配置为以小于一分钟的频率发送遥测数据的所有克隆。此外，还可以方便地将数组常量与 **IN** 和 **NIN**（不在）运算符结合使用。例如，

        SELECT * FROM devices
        WHERE property.reported.connectivity IN ['wired', 'wifi']

检索报告了 WiFi 或有线连接的所有克隆。有关筛选功能的完整参考，请参阅 [WHERE 子句][lnk-query-where]部分。
...
请务必注意，query 对象将会根据查询所需的反序列化选项（即 twin 或 job 对象）或者要使用的纯文本 Json（如果使用投影），公开多个 **Next***。
...
目前，仅在使用聚合时才支持投影，也就是说，非聚合查询只能使用 `SELECT *`。此外，仅支持将聚合与分组结合使用。
...
目前，可以在 IoT 中心查询语言中以 **devices.jobs** 形式查询此集合。
...
检索 2016年 9 月之后为设备 **myDeviceId** 创建的所有已完成克隆更新作业。
...
每个 IoT 中心查询包括 SELECT 和 FROM 子句，以及可选的 WHERE 和 GROUP BY 子句。每个查询针对 JSON 文档的集合（例如设备克隆）运行。FROM 子句指示要迭代的文档集合（**devices** 或 **devices.jobs**）。然后，应用 WHERE 子句中的筛选器。使用聚合时，根据 GROUP BY 子句中的指定将此步骤的结果分组，对于每个组，将根据 SELECT 子句中的指定生成一行。
...
其中，**attribute\_name** 引用 FROM 集合中 JSON 文档的任一属性。在[克隆查询入门][lnk-query-getstarted]部分中可以找到 SELECT 子句的一些示例。

目前，仅支持在针对克隆执行的聚合查询中使用除 **SELECT *** 以外的选择子句。
...
* 求值为 JSON 类型的实例（例如布尔值、数字、字符串、数组或对象），
* 由设备 JSON 文档中的操作数据以及使用内置运算符和函数的常量定义。

*条件* 是求值为布尔值的表达式，也就是说，除布尔值 **true** 以外的任何常量被视为 **false**（包括 **null**、**undefined**、任何对象或数组实例、任何字符串，以及明确的布尔值 **false**）。
...
| unary\_operator | “运算符”部分所述的任何一元运算符。 |
...
了解如何使用 [IoT 中心 SDK][lnk-hub-sdks] 在应用中执行查询。
