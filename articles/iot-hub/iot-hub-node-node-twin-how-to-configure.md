...
## 创建服务应用
在此部分，会创建一个 Node.js 控制台应用，它会使用新的遥测配置对象更新与 **myDeviceId** 关联的设备克隆的*所需属性*。该应用随后会查询存储在中心的设备克隆，并显示设备的所需配置与报告配置之间的差异。
...
4. 将以下代码添加到 **SetDesiredAndQuery.js** 文件，并将 **{service connection string}** 占位符替换为在创建中心时复制的连接字符串：
    ...
    **Registry** 对象会公开从服务与设备克隆进行交互所需的所有方法。前面的代码在初始化 **Registry** 对象后检索 **myDeviceId** 的设备克隆，并使用新的遥测配置对象更新其所需属性。在此之后，它调用 **queryTwins** 函数事件 10 秒。
...
1. 恰好在 `registry.getDeviceTwin()` 调用前添加以下代码以实现 **queryTwins** 函数：
    ...
    前面的代码查询存储在中心的设备克隆，并打印所需遥测配置和报告遥测配置。请参阅 [IoT 中心查询语言][lnk-query]以了解如何跨所有设备生成丰富的报告。
...
## 后续步骤
在本教程中，从后端应用程序将所需配置设置为*所需属性*，并编写一个模拟设备应用来检测该更改及模拟多步骤更新过程（将其状态作为*报告属性*报告给设备克隆）。
