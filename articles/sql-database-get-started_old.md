...
在本教程中，您将了解有关使用 Azure 管理门户执行 Azure SQL数据库 管理任务的基础知识。如果您对数据库管理不熟悉，则可以通过这些课程在大约 30 分钟的时间内学习一些基本技能。

本教程假定您之前未使用过 SQL Server 或 Azure SQL数据库。完成本教程后，您将在 Azure 上拥有一个示例数据库，并了解如何使用管理门户执行基本管理任务。
...
-   [步骤 5：创建架构][步骤 5：创建架构]
-   [步骤 6：插入数据][步骤 6：插入数据]
-   [步骤 7：在 SQL数据库 管理门户中查询示例和系统数据][步骤 7：在 SQL数据库 管理门户中查询示例和系统数据]
...
3.  单击“SQL数据库”，然后单击“自定义创建”。
    ...
    选择此选项时，您还能够以管理员身份创建一个新服务器。作为系统管理员，您可以执行更多任务，包括连接到 SQL数据库 管理门户，本教程的后面将会介绍该任务。
...
4.  单击“自定义创建”后，将显示“数据库设置”页。在此页面中，您需要提供在服务器上创建空数据库的基本信息。将在后面的步骤中添加表和数据。
    ...
    -   选择“新建 SQL数据库 服务器”。选择新服务器时会另外添加一页，可在该页上设置管理员帐户和区域。
...
5.  如下所示填写“服务器设置”：
    ...
    -   输入一个没有空格的单词作为管理员名称。SQL数据库 在加密连接中使用 SQL 身份验证来验证用户身份。将使用您提供的名称创建一个具有管理员权限的新 SQL Server 身份验证登录名。管理员名称既不能是 Windows 用户，也不能是 Windows Live ID。SQL数据库 不支持 Windows 身份验证。
    ...
    -   确保“允许 Azure 服务访问此服务器”复选框处于选中状态，以便您能够使用 SQL数据库 管理门户、Office 365 中的 Excel 或 Azure SQL 报告连接到此数据库。

    -   完成后，请单击页面底部的复选标记。

    请注意，您没有指定服务器名称。因为 SQL数据库 服务器必须可供全球访问，所以 SQL数据库 会在创建服务器后配置适当的 DNS 条目。生成的名称可确保与其他 DNS 条目没有名称冲突。您不能更改 SQL数据库 服务器的名称。

    在下一步中，您将配置防火墙以便允许您计算机上运行的应用程序通过建立连接来访问 SQL数据库 服务器上的数据库。

## <span id="ConfigFirewall"></span></a>步骤 3：配置防火墙

若要配置防火墙以便允许连接通过，您需要在服务器页上输入信息。

**注意：**SQL数据库 服务仅适用于 TDS 协议使用的 TCP 端口 1433，因此，请确保您的网络和本地计算机上的防火墙允许端口 1433 上的传出 TCP 通信。有关详细信息，请参阅 [SQL数据库 防火墙][SQL数据库 防火墙]。

1.  在左侧导航窗格中，单击“SQL数据库”。
...
3.  在服务器页上，单击“配置”以打开防火墙配置设置并按以下方式指定规则：
    ...
    -   复制当前客户端 IP 地址。它是您的路由器或代理服务器侦听的 IP 地址。SQL数据库 会检测当前连接所使用的 IP 地址，以便您可以创建一个接受来自该设备的连接请求的防火墙规则。
...
现在，您已在 Azure 上拥有了 SQL数据库 服务器、允许访问该服务器的防火墙规则、数据库对象以及管理员登录名。接下来，您将使用 SQL数据库 管理门户中的查询窗口运行 Transact-SQL 脚本，以创建一个预定义数据库。
...
在本步骤中，您需要运行两个脚本。第一个脚本创建一个定义表、列和关系的架构。第二个脚本添加数据。每个步骤在单独的连接中独立执行。如果您之前已在 SQL Server 中构建数据库，那么您将在 SQL数据库 中看到的差异之一是 CREATE 和 INSERT 命令必须在单独的批处理文件中运行。SQL数据库 实施此要求是为了最大程度地减少数据在传送过程中受到的攻击。

**注意：**相关架构和数据值摘自此 [MSDN 文章][MSDN 文章]且已经过修改，可用于 SQL数据库。
...
3.  单击页面底部的“管理”。如果看不到“管理”，请刷新浏览器窗口。这将打开 SQL数据库 管理门户。此门户与 Azure 管理门户不同。您将使用此门户运行 Transact-SQL 命令和查询。
...
5.  在 SQL数据库 管理门户中单击“新建查询”。将在工作区中打开一个空查询窗口。在下一步中，您将使用此窗口来复制用于将结构和数据添加到空数据库的一系列预定义脚本。
...
## <span id="QueryDBSysData"></span></a>步骤 7：在 SQL数据库 管理门户中查询示例和系统数据

若要检查您的工作，请运行可返回您刚输入的数据的查询。您也可以运行内置存储过程和数据管理视图，这些视图可提供有关 SQL数据库 服务器上运行的数据库的信息。
...
在 SQL数据库 中，您可以使用 Transact-SQL 创建登录名和授予权限。在本课程中，您将使用 Transact-SQL 执行以下三项操作：创建 SQL Server 身份验证登录名、创建数据库用户并通过角色成员身份授予权限。

SQL Server 身份验证登录名用于建立服务器连接。访问 SQL数据库 服务器上数据库的所有用户均要通过提供 SQL Server 身份验证登录名和密码来建立连接。

若要创建登录名，您必须先连接到 **master** 数据库。

#### <span id="CreateLogin"></span></a>创建 SQL Server 身份验证登录名

1.  在[管理门户][管理门户]中，选择“SQL数据库”、单击“服务器”、选择该服务器，然后单击白色箭头以打开
    服务器页。

    ![图像 5][图像 5]

2.  在“快速启动”页上，单击“管理服务器”以打开一个指向 SQL数据库 管理门户的新连接。

3.  输入管理员名称和密码。这是您在创建服务器时所指定的管理员登录名。

    ![图像 20][图像 11]

4.  将在新浏览器窗口中打开 SQL数据库 管理门户。单击顶部的“选择数据库”，然后单击 **master**。
...
1.  返回到可连接 **School** 数据库的 SQL数据库 管理门户页。如果您关闭了浏览器窗口，请使用上一课程“使用 Transact-SQL 脚本添加数据和架构”中的步骤启动一个到 **School** 数据库的新连接。

    在 SQL数据库 管理门户页上，**School** 数据库名称将显示在左上角。
...
2.  在数据连接向导中，输入 SQL数据库 服务器的完全限定域名，后跟具有数据库访问权限的 SQL Server 身份验证登录名。

可在“快速链接”下的“数据库”页上找到服务器名称。在 Azure 管理门户上，您可以在 SQL数据库“服务器”页的“仪表板”上的“管理 URL”中找到该服务器名称。

服务器名称由一串字母和数字组成，后跟“.database.chinacloudapi.cn”。在数据库连接向导中指定此名称。在指定该名称时不要包含 http:// 或 https:// 前缀。
...
<!-- <h2><a id="ConfigureDataSync"></a>Step 10: Configure SQL Data Sync</h2>  <h4>SQL Data Sync</h4>  Now that you've created your SQL数据库 instances, you can leverage SQL Data Sync to keep your high value data synchronized across multiple locations.  SQL Data Sync is a feature of SQL数据库 that enables you to synchronize selected data, either on a schedule or on-demand, without writing any code or scripts. SQL Data Sync supports synchronization between SQL数据库 instances or hybrid topologies that include SQL数据库s and instances of SQL Server.  For more information about SQL Data Sync, see [Getting Started with SQL Data Sync](http://go.microsoft.com/fwlink/?LinkId=274959). -->

## <span id="NextSteps"></span></a>后续步骤

</p>
现在，您已了解 SQL数据库 和管理门户，接下来，您可以尝试 SQL Server 数据库管理员使用的其他一些工具和方法。

若要主动管理新数据库，请考虑安装和使用 SQL Server Management Studio。Management Studio 是用于管理 SQL Server 数据库（包括在 Azure 上运行的数据库）的主要数据库管理工具。使用 Management Studio，您可以保存查询以供将来使用、添加新表和存储过程，以及磨练您在丰富脚本环境（包括语法检查程序、智能感知和模板）中掌握的 Transact-SQL 技能。若要开始操作，请按照[使用 SQL Server Management Studio 管理 SQL数据库][使用 SQL Server Management Studio 管理 SQL数据库] 中的说明操作。
...
也可采用其他方法将本地数据库移动到 SQL数据库。如果您拥有现有数据库，或者下载了示例数据库进行练习，请尝试以下替代方法：

-   [将数据库迁移到 SQL数据库][将数据库迁移到 SQL数据库]
-   [在 SQL数据库 中复制数据库][在 SQL数据库 中复制数据库]
  ...
  [步骤 7：在 SQL数据库 管理门户中查询示例和系统数据]: #QueryDBSysData
  ...
  [SQL数据库 防火墙]: http://social.technet.microsoft.com/wiki/contents/articles/2677.sql-azure-firewall-zh-cn.aspx
  ...
  [将数据库迁移到 SQL数据库]: http://msdn.microsoft.com/zh-cn/library/azure/ee730904.aspx
  [在 SQL数据库 中复制数据库]: http://msdn.microsoft.com/zh-cn/library/azure/ff951624.aspx
