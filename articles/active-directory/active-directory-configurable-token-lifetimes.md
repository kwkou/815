...
>
>下午 12:15，用户尝试访问 Web 应用程序 B。浏览器重定向到 Azure AD 并检测会话 Cookie。Web 应用程序 B 的服务主体已链接到策略 1，但同时也属于使用默认策略 2 的父租户。由于链接到服务主体的策略优先级高于租户默认策略，因此策略 2 生效。会话令牌最初是在过去 30 分钟内颁发的，因此被视为有效。用户使用授予权限的 ID 令牌重定向回到 Web 应用程序 B。
>
