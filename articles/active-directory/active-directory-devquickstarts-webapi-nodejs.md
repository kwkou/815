...
> [AZURE.NOTE]
> 本文未涵盖如何使用 Azure AD B2C 来实施登录、注册和配置文件管理，而是着重介绍如何在用户已通过身份验证后调用 Web API。如果尚未开始，应该先从[如何与 Azure Active Directory 集成文档](/documentation/articles/active-directory-how-to-integrate/)入手，了解 Azure Active Directory 的基础知识。
> 
> 
...
## 关于 Node.js 模块
在本演练中，我们将使用 Node.js 模块。模块是可加载的 JavaScript 包，可为应用程序提供特定功能。你通常会使用 Node.js NPM 命令行工具在 NPM 安装目录中安装模块，但一些模块（如 HTTP 模块）是作为核心 Node.js 包的一部分提供的。安装的模块保存在 Node.js 安装目录的根目录下的 node\_modules 目录中。node\_modules 目录中的每个模块都保留自己的 node\_modules 目录，其中包含它依赖的所有模块，每个必需的模块都有一个 node\_modules 目录。这种递归的目录结构表示了依赖关系链。

这种依赖关系链结构会导致应用程序占用空间变大，但保证满足所有依赖项的要求，并且开发中使用的模块版本也将在生产中使用。这使得生产应用程序的行为更有预测性，并防止出现影响用户的版本控制问题。

## 1\.注册 Azure AD 租户
若要使用本示例，需要一个 Azure Active Directory 租户。如果不确定什么是租户或者如何获取租户，请参阅[如何获取 Azure AD 租户](/documentation/articles/active-directory-howto-tenant/)。
...
- 提醒：为应用程序创建一个**应用程序密码**并复制下来。稍后将需要使用它。
- 提醒：复制分配给应用的**应用程序 ID**。稍后也会用到。

## 3\.下载适用于平台的 node.js
若要成功使用本示例，必须正确安装 Node.js。

请从 [http://nodejs.org](http://nodejs.org) 安装 Node.js。

## 4\.在平台上安装 MongoDB
若要成功使用本示例，必须正确安装 MongoDB。我们将使用 MongoDB 使 REST API 持久保留在服务器实例之间。
...
我们将使用 Resitfy 生成 REST API。Restify 是从 Express 派生的精简灵活的 Node.js 应用程序框架，它提供一套可靠的功能用于在 Connect 顶层生成 REST API。
...
输入以下命令，在 node\_modules 目录中安装以下模块：
...
server.js 文件将提供 Web API 服务器的大多数功能。我们要将大部分代码添加到此文件。若要用于生产，需要将功能重构为较小的文件，例如单独的路由和控制器。在本演示中，我们将为此功能使用 server.js。
...
此代码文件会将配置参数从 Azure Active Directory 门户传递到 Passport.js。在本演练的第一部分中向门户添加 Web API 时，已经创建了这些配置值。我们将解释在复制代码后，要输入哪些参数值。
...
回顾我们在 ***步骤 11*** 中创建的 `config.js` 文件，我们将数据库称为 `tasklist`，因为这是我们在 mogoose\_auth\_local 连接 URL 的末尾放置的内容。用户无需事先在 MongoDB 中创建此数据库，用户首次运行服务器应用程序时，系统将创建此数据库（假定它不存在）。

现在，我们已告诉服务器要使用哪个 MongoDB 数据库，接下来我们需要编写一些附加代码，为服务器任务创建模型和架构。
...
## 14\.为 Task REST API 服务器添加路由

现在，我们已经创建了一个可用的数据库模型，接下来让我们添加用于 REST API 服务器的路由。

### 关于 Restify 中的路由

Restify 中路由的工作原理与使用 Express 堆栈时的路由工作原理完全相同。可以使用客户端应用程序应该调用的 URI 定义路由。通常，需要在单独的文件中定义路由。若用于本演练，我们将在 server.js 文件中定义路由。若用于生产，我们建议在各自的文件中定义路由。
...
这是最基本级别的模式。Resitfy（和 Express）提供了更深层的功能，例如，定义应用程序类型，以及跨不同的终结点执行复杂路由。若用于本演练，我们将使这些路由保持精简。
...
## 15\.创建服务器！
我们已经定义了数据库，部署了路由，最后一件事就是添加用于管理调用的服务器实例。
...
执行此操作最简单的方法是在命令行中使用 curl。在此之前，我们需要一个简单实用工具，用于将输出分析为 JSON。为此，请安装 json 工具，因为下面的所有示例都要使用该工具。
...
现在，我们已有正在运行的 REST API（顺便祝贺你！），接下来将它用于 Azure AD。
...
到目前为止，我们已构建一个典型的 REST TODO 服务器，其中不包含任何授权种类。我们在此开始汇总。
...
> [AZURE.IMPORTANT]
上述代码接受正好向服务器进行身份验证的任何用户。这便是自动注册。在生产服务器中，你希望所有人都必须先经历你确定的注册过程。这通常是在使用者应用中看到的模式，允许注册 Facebook，但接着请求填写其他信息。如果这不是命令行程序，我们就只能从返回的令牌对象中提取电子邮件，然后请求他们填写其他信息。由于这是测试服务器，因此，我们直接将它们加入到内存中的数据库。
> 
> 
...
## 祝贺你！ 你已经使用 OAuth2 实现了 REST API 服务！
