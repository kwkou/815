...
除了用户对象外，还可以对其他对象（如组和应用程序）执行类似操作。若要对目录调用图形 API，应用程序必须向 Azure AD 注册并配置为允许访问目录。通常通过用户或管理员同意流可以实现这一点。
...
- **受权限范围保护**：AAD 图形 API 会公开权限范围，以启用对 AAD 数据的安全/同意访问，并支持各种客户端应用类型，包括：
  ...
  - 使用服务/守护程序客户端（应用角色）等应用程序定义的基于角色的访问控制的应用
    ...
    委托和应用角色权限范围都表示图形 API 公开的特权，客户端应用程序可以通过应用程序注册权限请求这些权限范围（[Azure 经典管理门户中的功能](https://manage.windowsazure.cn)）。客户端可以通过检查委派权限的访问令牌中收到的范围（“scp”）声明，以及应用角色权限的角色（“roles”）声明，验证为其授予的权限范围。了解有关 [Azure AD 图形 API 权限范围](https://msdn.microsoft.com/Library/Azure/Ad/Graph/howto/azure-ad-graph-api-permission-scopes)的详细信息。
