...
+ [Azure CDN代申请证书和用户自有证书的HTTPS加速服务的区别是什么？](#step28)
...
## **Azure CDN代申请证书和用户自有证书的HTTPS加速服务的区别是什么？**<a id="step28"></a>

Azure CDN代为申请证书的HTTPS加速服务和用户自有证书都是自助化配置，性能上差异不大，主要区别在于价格和证书：

1. 价格：
    - Azure代为申请证书的方式涉及到证书的申请和维护，属于高级版CDN服务；
    - 用户上传自有证书的HTTPS加速服务，将于2017年7月1日起，属于标准CDN服务。高级服务和标准服务价格请参考[Azure官方网站](https://www.azure.cn/pricing/details/cdn/)。
2. 证书：
    - Azure代为申请证书的SSL证书类型为SAN多域名证书（SAN/UCC SSL），SANs SSL证书允许您在同一张证书中，添加多个需要保护的"域名"或"服务器"名。证书签发机构： https://www.digicert.com/。
    - 用户自有证书支持SNI证书，SNI证书可以多个HTTPS 客户共享同一个IP 地址。
3. 证书部署时间：
    - Azure CDN代为申请证书的部署时间是7个工作日。
    - 用户自有证书的方式，部署时间是2小时左右。
4. 选择建议：
    - 如果用户已经有加速域名对应的证书，可以选择用户自有证书的方式。SNI证书不支持Windwos XP系统下所有的IE版本，浏览器会提示不受信任。
    - 如果没有证书，又不想自己申请，可以选择Azure CDN代申请证书的方式。
