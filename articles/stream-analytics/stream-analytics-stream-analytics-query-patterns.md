...
## 查询示例：数据类型转换
**说明**：定义输入流中属性的类型。例如，车重在输入流中是字符串格式的，需要转换为 INT 才能执行 SUM 操作。
...
**说明**：根据某些条件，为字段提供不同的计算。例如，针对通过的相同品牌的汽车中有多少符合 1 的特殊情况提供字符串说明。
...
## 查询示例：将数据发送到多个输出
**说明**：将数据从单个作业发送到多个输出目标。例如，分析基于阈值的警报数据，并将所有事件存档到 blob 存储
...
**说明**：INTO 子句告诉流分析哪一个输出可以通过此语句写入数据。第一个查询将我们接收到的数据传递到名为 ArchiveOutput 的输出。第二个查询进行了一些简单的聚合和筛选操作，然后将结果发送到下游的报警系统。*注意*：可以重复使用多个输出语句中的 CTE（即 WITH 语句）结果 – 这有一些额外的好处，在读取输入源时，可以打开较少的读取器。例如，
...
## 查询示例：对唯一值进行计数
**说明**：计算某时间范围内流中出现的唯一字段值数。例如，2 秒内有多少辆唯一品牌的汽车通过了收费亭？
...
## 查询示例：检测缺少的事件
**说明**：检查流有没有与特定条件匹配的值。例如，90 秒内是否有 2 辆同一品牌的汽车连续通过收费路段？
...
## 查询示例：检测某个条件的持续时间
**说明**：了解某情况的持续时间。例如，假设某个 Bug 导致所有车的重量不正确（超出 20,000 磅），我们需要计算该 Bug 的持续时间。
