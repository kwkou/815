...
浏览交互模式时，我们会以相同的关注度来查看设备控制和设备数据。根据分类，设备控制可以是任何一方提供给设备的任何信息，其目的是根据其状态或其环境的状态更改或影响其行为。根据分类，设备数据可以是设备向任何其他方发出的有关其状态以及观察到的其环境状态的任何信息。
...
浏览交互模式时，我们在进行威胁建模时会以相同的关注度来查看设备控制和设备数据。根据分类，设备控制可以是任何一方提供给设备的任何信息，其目的是根据其状态或其环境的状态更改或影响其行为。根据分类，设备数据可以是设备向任何其他方发出的有关其状态以及观察到的其环境状态的任何信息。
...
下图提供了简化的 Azure IoT 体系结构视图，其中使用 Microsoft 威胁建模工具所用的数据流原理图模型：
