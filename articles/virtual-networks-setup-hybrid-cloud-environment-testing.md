...
使用[测试实验室指南：Windows Server 2012 R2 的基础配置](https://www.microsoft.com/zh-CN/download/details.aspx?id=39638)的"配置 Corpnet 子网的步骤"部分中的说明在名为 Corpnet 的子网上配置 DC1、APP1 和 CLIENT1 计算机。**此子网必须与组织网络隔离，因为它将通过 RRAS1 计算机直接连接到 Internet。** 
...
在 RRAS1 上的管理员级 Windows PowerShell 命令提示符处使用这些命令。在运行这些命令之前，请填写变量值并删除 < 和 > 字符。你可以从显示的 **Get-NetAdapter** 命令中获取网络适配器的当前名称。
...
接下来，请按[如何安装和配置 Azure PowerShell](/documentation/articles/install-configure-powershell) 中的说明 在你的本地计算机上安装 Azure PowerShell。
...
如果接口名称不是"Ethernet"，请使用 **Get-NetAdapter** 命令来确定接口名称。
...
1.	在本地计算机上的 Azure 管理门户中，单击左窗格中的**"网络"**，然后单击**"TestVNET"**和**"仪表板"**。
