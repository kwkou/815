...
本指南说明如何以编程方式从 Python 执行常见服务管理任务。[Azure SDK for Python][download-SDK-Python] 中的 **ServiceManagementService** 类支持以编程方式访问[管理门户][management-portal]中提供的众多与服务管理相关的功能（例如**创建、更新和删除云服务、部署、数据管理服务、虚拟机以及地缘组**）。此功能可用于构建需要以编程方式访问服务管理的应用程序。 
...
> [WACOM.NOTE] 从 Azure SDK for Python v0.8.0 开始，在 Windows 上运行时，可以使用通过 OpenSSL 创建的证书。这需要 Python 2.7.4 版或更高版本。

### Windows 上的管理证书 (MakeCert)

你可以使用"makecert.exe"在你的计算机上创建自签名管理证书。以**管理员**身份打开 **Visual Studio 命令提示符**，并使用以下命令将"AzureCertificate"替换为你要使用的证书名称。
...
在创建云服务、存储服务或地缘组时，你将需要提供有效位置。**list_locations** 方法将始终返回当前可用位置的最新列表。截止到本文撰写时为止，可用位置为：
...
你可以通过将服务名称传递到 **delete\_hosted\_service** 方法来删除云服务：
...
* **package_url**：部署包（.cspgk 文件）的 URL。包文件必须存储在 Azure Blob 存储帐户中，该帐户必须与包要上载到的托管服务属于同一个订阅。你可以使用 [Azure PowerShell cmdlet] 或使用 [cspack 命令行工具]创建部署包。
* **configuration**：编码为 base64 的服务配置文件（.cscfg 文件）。
* **label**：托管服务名称的标签（自动编码为 base64）。

以下示例为名为"myhostedservice"的托管服务创建新部署"v1"：
...
请注意，在上面的示例中，可以通过将 **create\_deployment** 返回的结果传递到 **get\_operation\_status** 方法来检索 **create\_deployment** 操作的状态。

可以使用 **get\_deployment\_by\_slot** 或 **get\_deployment\_by\_name** 方法访问部署属性。下面的示例通过指定部署槽来检索部署。此示例还循环访问部署的所有实例：
...
可以使用 **change\_deployment\_configuration** 方法或 **update\_deployment\_status** 方法更新部署。
...
请注意，在上面的示例中，可以通过将 **change\_deployment\_configuration** 返回的结果传递到 **get\_operation\_status** 方法来检索 **change\_deployment\_configuration** 操作的状态。
...
Azure 提供两种部署环境：过渡和生产。通常，在将服务部署到生产环境之前，会将服务部署到过渡环境以进行测试。在需要将过渡环境中的服务提升到生产环境时，你可以执行此操作而无需重新部署该服务。这可通过交换部署来完成。（有关交换部署的详细信息，请参阅[部署 Azure 服务]。）

以下示例演示如何使用 **swap\_deployment** 方法来交换两个部署（部署名称分别为"v1"和"v2"）。在该示例中，在调用 **swap\_deployment** 之前，部署"v1"位于生产槽中，部署"v2"位于过渡槽中。在调用 **swap\_deployment** 之后，"v2"位于生产槽中，"v1"位于过渡槽中。  
...
你可以通过将存储服务名称传递到 **delete\_storage\_account** 方法来删除存储服务。删除存储服务会删除该服务中存储的所有数据（Blob、表和队列）。
...
你可以通过调用 **list\_affinity\_groups** 方法列出地缘组并检查其属性：
...
你可以通过将组名传递到 **delete\_affinity\_group** 方法来删除地缘组。请注意，地缘组必须与任何服务解除关联（或者必须删除使用地缘组的服务），然后才能删除该地缘组。
...
[download-SDK-Python]: /zh-cn/documentation/articles/python-how-to-install/
...
[存储服务]: /zh-cn/documentation/articles/storage-introduction/
[azure-blobs]: /zh-cn/documentation/articles/storage-python-how-to-use-blob-storage/
[azure-tables]: /zh-cn/documentation/articles/storage-python-how-to-use-table-storage/
[azure-queues]: /zh-cn/documentation/articles/storage-python-how-to-use-queue-storage/
[Azure 服务配置架构 (.cscfg)]: http://msdn.microsoft.com/zh-cn/library/windowsazure/ee758710.aspx
[云服务]: http://msdn.microsoft.com/zh-cn/library/windowsazure/jj155995.aspx
