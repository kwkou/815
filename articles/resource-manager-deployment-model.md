...
在本主题中，你将了解 Azure Resource Manager 和经典部署模型、资源的状态，以及为何要使用不同的模型来部署资源。Resource Manager 部署模型包含与经典部署模型的重要差异，这两个模型彼此之间并不完全兼容。若要简化资源部署和管理，Azure 建议您为新资源使用资源管理器，如果可能，请通过资源管理器重新部署现有资源。
