<properties
   pageTitle="在 Windows Azure 存储空间中使用 Azure 密钥保管库加密和解密 Blob"
   description="本教程将指导您如何将 Windows Azure 存储空间的客户端加密与 Azure 密钥保管库配合使用来加密和解密 blob"
   services="storage"
   documentationCenter=""
   authors="adhurwit"
   manager=""
...
# 在 Windows Azure 存储空间中使用 Azure 密钥保管库加密和解密 blob
...
有关 Azure 存储空间的客户端加密的概述信息，请参阅 [Windows Azure 存储空间客户端加密 – 预览](http://blogs.msdn.com/b/windowsazurestorage/archive/2015/04/28/client-side-encryption-for-microsoft-azure-storage-preview.aspx)
...
下面是一个 blob 的当前 Azure 管理门户的屏幕截图，该 blob 已使用客户端加密通过密钥保管库中存储的密钥进行加密。KeyId 属性是密钥保管库中充当密钥加密密钥 (KEK) 的密钥的 URI。EncryptedKey 属性包含内容加密密钥 (CEK) 的加密版本。
...
## 解密 blob 并下载
解密就是解析程序类存在的价值。用于加密的密钥的 ID 与其元数据中的 Blob 相关联，因此您没有理由检索该密钥，请记住密钥与 blob 之间的关联关系。您只需确保该密钥保留在密钥保管库中。

RSA 密钥的私钥则保留在密钥保管库中，为了进行解密，来自包含 CEC（内容加密密钥）的 blob 元数据的加密密钥将发送到密钥保管库进行解密。
...
以下是在 PowerShell 中，在密钥保管库中创建可用作 SymmetricKey 的密码的示例：
...
在控制台应用程序中，可以使用与之前相同的调用将此密码作为 SymmetricKey 进行检索。
...
有关将 Windows Azure 存储空间与 C# 配合使用的详细信息，请参阅[用于 .NET 的 Windows Azure 存储客户端库](https://msdn.microsoft.com/zh-cn/library/azure/dn261237.aspx)

有关 Blob REST API 的详细信息，请参阅 [Blob 服务 REST API](https://msdn.microsoft.com/zh-cn/library/azure/dd135733.aspx)

有关 Windows Azure 存储空间的最新信息，请转到 [Windows Azure 存储空间团队博客](http://blogs.msdn.com/b/windowsazurestorage/)
