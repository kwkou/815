<properties
   pageTitle="为 Expressroute 配置虚拟网络 | Windows Azure"
...
这些步骤将指导你完成为 ExpressRoute 配置虚拟网络和网关。
...
6. 在“虚拟网络地址空间”页上，输入以下信息，然后单击右下角的复选框以配置网络。
	...
	- 添加网关子网 - 单击此项可添加网关子网。网关子网仅用于此虚拟网络网关，并且是此配置必需的。***重要说明：***ExpressRoute 的网关子网必须是 /28 或更大值。
...
11. **将你的网络连接到线路。** 只有在你已确认线路已转为以下状态后，才继续按下面的说明进行操作： 
	...
	请确认你至少拥有一个已创建网关的 Azure 虚拟网络。网关子网必须是 /28 或更大值才能使用 ExpressRoute 连接，并且必须已启动且正在运行。
