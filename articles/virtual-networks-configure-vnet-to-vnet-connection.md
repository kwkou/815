<properties
   pageTitle="配置 VNet 到 VNet 连接 | Windows Azure"
...
本文将引导你执行相关步骤，以便使用经典部署模型（也称为服务管理）来创建虚拟网络并将其连接到一起。
...
此过程主要使用 Azure 经典门户，但必须使用 Windows Azure PowerShell cmdlet 来连接 VPN 网关。
...
完成前面的所有步骤后，需要将 IPsec/IKE 预共享密钥设置为相同。你可以使用 REST API 或 PowerShell cmdlet 完成此操作。如果使用 PowerShell，请确认你使用的是[最新版本](/downloads)的 Windows Azure PowerShell cmdlet。以下示例使用 PowerShell cmdlet 将密钥值设置为 A1b2C3D4。请注意，两个 VNet 应使用相同的密钥值。请编辑以下示例，使之反映你自己的值。
