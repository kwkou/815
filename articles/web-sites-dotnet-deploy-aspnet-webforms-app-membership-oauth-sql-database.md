<properties 
	...
	description="本教程说明如何生成整合了 SQL 数据库的安全 ASP.NET 4.5 Web 窗体网站并将其部署到 Azure。" 
...
##概述
本教程说明如何生成整合了 SQL 数据库的安全 ASP.NET 4.5 Web 窗体网站并将其部署到 Azure。
...
本教程假定你之前未使用过 Windows Azure。完成本教程之后，你将能够在云中启动并运行使用云数据库的网站。
...
你将生成一个简单的联系人列表网站，该应用程序基于 ASP.NET 4.5 Web 窗体构建并使用实体框架进行数据库访问。下图显示了用于对数据库进行读取和写入访问的 Web 窗体页：
...
2. 在“发布 Web”对话框的“配置文件”选项卡中，单击“Azure 网站”。
	  
3. 如果你尚未登录，请在“选择现有网站”对话框中单击“登录”按钮。完成登录后，选择你在本教程第一部分中创建的网站。单击“确定”继续。![选择“现有网站”对话框](./media/web-sites-dotnet-deploy-aspnet-webforms-app-membership-oauth-sql-database/SecureWebForms07.png) Visual Studio 将下载发布设置。
...
>[AZURE.NOTE]如果在发布已建立的网站时遇到错误，则可以在添加新的文件之前清除该位置。再次发布应用程序，但这次请在“发布 Web”对话框中选择“设置”选项卡。然后，将配置设置为“调试”，并选择“删除目标位置的其他文件”选项。选择“发布”以再次部署应用程序。![“发布 Web”对话框](./media/web-sites-dotnet-deploy-aspnet-webforms-app-membership-oauth-sql-database/SecureWebForms11.png)
...
3. 复制 **SSL URL** 以便稍后使用。SSL URL 将为 `https://localhost:44300/`，除非你之前已创建 SSL网站（如下所示）。![项目属性](./media/web-sites-dotnet-deploy-aspnet-webforms-app-membership-oauth-sql-database/SecureWebForms18.png)  
...
7. 在“创建客户端 ID”对话框中，保留默认“网站”作为应用程序类型。  
...
15. 输入你的凭据后，系统将提示你刚创建的网站，以便授予其权限：![项目默认服务帐户](./media/web-sites-dotnet-deploy-aspnet-webforms-app-membership-oauth-sql-database/SecureWebForms21f.png)  
...
##使用成员资格 API 来限制访问 
ASP.NET 标识是在生成 ASP.NET网站时用于身份验证的成员资格系统。它可以轻松地将特定于用户的配置文件数据与应用程序数据相集成。此外，ASP.NET 标识允许你为应用程序中的用户配置文件选择持久模型。你可以将数据存储在 SQL Server 数据库或其他数据存储中，包括 Azure 存储表等 *NoSQL* 数据存储。
...
3. 从“配置文件”选项卡中，选择“Azure 网站”作为发布目标（如果尚未选择）。![“发布 Web”对话框](./media/web-sites-dotnet-deploy-aspnet-webforms-app-membership-oauth-sql-database/SecureWebForms23.png)  
4. 如果你尚未登录，请单击“登录”。
5. 从“现有网站”下拉框中选择你在本教程前面创建的现有网站，然后单击“确定”按钮。![选择“现有网站”对话框](./media/web-sites-dotnet-deploy-aspnet-webforms-app-membership-oauth-sql-database/SecureWebForms25.png) 如果系统询问你是否保存对配置文件所做的更改，请选择“是”。
...
###停止应用程序 
为了防止其他人注册和使用你的示例应用程序，你可以停止该网站。

1. 在 Visual Studio 中，从“视图菜单”选择“服务器资源管理器”。 
2. 在“服务器资源管理器”中，导航到“网站”。
3. 右键单击每个网站实例并选择“停止网站”。![停止网站菜单项](./media/web-sites-dotnet-deploy-aspnet-webforms-app-membership-oauth-sql-database/SecureWebForms26a.png)  

	或者，也可以从 Windows Azure 管理门户中选择该网站，然后单击页面底部的“停止”图标。![添加新联系人页](./media/web-sites-dotnet-deploy-aspnet-webforms-app-membership-oauth-sql-database/SecureWebForms26b.png)
...
##后续步骤
有关 ASP.NET Web 窗体的详细信息，请参阅 ASP.NET网站上的 [了解 ASP.NET Web 窗体](http://www.asp.net/web-forms)和 [Windows Azure 教程和指南](/documentation/services/web-sites/#net)。
