...
6. 遵循此处所述的步骤选择存储冗余选项。
    ...
    如果你要使用 Azure 作为主要备份存储终结点（例如，你要从 Windows Server 备份到 Azure），请考虑选择（默认的）[异地冗余存储](../storage/storage-redundancy#geo-redundant-storage)选项。
...
## 步骤 2：下载保管库凭据文件
本地计算机需要先在备份保存库中通过身份验证才能将数据备份到 Azure。身份验证是通过保管库凭据实现的。从 Azure 门户通过安全通道下载保管库凭据文件。证书私钥不会在门户或服务中持久保存。
