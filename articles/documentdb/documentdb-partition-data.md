...
[Azure DocumentDB](/home/features/documentdb/) 旨在帮助你实现快速、可预测的性能并且随着应用程序的增长无缝扩展。 本文概述 DocumentDB 分区的工作原理，并且描述如何配置 DocumentDB 集合以有效地扩展应用程序。
...
在 DocumentDB 中，可在任何规模以毫秒级响应时间对无架构 JSON 文档进行存储和查询。 DocumentDB 提供了容器来存储名为 **collections**的数据。 集合是逻辑资源，并且可以跨一个或多个物理分区或服务器。 DocumentDB 基于存储大小和预配的集合吞吐量确定分区数。 DocumentDB 中的每个分区都具有固定大小的与之关联的 SSD 支持的存储，并且会复制分区以实现高可用性。 分区完全由 Azure DocumentDB 进行管理，因此，你无需编写复杂的代码或亲自管理分区。 DocumentDB 集合在存储和吞吐量方面是 **几乎无限制** 的。 
...
## 分区键 <a name="partition-keys"></a>
选择分区键是设计时需要做出的一项重要决定。 选择的 JSON 属性名必须具有一系列的值，且有望均匀地分布访问模式。 
...
## <a name="single-partition-and-partitioned-collections"></a>单区集合和分区集合
...
从 SDK 1.12.0 及更高版本开始，DocumentDB 支持使用 SQL 对已分区的集合执行[聚合函数](/documentation/articles/documentdb-sql-query/#Aggregates/) `COUNT`、`MIN`、`MAX`、`SUM` 和 `AVG`。 查询必须包括单个聚合运算符，并且必须在投影中包括单个值。
...
## 从单个分区迁移到 DocumentDB 中的分区集合 <a name="migrating-from-single-partition"></a>

> [AZURE.IMPORTANT]
> 如果要导入到 MongoDB 的 API，请按照这些[说明](/documentation/articles/documentdb-mongodb-migrate/)操作。
> 
> 
...
- 每个租户一个集合：每个租户都有自己的集合。 在此模型中，你可以依据租户保留性能。 DocumentDB 采用新的基于消耗的定价模型，这种模型对拥有少量租户的多租户应用程序更加划算。
