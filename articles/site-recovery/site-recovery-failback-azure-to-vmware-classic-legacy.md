...
> [AZURE.SELECTOR]
- [Azure 经典管理门户](/documentation/articles/site-recovery-failback-azure-to-vmware-classic/)
- [Azure 经典管理门户（旧版）](/documentation/articles/site-recovery-failback-azure-to-vmware-classic-legacy/)
...
4. 查看是否存在包含 **disk.EnableUUID** 的行。如果存在该行且其值设置为 **False**，请将该值设置为 **True**（不区分大小写）。如果存在该行并且其值设置为 true，请单击“取消”，然后在启动来宾操作系统后，在该系统中测试 SCSI 命令。如果不存在该行，请单击“添加行”。
...
4.  在打开的新窗口中，针对要故障回复的 VM 选择“操作系统类型”>“获取详细信息”。在“主服务器详细信息”中，找到并选择你要保护的虚拟机。VM 将按照其在故障转移之前的 vCenter 主机名列出。
...
3. 你可以基于多个选项进行恢复，但建议使用“最新标记”并选择“对所有 VM 应用”，以确保使用虚拟机中的最新数据。
4. 运行“就绪状态检查”。 这将会检查是否配置了正确的参数来启用 VM 恢复。如果所有检查都已成功，请单击“下一步”。否则，请查看日志并解决错误。

	![](./media/site-recovery-failback-azure-to-vmware/image39.png)

8.  在“VM 配置”中，确保恢复设置设置正确。你可以根据需要更改 VM 设置。
