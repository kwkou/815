...
若要对应用程序网关配置端到端 SSL，需要网关证书和后端服务器证书。网关证书用于加密和解密通过 SSL 发送给它的流量。网关证书需要采用个人信息交换 (pfx) 格式。此文件格式适用于导出私钥，后者是应用程序网关对流量进行加解密所必需的。

若要加密端到端 SSL，后端必须已加入应用程序网关的白名单。将后端的公用证书上传到应用程序网关即可完成此操作。这可确保应用程序网关仅与已知的后端实例通信，从而保护端到端通信。以下步骤介绍此过程：
...
以下示例创建一个虚拟网络和两个子网。一个子网用于托管应用程序网关。另一个子网用于托管 Web 应用程序的后端服务器。
...
> [AZURE.IMPORTANT] 应用程序网关不支持使用通过定义的域标签创建的公共 IP 地址。仅支持具有动态创建的域标签的公共 IP 地址。如果需要应用程序网关具有友好的 DNS 名称，建议使用 CNAME 记录作为别名。
...
> [AZURE.NOTE] 完全限定的域名 (FQDN) 也是可以替换后端服务器 IP 地址的有效值。
...
> [AZURE.NOTE] 此示例配置用于 SSL 连接的证书。该证书需采用 .pfx 格式，并且密码必须为 4 到 12 个字符。
...
> [AZURE.NOTE] 此步骤中提供的证书应该是后端中存在的 pfx 证书的公钥。此步骤会将后端加入应用程序网关的白名单。
