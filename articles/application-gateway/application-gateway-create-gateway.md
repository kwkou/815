...
Azure 应用程序网关是第 7 层负载平衡器。它可在云端或本地的服务器间提供故障转移、性能路由 HTTP 请求。应用程序网关具有以下应用程序传递功能：HTTP 负载平衡、基于 Cookie 的会话相关性和安全套接字层 (SSL) 卸载。
...
使用 **New-AzureApplicationGateway** 命令创建应用程序网关时，无需设置配置；新建资源时，使用XML或配置对象进行配置。
...
- **前端端口：**此端口是应用程序网关上打开的公共端口。流量到达端口后，重返后端服务器。
- **侦听器：**侦听器具有前端端口、协议（Http 或 Https，区分大小写）和 SSL 证书名称（如果要配置 SSL 卸载）。
- **规则：**规则将会绑定侦听器和后端服务器池，规则对侦听器和后端服务器池做出约束，定义了流量抵达特定侦听器时应指向的后端服务器池。
...
若要创建网关，请使用 **New-AzureApplicationGateway** cmdlet，并将值替换为你自己的值。请注意网关的收费从其成功启动开始，本阶段不收取费用。
...
配置网关后，使用 **Start-AzureApplicationGateway** cmdlet 来启动网关。启动后开始收费。
...
3. 使用 **Get-AzureApplicationGateway** cmdlet验证网关是否已删除。
