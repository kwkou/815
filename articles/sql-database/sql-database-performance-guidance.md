...
Azure SQL 数据库提供三个[服务层](/documentation/articles/sql-database-service-tiers/)：基本、标准和高级。每个服务层都会对供 SQL 数据库使用的资源进行严格的隔离，确保该服务级别的性能可以预测。本文将指导用户为其应用程序选择服务层，并讨论如何通过多种方式调整应用程序，以便充分利用 Azure SQL 数据库。
...
> [AZURE.NOTE]
目前只能对单一数据库使用 Azure 内存中联机事务处理 (OLTP) 预览版。不能将其用于弹性池中的数据库。
> 
> 
...
<!--update: wording update; add introduction for Ibiza portal tools-->