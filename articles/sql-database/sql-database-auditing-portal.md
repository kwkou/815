...
7. 配置审核设置后，可以打开新的“威胁检测(预览版)”功能，并配置电子邮件，以接收安全警报。 使用威胁检测可以接收针对异常数据库活动（可能表示潜在的安全威胁）发出的前瞻性警报。 有关更多详细信息，请参阅[威胁检测](/documentation/articles/sql-database-threat-detection/)。
8. 单击“保存” 。
...
2. **辅助数据库** ：可以通过主数据库审核设置打开/关闭 Blob 审核。
   ...
   * 在主数据库上打开 Blob 审核。 必须在主数据库本身上启用 blob 审核，而不要在服务器上启用。
...
## <a name="viewing-blob-auditing-logs"></a>查看 blob 审核日志
...
* 其他方法：
   ...
   * 可以通过 Azure 存储资源管理器同时下载多个文件 - 右键单击特定子文件夹（包含特定日期生成的所有日志文件的子文件夹），然后选择“另存为”，将文件保存到本地文件夹中。
...
## <a name="viewing-table-audit-logs"></a>查看表审核日志
...
4. 返回到存储配置边栏选项卡，然后**重新生成**辅助访问密钥（为下一个密钥刷新周期做好准备）。
