...
- 你将需要一个 [Microsoft Azure](http://www.windowsazure.cn) 帐户。如果没有，请使用 [1rmb 试用版](/pricing/1rmb-trial)。此外，你可以阅读 [Azure 站点恢复管理器定价](/home/features/site-recovery/#price)。
...
1. 在"快速启动"页面上<b></b>，在"准备 VMM 服务器"中，单击"下载用于在 VMM 服务器上安装的 Microsoft Azure 站点恢复 提供程序"<b></b>来获取最新版本的提供程序安装文件。
...
12. 在"复制方法"中<b></b>，指定在开始定期复制之前将如何处理从源到目标位置的初始数据复制。 
	...
	- <b>脱机</b> - 此方法指定将使用外部介质执行初始复制。如果你要避免网络性能下降或在地理上处于远程位置，则这种方法很有用。要使用这种方法，请在源云中指定导出位置，并在目标云中指定导入位置。当你为虚拟机启用保护时，虚拟硬盘将复制到指定的导出位置。你将其发送到目标站点，并将其复制到导入位置。系统将导入的信息复制到副本虚拟机。有关脱机复制先决条件的完整列表，请参阅部署指南中的<a href="/documentation/articles/site-recovery-vmm-to-vmm/#step-3-install-the-azure-site-recovery-provider">步骤 3：为 VMM 云配置保护设置</a>。
...
##<a id="step-5-configure-network-mapping"></a> 步骤 5：配置网络映射
1. 在“快速启动”页上，单击“映射网络”。
...
本部分提供了 Microsoft Azure 站点恢复 服务（"服务"）的更多隐私信息。若要查看 Microsoft Azure 服务的隐私声明，请参阅 
