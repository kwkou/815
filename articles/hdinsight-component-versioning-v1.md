...
* 某版本的支持到期后，其就不能通过 Azure 管理门户提供了。下表列出 Azure 管理门户上提供的版本。可继续使用 Windows PowerShell [New-AzureHDInsightCluster](https://msdn.microsoft.com/zh-cn/library/dn947292.aspx) 命令中的 `Version` 参数和 .NET SDK 获取群集版本，直到其弃用日期为止。
