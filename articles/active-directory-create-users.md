...
3. 在"告诉我们有关此用户的信息"页上，对于"用户类型"，请选择下列其中一项： 
	...
	3. **另一个 Azure AD 目录中的用户** - 指示你要将另一个 Azure AD 目录中的用户帐户添加到你的目录。你需要成为另一个目录的成员才能选择该目录中的用户。 
4. 基于你的选择，键入用户名或此用户通过其进行登录的 Microsoft 帐户名。
5. 在该用户的"配置文件"页上，提供用户的名字和姓氏、用户友好名称，并从"角色"下拉菜单中选择用户角色。有关用户和管理员角色的详细信息，请参阅[在 Azure AD 中分配管理员角色](active-directory-assign-admin-roles)。指定是否要**启用多重身份验证**。
6. 在"获取临时密码"页上，单击"创建"。
