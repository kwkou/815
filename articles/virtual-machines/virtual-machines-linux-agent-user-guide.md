...
Azure Linux 代理 (/usr/sbin/waagent) 管理虚拟机与 Azure 结构控制器的交互。有关在正在运行的 VM 上安装和更新代理的信息，另请参阅[如何更新 Azure Linux 代理](/documentation/articles/virtual-machines-linux-update-agent/)。

Azure Linux 代理负责以下事项：

> [AZURE.NOTE] 有关本指南的最新版本，请参阅 Azure Linux 代理[自述文件](https://github.com/Azure/WALinuxAgent/blob/master/README.md)。
...
## 要求
下列系统已经过测试并确认兼容 Azure Linux 代理。**请注意，此列表可能不同于 Azure 平台所支持系统的官方列表**，详见以下网页：[http://www.azure.cn/documentation/articles/virtual-machines-linux-endorsed-distros/](/documentation/articles/virtual-machines-linux-endorsed-distros/)
...
如果指定了指向可执行计划的路径，则在结构指示配置文件对虚拟机可用时，将调用此计划。指向 XML 配置文件的路径作为可执行计划的参数提供。这可在配置文件发生更改时多次调用。此文件的当前路径为 /var/lib/waagent/HostingEnvironmentConfig.xml。

**Role.TopologyConsumer：**

类型：字符串；默认值：无

如果指定了指向可执行计划的路径，则在结构指示新的网络拓扑布局对虚拟机可用时，将调用此计划。指向 XML 配置文件的路径将作为可执行计划的参数提供。这可在网络拓扑发生更改时（例如，由于服务修复）多次调用。此文件的当前位置为 /var/lib/waagent/SharedConfig.xml。
...
类型：字符串；默认值：/mnt/resource

这将指定资源磁盘的安装路径。请注意，资源磁盘是临时磁盘，可能在取消预配 VM 时被清空。
