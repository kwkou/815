...
本文说明如何使用 VMAcesss VM 扩展 [(Github)](https://github.com/Azure/azure-linux-extensions/tree/master/VMAccess) 来检查或修复磁盘、重置用户访问权限、管理用户帐户，或重置 Linux 上的 SSHD 配置。本文需要 [Azure 帐户](/pricing/1rmb-trial/)、[SSH 密钥](/documentation/articles/virtual-machines-linux-mac-create-ssh-keys/)、Azure Linux 虚拟机、已安装 Azure CLI 并使用 `azure config mode arm` 将其切换到 ARM 模式。
...
- 使用 VMAccess 要处理和操作的原始 JSON 文件。

在快速命令部分，我们将使用 Azure CLI `azure vm reset-access` 方法。在以下命令示例中，请将 &lt; 与 &gt; 之间的值替换为你自己环境中的值。
...
Linux VM 上的磁盘显示错误。不知道怎样重置 Linux VM 的 root 密码，或者不小心删除了 SSH 私钥。此问题如果发生在数据中心上市之前，必须驱车转到当地、提供掌纹来解锁大门、进入机房，然后打开 KVM 以进入服务器控制台。请将 Azure VMAccess 扩展想像成该 KVM 交换机，它允许你访问控制台以重置 Linux 访问或执行磁盘级维护。

在详细演练中，我们将使用使用原始 JSON 文件的长格式 VMAccess。从 Azure 模板也可以调用这些 VMAccess JSON 文件。
...
如果你已失去 Linux VM 的 root 访问权限，可以启动 VMAccess 脚本重置 root 密码。
