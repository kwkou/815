...
1. 在“验证”页中，单击“下一步”。此页应与以下页类似。因为你尚未配置可用性组侦听器，会出现一个侦听器配置警告。你可以忽略此警告，因为本教程不会配置侦听器。本教程稍后将帮助你创建侦听器。有关如何配置侦听器的详细信息，请参阅[在 Azure 中为 AlwaysOn 可用性组配置内部负载均衡器](/documentation/articles/virtual-machines-windows-portal-sql-alwayson-int-listener/)。
...
若要直接连接到可用性组，需要在 Azure 中配置内部负载均衡器，然后在群集中创建侦听器。本部分提供这些步骤的综合概述。有关详细说明，请参阅[在 Azure 中为 AlwaysOn 可用性组配置内部负载均衡器](/documentation/articles/virtual-machines-windows-portal-sql-alwayson-int-listener/)。
...
| **负载均衡规则协议** | TCP
| **负载均衡规则端口** | 1433 - 请注意，这是 SQL Server 的默认端口。
| **负载均衡规则端口** | 1433 - 请注意，这是 SQL Server 的默认端口。
| **负载均衡规则后端端口** | 1433
...
1. 单击“IP 地址”下面的“静态 IP 地址”，并将静态 IP 地址设置为在 Azure 门户预览上用于 **sqlLB** 负载均衡器的相同地址。请注意，你还要在 PowerShell 脚本的 `$ILBIP` 变量中使用此同一个 IP 地址。为此地址启用 NetBIOS，然后单击“确定”。
