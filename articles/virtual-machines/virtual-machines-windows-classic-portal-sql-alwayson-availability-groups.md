...
| 虚拟机配置 |**版本发布日期** =（最新）<br/>**虚拟机名称** = ContosoWSFCNode<br/>**层** = 标准<br/>**大小** = A2（双核）<br/>**新用户名** = AzureAdmin<br/>**新密码** = Contoso!000<br/>**确认密码** = Contoso!000 |**版本发布日期** =（最新）<br/>**虚拟机名称** = ContosoSQL1<br/>**层** = 标准<br/>**大小** = A3（四核）<br/>**新用户名** = AzureAdmin<br/>**新密码** = Contoso!000<br/>**确认密码** = Contoso!000 |**版本发布日期** =（最新）<br/>**虚拟机名称** = ContosoSQL2<br/>**层** = 标准<br/>**大小** = A3（四核）<br/>**新用户名** = AzureAdmin<br/>**新密码** = Contoso!000<br/>**确认密码** = Contoso!000 |
| 虚拟机配置 |**云服务** = 之前创建的唯一云服务 DNS 名称（例如：ContosoDC123）<br/>**地域/地缘组/虚拟网络** = ContosoNET<br/>**虚拟网络子网** = Back\(10.10.2.0/24\)<br/>**存储帐户** = 使用自动生成的存储帐户<br/>**可用性集** = 创建可用性集<br/>**可用性集名称** = SQLHADR |**云服务** = 之前创建的唯一云服务 DNS 名称（例如：ContosoDC123）<br/>**地域/地缘组/虚拟网络** = ContosoNET<br/>**虚拟网络子网** = Back\(10.10.2.0/24\)<br/>**存储帐户** = 使用自动生成的存储帐户<br/>**可用性集** = SQLHADR（也可在创建计算机后配置可用性集。所有三个计算机都应分配到 SQLHADR 可用性集。） |**云服务** = 之前创建的唯一云服务 DNS 名称（例如：ContosoDC123）<br/>**地域/地缘组/虚拟网络** = ContosoNET<br/>**虚拟网络子网** = Back\(10.10.2.0/24\)<br/>**存储帐户** = 使用自动生成的存储帐户<br/>**可用性集** = SQLHADR（也可在创建计算机后配置可用性集。所有三个计算机都应分配到 SQLHADR 可用性集。） |
...
> [AZURE.NOTE]
上述配置建议使用“标准”层虚拟机，因为“基本”层计算机不支持稍后创建可用性组侦听器时所需的负载均衡终结点。此外，为了在 Azure VM 中测试可用性组，建议采用此处的计算机大小。为获得生产工作负荷的最佳性能，请参阅 [Azure 虚拟机中 SQL Server 的性能最佳实践](/documentation/articles/virtual-machines-windows-sql-performance/)中关于 SQL Server 计算机大小和配置的建议。
> 
> 
...
2. 启动下载的 RDP 文件，并使用已配置的管理员帐户 \(**BUILTIN\\AzureAdmin**\) 和密码 \(**Contoso!000**\) 登录到 VM。
...
9. 地址 **10.10.2.4** 是分配到 Azure 虚拟网络中 10.10.2.0/24 子网中的 VM 的地址，而该 VM 是 **ContosoDC**。若要验证 **ContosoDC** 的 IP 地址，请在命令提示符中使用 **nslookup contosodc**，如下所示。
...
5. 此时会弹出“连接到服务器”对话框。在“服务器名称”中键入 **ContosoSQL2**，然后单击“连接”。
    ...
    ![新建 AG 向导，连接到服务器](./media/virtual-machines-windows-classic-portal-sql-alwayson-availability-groups/IC665527.gif)  
