...
> [AZURE.NOTE] 这是支持运行 SAP 生产系统的部署方案。
...
* 使用管理员特权添加另一个本地帐户，该帐户可供 Azure.cn 支持人员使用，或者在部署 VM 之前分配为上下文以供服务和应用程序短暂访问，并可供其他适当的用户使用。
...
* 使用管理员特权添加另一个本地帐户，该帐户可供 Azure.cn 支持人员在调查问题时使用，或者在部署 VM 之前分配为上下文以供服务和应用程序短暂访问，并可供其他适当的用户使用。
...
> ![Linux][Logo_Linux] Linux
>
> [如何捕获 Linux 虚拟机以用作资源管理器模板][virtual-machines-linux-capture-image]

___

### 在本地与 Azure 之间传输 VM 和 VHD
由于无法通过 Azure 门户预览上载 VM 映像和磁盘，因此需要使用 Azure PowerShell cmdlet 或 CLI。另一种可行的方法是使用“AzCopy”工具。该工具可在本地与 Azure 之间（双向）复制 VHD。它还可以在 Azure 区域之间复制 VHD。有关如何下载和使用 AzCopy，请参阅[此文档][storage-use-azcopy]。

第三种替代做法是使用面向 GUI 的各种第三方工具。但是，请确保这些工具支持 Azure 页 Blob。在本文档中，我们需要使用页 Blob 存储（以下网页中介绍了差异：<https://msdn.microsoft.com/zh-cn/library/azure/ee691964.aspx>）。另外，Azure 提供的工具能够非常高效地压缩需要上载的 VM 和 VHD。这种优势非常重要，因为这种压缩效率可以减少上载时间（具体的上载时间根据从本地设施和目标 Azure 部署区域与 Internet 网络之间的上载链路的情况而有一定的差异）。我们可以合理地推测出这样一个结论：将位于欧洲的某个 VM 或 VHD 上载到位于美国的 Azure 数据中心所需的时间，比将同一个 VM/VHD 上载到位于欧洲的 Azure 数据中心所需的时间要长。
...
* 使用 _New-AzureRmVM_ 从 VM 配置创建新的 VM - 请参阅 <https://msdn.microsoft.com/zh-cn/library/mt603754.aspx>
* 使用 _Add-AzureRmVMDataDisk_ 将数据磁盘添加新的 VM - 请参阅 <https://msdn.microsoft.com/zh-cn/library/mt603673.aspx>
...
* 使用 _New-AzureStorageContext_ 创建源存储帐户的存储帐户上下文 - 请参阅 <https://msdn.microsoft.com/zh-cn/library/dn806380.aspx>
* 使用 _New-AzureStorageContext_ 创建目标存储帐户的存储帐户上下文 - 请参阅 <https://msdn.microsoft.com/zh-cn/library/dn806380.aspx>
* 开始复制
...
#### 磁盘处理
在大多数方案中，你需要创建更多的磁盘，以便将 SAP 数据库部署到 VM 中。我们已在本文档的 [SAP 部署的 VM/VHD 结构][planning-guide-5.5.1]一章中讨论了有关 VHD 数量的注意事项。部署基础 VM 后，可以在 Azure 门户预览中附加和分离磁盘。当该 VM 已启动并运行或者处于停止状态时，便可以附加/分离磁盘。在附加磁盘时，Azure 门户预览可让你附加空磁盘，或者当前尚未附加到其他 VM 的现有磁盘。
...
**重要说明**：对于 Azure 标准存储，我们**不**想要使用主机缓存。因此，应该将“主机缓存首选项”保留为默认值“无”。使用 Azure 高级存储时，如果 I/O 特征大多数读取为类似对数据库数据文件的一般 I/O 流量，则应该启用“读取缓存”。在数据库事务日志文件中，不建议使用缓存。
...
#### 仅限云方案的 SAP 系统配置和 SAP GUI 连接
请参阅以下文章，其中对此主题提供了详细说明： 
...
参考文档：<https://help.sap.com/saphelp_nwpi71/helpdata/en/47/c56a6938fb2d65e10000000a42189c/content.htm>
...
可以在 Linux 上使用以下示例代码。对于 Windows，请使用上述 PowerShell，或改编示例以使用 %rgName% 而不是 $rgName，并使用 Windows 命令 _set_ 来设置环境变量。
...
| 消息服务器 | sapms`<sid`> 请参阅 ** | 3600 | 可用 sapms`<anySID`> | sid = SAP-System-ID |
...
可以在 <http://scn.sap.com/docs/DOC-17124> 中找到有关不同 SAP 产品或 SAP 产品的不同服务所需的端口的更多详细信息。参考此文档应该可以在 VPN 设备中打开特定 SAP 产品和方案所需的专用端口。

在这种方案中部署 VM 时采取的其他安全措施可能包括创建一个[网络安全组][virtual-networks-nsg]来定义访问规则。

### 处理不同的虚拟机系列

在过去 12 个月内，Microsoft 已添加更多 VM 类型，这些类型在 vCPU 数量、内存，更重要的是其运行所在的硬件方面各不相同。SAP 不一定支持所有 VM（请在 SAP 说明 [1928533] 中查看支持的 VM 类型）。其中一些 VM 在不同代次的主机硬件上运行。这些代次的主机硬件以 Azure 缩放单位的数据粒度进行部署。有时，选择的不同 VM 大小可能无法在相同的缩放单位上运行。根据不同的硬件，可用性集跨缩放单位的能力会受到限制。例如，如果你想要在 A5-A11 VM 上运行 DBMS，并在 G 系列 VM 上运行 SAP 应用程序层，则必须部署单个 SAP 系统，或者在不同的可用性集内部署不同的 SAP 系统。


#### 使用局域网打印机从 Azure 中的 SAP 实例打印
##### 通过跨界方案中的 TCP/IP 打印
...
#### 在跨界方案中将 SAP Azure 系统集成到纠正与传输系统 (TMS)
...
若要解决这种延迟，使系统能够快速地从传输目录中读取或者向其写入数据，你可以设置两个 STMS 传输域（一个用于本地，一个用于 Azure 中的系统），并将其链接起来。请查看以下文档，其中说明了 SAP TMS 的这个概念所依据的原理：
...
#### 从位于 Azure 中的 SAP 实例访问“本地”文件共享或进行相反的访问
...
### 将 Azure 中的 SAP 实例集成到 SAProuter
...
初始门户 URI 为 http(s):`<Portalserver`>:5XX00/irj，其中的端口值计算公式为 50000 + (系统编号 x 100)。SAP 系统 00 的默认门户 URI 为 `<dns name`>.`<azure region`>.Cloudapp.azure.com:PublicPort/irj。有关详细信息，请查看 <http://help.sap.com/saphelp_nw70ehp1/helpdata/de/a2/f9d7fed2adc340ab462ae159d19509/frameset.htm>。
...
可在以下文档中找到更多详细信息：<https://www.azure.cn/documentation/articles/virtual-machines-manage-availability>

#### Azure 存储冗余

始终复制 Azure 存储帐户中的数据以确保持久性和高可用性，并且即使在遇到临时硬件故障时也符合 Azure 存储空间 SLA 要求

由于 Azure 存储空间默认保留 3 个数据映像，因此不需要部署跨多个 Azure 磁盘的 RAID5 或 RAID1。

可在以下文章中找到更多详细信息：<https://www.azure.cn/documentation/articles/storage-redundancy/>
...
> [AZURE.NOTE] 请注意，Azure VM 重新启动主要是保护 VM 而不是应用程序。VM 重新启动并未提供 SAP 应用程序的高可用性，而是提供特定基础结构级别的可用性，因而间接实现 SAP 系统的“更高可用性”。此外，在计划内或计划外的主机中断之后重新启动 VM 所需的时间也没有 SLA。因此，此“高可用性”方法不适用于 SAP 系统的关键组件，例如 (A)SCS 或 DBMS。
...
每个 SAP 实例都在自身的 Azure 存储帐户中 - 当一个 Azure 存储空间可能不可用时，只导致一个 VM 及其 SAP AS 实例不可用。不过请注意，一个 Azure 订阅中的 Azure 存储帐户数目有限。为了确保在 VM 重新启动后自动启动 (A)SCS 实例，请务必设置[对 SAP 实例使用 Autostart][planning-guide-11.5] 一章所述的 (A)SCS 实例启动配置文件中的 Autostart 参数。
...
当部署许多 SAP 系统，并且所部署的 VM 数目将超过每个订阅的存储帐户上限时，可能需要稍微折衷以下所述的概念。在此情况下，VM 的 VHD 需要合并到一个存储帐户中。通常你会通过合并不同 SAP 系统的 SAP 应用程序层 VM 的 VHD 来实现此目的。也可以将不同 SAP 系统的不同 DBMS VM 的不同 VHD 合并到一个 Azure 存储帐户中。因此，请记住 Azure 存储帐户的 IOPS 限制 ( <https://www.azure.cn/documentation/articles/storage-scalability-targets> )
...
不支持在本地与 Azure 之间分区应用程序层本身，或拆分应用程序和 DBMS 层。SAP 系统可以完全在本地或在 Azure 中部署。此外，不支持在本地运行一些应用程序服务器，并在 Azure 中运行其他一些应用程序服务器。这是讨论的起点。我们也不支持将 SAP 系统的 DBMS 组件和 SAP 应用程序服务器层部署在两个不同的 Azure 区域。例如DBMS 位于中国北部，SAP 应用程序层位于中国北部。不支持此类配置是因为 SAP NetWeaver 体系结构的延迟敏感性。
...
简单而言，执行脱机备份要求通过 Azure 门户预览关闭 VM，然后将基础 VM 磁盘以及所有附加的 VHD 复制到 VM。这样就能保存 VM 及其关联磁盘的时间点映像。建议将“备份”复制到一个不同的 Azure 存储帐户。因此，可以应用本文档的[在 Azure 存储帐户之间复制磁盘][planning-guide-5.4.2]一章中所述的过程。除了使用 Azure 门户预览关机之外，也可以按如下所述通过 Powershell 或 CLI 来执行此操作：<https://www.azure.cn/documentation/articles/virtual-machines-deploy-rmtemplates-powershell/>

还原该状态将要删除基础 VM 以及基础 VM 的原始磁盘和装载的 VHD，将保存的 VHD 复制回到原始存储帐户，然后重新部署系统。以下文章说明了如何使用 Powershell 编写此过程的脚本：<http://www.westerndevs.com/azure-snapshots/>
...
> [AZURE.NOTE] 
> 截至 2015 年 12 月，使用 VM 备份并不保留用于 SAP 许可的唯一 VM ID。这意味着，从 VM 备份还原需要安装新的 SAP 许可证密钥，因为还原的 VM 被视为新的 VM，而不是替代以前保存的旧版。截至 2016 年 1 月，Azure VM 备份尚不支持使用 Azure Resourc Manager 部署的 VM。

> ![Windows][Logo_Windows] Windows
>
> 理论上，如果 DBMS 系统支持 Windows VSS（卷影复制服务 <https://msdn.microsoft.com/zh-cn/library/windows/desktop/bb968832(v=vs.85).aspx>），则与 SQL Server 一样，将以一致的方式备份运行数据库的 VM。不过请注意，无法根据 Azure VM 备份时间点来还原数据库。因此，建议使用 DBMS 功能执行数据库的备份，而不要依赖于 Azure VM 备份
>
