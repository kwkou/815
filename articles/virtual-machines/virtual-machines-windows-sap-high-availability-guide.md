...
4.  创建群集后，运行群集验证测试。
    ...
    在过程中的此时点，可忽略关于磁盘的任何警告。 稍后将添加文件共享见证与 SIOS 共享磁盘。 在此阶段不必担心仲裁问题。
...
8.  输入第二个群集节点主机的名称。
    ...
    可以忽略关于仲裁和磁盘的警告。 稍后将会根据 [为 SAP ASCS/SCS 群集共享磁盘安装 SIOS DataKeeper Cluster Edition][sap-ha-guide-8.12.3]中所述设置仲裁和共享磁盘。
