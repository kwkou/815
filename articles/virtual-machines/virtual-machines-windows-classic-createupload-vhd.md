...
> [AZURE.IMPORTANT]Azure 具有用于创建和处理资源的两个不同的部署模型：[资源管理器和经典](/documentation/articles/resource-manager-deployment-model/)。本文介绍使用经典部署模型。Azure 建议大多数新部署使用资源管理器模型。你也可以使用资源管理器模型，[捕捉](/documentation/articles/virtual-machines-windows-capture-image/)和[上传](/documentation/articles/virtual-machines-windows-upload-image/)虚拟机。
