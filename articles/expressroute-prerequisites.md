<properties
   pageTitle="采用 ExpressRoute 所要满足的先决条件 | Windows Azure"
...
若要使用 ExpressRoute 连接到 Windows Azure 服务，你需要确认是否符合以下部分中所列的要求。

## 帐户要求

- 使用中的有效 Windows Azure 帐户。需有此帐户才能设置 ExpressRoute 线路。
...
客户基础结构与服务提供商基础结构之间的物理连接没有任何冗余要求。Windows Azure 不要求在第 3 层配置冗余。但 Windows Azure 确实要求在 Windows Azure 边缘与客户网络之间，通过服务提供商针对要启用的每个对等互连设置冗余路由。如果未以冗余方式配置路由会话，服务的可用性 SLA 将会失效。
