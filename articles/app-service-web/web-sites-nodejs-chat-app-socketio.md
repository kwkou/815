...
1. Socket.IO 使用的 WebSocket 在 Azure 上默认不启用。若要启用 Web 套接字，请使用以下命令：
	...
	>[AZURE.NOTE]
	“azure site set -w”命令仅适用于 Azure 命令行界面 0.7.4 或更高版本。也可以使用 [Azure 门户预览](https://portal.azure.cn)启用 WebSocket 支持。<p>若要使用 Azure 门户预览启用 WebSocket，请在“Web 应用”边栏选项卡中单击该 Web 应用，然后单击“所有设置”>“应用程序设置”。在“Web 套接字”下，单击“打开”。然后单击“保存”。
...
> [AZURE.NOTE] 向外扩展 Socket.IO 解决方案还要求支持粘滞会话。默认情况下，可以通过 Azure 请求路由为 Azure Web Apps 启用粘滞会话。有关详细信息，请参阅 [Instance Affinity in Azure Web Sites]（Azure 网站中的实例关联）。
