...
应用服务 Web 应用为 Web 服务器和 Web 应用程序中的日志记录信息提供了诊断功能。这些诊断功能按逻辑分为 **Web 服务器诊断**和**应用程序诊断**。
...
启用“应用程序诊断”时，还需选择“级别”。该设置允许你将捕获到的信息筛选为“信息性”信息、“警告”信息或“错误”信息。若将此选项设置为“详细”，则将记录应用程序生成的所有信息。

> [AZURE.NOTE] 不同于更改 Web.config 文件，启用应用程序诊断或更改诊断日志级别不会回收应用程序运行于其中的应用域。

在[经典管理门户](https://manage.windowsazure.cn) Web 应用的“配置”选项卡中，可以选择“存储”或“文件系统”进行**Web 服务器日志记录**。选择“存储”允许你选择存储帐户，然后日志会写入一个 Blob 容器。“站点诊断”的所有其他日志仅写入文件系统。
...
><p>
><p> 1. 在“配置”选项卡上，将相应的日志记录功能设置为“关闭”。保存你的设置。
><p> 2. 允许再次将日志记录到存储帐户 blob 或表。保存你的设置。
...
> [AZURE.NOTE] 如果尚未安装 Azure 命令行接口，或者尚未将其配置为使用 Azure 订阅，请参阅 [How to Use Azure Command-Line Interface](/documentation/articles/xplat-cli-install/)（如何使用 Azure 命令行接口）。
