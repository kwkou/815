...
**扩展您的应用程序** | [在 Azure App Service 中缩放 Web 应用][websitescale]和 [MySQL 集群 CGE][cge]
...
**提高性能** | <ul><li><p>[禁用 ARR cookie](http://ppe.blogs.msdn.com/b/windowsazure/archive/2013/11/18/disabling-arr-s-instance-affinity-in-windows-azure-web-sites.aspx) - 在多个 Web 应用实例上运行 WordPress 时可以提高性能</p></li><li><p>启用缓存。[Redis 缓存](/documentation/services/redis-cache/)可以与 [Redis 对象缓存 WordPress 插件](https://wordpress.org/plugins/redis-object-cache/)一起使用</p></li><li><p>[如何使用 Wincache 提高 WordPress 速度](http://ruslany.net/2010/03/make-wordpress-faster-on-iis-with-wincache-1-1/) - 对于 Web 应用，Wincache 默认处于启用状态</p></li><li><p>[在 Azure App Service 中扩展 Web 应用](/documentation/articles/web-sites-scale/)并用 [ClearDB 高可用性路由](http://www.cleardb.com/developers/cdbr/introduction)或 [MySQL 群集 CGE](http://www.mysql.com/products/cluster/)</p></li></ul>
...
**启用电子邮件** | <ol><li><p>[为 WordPress 安装 SendGrid 插件](http://wordpress.org/plugins/sendgrid-email-delivery-simplified/)</p></li></ol>
**配置自定义域名** | [在 Azure App Service 中配置自定义域名][customdomain]
**启用自定义域名的 HTTPS** | [在 Azure App Service 中启用 Web 应用的 HTTPS][httpscustomdomain]
**负载均衡或地理分配站点** | [通过 Azure 流量管理器路由流量][trafficmanager]。如果您使用自定义域，请参阅[在 Azure App Service 中使用自定义域名][customdomain]，了解有关使用含自定义域名的流量管理器的信息
