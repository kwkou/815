...
本教程介绍如何生成并本地运行应用程序、如何将其部署到 Azure 并在云中运行，以及最终如何从头构建。你可以从头构建然后进行测试，之后根据你的喜好部署步骤。
...
- [故障排除](#troubleshooting) （如果运行示例时遇到问题，请转到此处。）
- [后续步骤](#next-steps)
...
1. 按 Ctrl+F5 运行应用程序。
	...
	在本地运行云服务项目时，Visual Studio 会自动调用 Azure 计算仿真程序和 Azure 存储仿真程序。计算仿真程序使用你的计算机资源，以模拟 Web 角色和辅助角色环境。存储仿真程序使用 [SQL Server Express LocalDB](http://msdn.microsoft.com/zh-cn/library/hh510202.aspx) 数据库模拟 Azure 云存储。 

	首次运行云服务项目时，仿真程序将会花费大约一分钟来启动。仿真程序完成启动后，默认浏览器中会打开应用程序的主页。
...
你已在本地计算机上完全运行应用程序，但未连接到云。存储仿真程序将队列和 Blob 数据存储在 SQL Server Express LocalDB 数据库中，应用程序将广告数据存储在另一个 LocalDB 数据库中。在 Web 应用程序首次尝试访问它时，实体框架 Code First 自动创建广告数据库。
...
1. 在浏览器中，打开 [Azure 管理门户](http://manage.windowsazure.cn)。

2. 单击"新建"-"计算"-"云服务"-"快速创建"。

4. 在 URL 输入框中，输入 URL 前缀。 

	此 URL 必须是唯一的。如果你选择的前缀已被其他人使用，将获得一条错误消息。

5. 选择你要在其中部署该应用程序的区域。

	此字段指定你的云服务将托管在哪个数据中心。对于生产应用程序，你可以选择离客户最近的区域。对于本教程，选择离你最近的区域。
...
1. 选择的区域与你为云服务所选择的相同。 
	...
	当云服务和数据库位于不同的数据中心（不同区域）时，延迟将增加，并且你将支付带宽数据中心之外的费用。数据中心内的带宽是免费的。
...
4. 在"URL"框中，输入 URL 前缀。 
	...
	此前缀加上在框下看到的文本将是你的存储帐户的唯一 URL。如果其他人已使用你输入的前缀，你必须选择不同的前缀。
...
>[WACOM.NOTE] 在本部分和下一部分中，你将在项目文件中存储凭据。[不要将敏感数据存储在公共源代码存储库中](http://www.asp.net/aspnet/overview/developing-apps-with-windows-azure/building-real-world-cloud-apps-with-windows-azure/source-control#secrets)。
...
2. 在 Azure 管理门户中，依次单击左窗格中的"SQL 数据库"、你在本教程中创建的数据库、"仪表板"选项卡、"显示连接字符串"。
...
4. 在"Web.Release.config"转换文件中，删除"{connectionstring}"并将管理门户中的 ADO.NET 连接字符串粘贴在其原位置。

5. 在粘贴到"Web.Release.config"转换文件的连接字符串中，请用你为新的 SQL 数据库创建的密码更换"{your_password_here}"。
...
7. 选择"ContosoAdsDbConnectionString"设置中的文本，然后粘贴你从本教程上一节复制的连接字符串。
...
Web 角色项目和辅助角色项目的 Azure 存储帐户连接字符串存储在云服务项目中的环境设置。对于每个项目，应用程序将在本地运行以及在云中运行时，将使用一组单独的设置。你将要更新用于 Web 和辅助角色项目的云环境设置。 
...
1. 在"创建存储连接字符串"对话框中，单击"使用你的订阅进行连接"，选择你以前创建的存储帐户，然后单击"确定"。如果你尚未登录，系统将提示你输入 Azure 帐户凭据。
...
*ServiceConfiguration.Cloud.cscfg* 文件包括为 Visual Studio 中的这些设置输入的值：
...
"<Instances>"设置指定 Azure 将在其上运行辅助角色代码的虚拟机的数量。[后续步骤](#next-steps) 部分包括有关横向扩展云服务的详细信息的链接，
...
3. 在向导的"设置"步骤中，单击"下一步"。
	...
	本教程可以使用"高级"选项卡中的默认设置。有关高级选项卡的信息，请参阅[发布 Azure 应用程序向导](http://msdn.microsoft.com/zh-cn/library/windowsazure/hh535756.aspx)。
...
1. 当部署状态为完成时，单击"网站 URL"以启动应用程序。

9. 就像在本地运行应用程序一样，现在可以通过创建、查看和编辑一些广告测试应用程序。

>[WACOM.NOTE] 完成测试后，删除或停止云服务。即使你不使用云服务，也会产生费用，因为要为它保留虚拟机资源。如果保持运行云服务，找到你 URL 的任何人都可以创建和查看广告。在 Azure 管理门户中，转到云服务的"仪表板"选项卡，然后单击页面底部的"删除"按钮。如果你只是想要暂时防止其他人访问站点，请单击"停止"。在这种情况下，会继续产生费用。不再需要时，可采用类似步骤删除 SQL 数据库和存储帐户。

## 从头开始创建应用程序 

如果你尚未下载 
[已完成的应用程序](http://code.msdn.microsoft.com/Simple-Azure-Cloud-Service-e01df2e4)，现在请下载。并非创建新文件和复制并粘贴代码到它们中，而是将文件从下载的项目复制到新的项目。
...
11. 在"添加新项目"对话框中，选择左窗格中"Visual C#"下面的"Windows 桌面"，然后单击"类库"模板。  

10. 将项目命名为"ContosoAdsCommon"，然后单击"确定"。

	你需要从 Web 和辅助角色项目引用实体框架上下文和数据模型。作为一种替代方法，你可以在 Web 角色项目中定义与 EF 相关的类，并从辅助角色项目中引用该项目。但是，之后你的辅助角色项目将引用它不需要的 Web 程序集。
...
13. 查找"Azure 存储空间"包，如果它在列表中，单击"更新"以获取最新版本的存储客户端库。

	![Update SCL](./media/cloud-services-dotnet-get-started/updstg.png)	

	存储客户端库更新频率高于 Visual Studio 项目模板，因此你通常会发现该版本中新创建的计划需求需要更新。
...
在本部分中，你将为本地测试配置 Azure 存储和 SQL 连接字符串。本教程前面的部署说明解释如何当应用程序在云中运行时设置连接字符串。
...
本部分中，你将代码将文件从已下载的解决方案复制到新的解决方案。以下各节将显示并解释此代码的关键部分。

要将文件添加到一个项目或一个文件夹，右键单击项目或文件夹，然后单击"添加"-"现有项"。选择所需的文件，然后单击"添加"。如果询问你是否想要替换现有文件，请单击"是"。
...
以下部分介绍与使用 Azure 环境、Blob 和队列相关的代码。本教程未说明如何使用基架创建 MVC 控制器和视图，如何编写适用于 SQL Server 数据库的实体框架代码，或者在 ASP.NET 4.5 中的异步编程基础知识。有关这些主题的信息，请参见以下资源：
...
从"Application_Start"方法调用的代码创建"映像"Blob 容器和"映像"队列（如果它们尚不存在）。这确保只要你开始使用新的存储帐户，或在新计算机上开始使用存储仿真程序时，将自动创建所需的 Blob 容器和队列。
...
然后，它获取对"映像"Blob 容器的引用，创建尚不存在的容器，并在新容器上设置访问权限。默认情况下，新容器只允许带存储帐户凭据的客户端访问 Blob。网站需要 Blob 是公共的，以便它可以使用指向图像 Blob 的 Url 显示图像。
...
类似代码获取对"映像"队列的引用并创建一个新队列。这种情况不需要权限更改。
...
Views\Home\Index.cshtml* 文件在主页上显示类别链接。链接将查询字符串变量中的"类别"枚举的整数值传递到"广告索引"页面。
...
之后 HttpPost"Create"方法上载 Blob 并更新数据库，它会创建队列消息，以通知后端进程映像已准备好转换为一个缩略图。
...
*Create.cshtml* 和 *Edit.cshtml* 文件指定窗体编码，使该控制器可获取"HttpPostedFileBase"对象。
...
Azure 辅助角色环境在辅助角色启动时调用"WorkerRole"类中的"OnStart"方法，并且它在"OnStart"方法完成时调用"Run"方法。
...
在 Azure 中运行应用程序或使用 Azure 计算仿真程序本地运行时，"RoleEnvironment"对象由 Azure 提供。如果你在本地运行时收到此错误，请确保你已将 ContosoAdsCloudService 项目设为启动项目。这将项目设置为使用 Azure 计算仿真程序运行。
...
要使用完整仿真程序运行该应用程序，你必须使用管理员权限打开 Visual Studio。
