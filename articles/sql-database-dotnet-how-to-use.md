...
你可以使用 Visual Studio 中的所有相同设计器工具处理 SQL Database，就像可以使用这些工具处理 SQL Server 一样。服务器资源管理器允许你查看（但不能编辑）数据库对象。Visual Studio 实体数据模型设计器功能完备，你可以用它来创建针对 SQL Database 的模型以用于 Entity Framework。
