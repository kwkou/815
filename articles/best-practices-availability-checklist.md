<properties
   pageTitle="可用性核对清单 | Windows Azure"
...
- **适当处理急剧突发事件。** 大多数应用程序需要处理不同时间的不同工作负荷，例如业务应用程序中上午出现的第一波高峰，或者电子商务 Web 应用中发布新产品时。自动缩放有助于处理负载，但可能需要一些时间使其他实例联机并处理请求。通过将应用程序设计为将请求加入其使用的服务队列，并在队列接近完全容量时适当降级，以防止突发和意外的活动高峰导致超出应用程序的处理能力。确保有足够的性能和容量可供非高峰条件清空队列和处理未完成的请求。有关详细信息，请参阅[基于队列的负载调节模式](https://msdn.microsoft.com/zh-cn/library/dn589783.aspx)。
...
- 通过本地和地理冗余**利用数据复制**。Azure 存储空间中的数据会自动复制，以防止基础结构故障时发生数据丢失，你可以配置这种复制的某些方面。例如，可以在多个地理区域中复制数据的只读副本（称为全局读取访问冗余存储，简称 RA-GRS）。请注意，使用 RA-GRS 会产生附加费用，请参阅 Microsoft Web 应用上的 [Azure 存储空间定价](/home/features/storage/#price)页以获取详细信息。
...
- **使用定期备份和时间点还原**，并确保符合恢复点目标 (RPO)。定期自动备份未保留在其他位置的数据，并确认可以在发生故障时可靠还原数据和应用程序本身。数据复制并不是备份功能，因为故障、错误或恶意操作造成的错误和不一致性将会复制到所有存储中。备份过程必须是安全的，这样才能保护传输中和存储中的数据。数据库或部分数据存储通常可以使用事务日志恢复到以前的某个时间点。Windows Azure 针对存储在 Windows Azure SQL 数据库中的数据提供备份工具。数据将导出到 Windows Azure Blob 存储上的备份包，并且可下载到存储的安全本地位置。
- **启用高可用性选项来维护 Redis 缓存的辅助副本。** 使用 Redis 缓存时，请选择“标准”选项来维护内容的辅助副本。有关详细信息，请参阅 Microsoft Web 应用上的[在 Azure Redis 缓存中创建缓存](https://msdn.microsoft.com/zh-cn/library/dn690516.aspx)页。
...
- **跟踪长时间运行的工作流的进度**并在故障时重试。长时间运行的工作流通常由多个步骤组成。设计这些类型的工作流时，请确保每个步骤都是独立且可以重试的，以最大程度地减少整个工作流需要回滚，或需要执行多个补偿事务的可能性。通过实施计划程序代理监督程序等模式，来监视和管理长时间运行的工作流的进度。有关详细信息，请参阅 Microsoft Web 应用上的[计划程序代理监督程序模式](https://msdn.microsoft.com/zh-cn/library/dn589780.aspx)页。
