...
7.  直接从 Azure PowerShell 调用脚本，或将此脚本连接到在包生成后进行的主机生成自动化。
    ...
    **远程桌面：**
    ...
    或者，可以导出带私钥的证书文件 PFX，并使用 [Azure 经典门户](https://manage.windowsazure.cn)将证书上载到每个目标云服务。阅读以下文章以了解详细信息：[http://msdn.microsoft.com/zh-cn/library/windowsazure/gg443832.aspx][]。
