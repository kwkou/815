...
| length |否 |Integer |子字符串的字符数。 必须引用该字符串内的一个位置。 |
...
### <a id="list" name="listkeys"></a>listKeys 和 list{Value}
...
以 **list** 开头的任何操作都可用作模板中的函数。 可用操作不仅包括 **listKeys**，也包括 **list**、**listAdminKeys** 和 **listStatus** 等操作。 若要确定存在列表操作的资源类型，请参阅资源提供程序的 [REST API 操作](https://docs.microsoft.com/zh-cn/rest/api/)。
