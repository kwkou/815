...
在前面的示例中，通过复制名为 **storageLoop**的循环创建的资源包含依赖关系。 有关示例，请参阅 [在 Azure Resource Manager 中创建多个资源实例](/documentation/articles/resource-group-create-multiple/)。
...
[引用函数](/documentation/articles/resource-group-template-functions-resource/#reference)使表达式能够从其他 JSON 名值对或运行时资源中派生其值。 引用表达式隐式声明一个资源依赖于另一个资源。 常规格式为：
...
若要了解详细信息，请参阅[引用函数](/documentation/articles/resource-group-template-functions-resource/#reference)。
...
* 若要了解有关创建 Azure Resource Manager模板的信息，请参阅[创作模板](/documentation/articles/resource-group-authoring-templates/)。 
* 有关模板中的可用函数列表，请参阅[模板函数](/documentation/articles/resource-group-template-functions/)。
