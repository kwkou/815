...
本文将指导你完成使用 PowerShell cmdlet 重置 Azure VPN 网关。这些说明适用于经典部署模型。我们还没有 cmdlet 或 REST API 来为使用资源管理器部署模型创建的虚拟网络重置 VPN 网关。这些都正在开发中。你可以通过在 Azure 门户中查看虚拟网络来确定 VPN 网关是否是使用经典部署模型创建的。使用经典部署模型创建的虚拟网络显示在 Azure 门户的虚拟网络（经典）部分中。
