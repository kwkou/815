...
了解如何使用 R 通过脚本操作来自定义基于 Windows 的 HDInsight 群集，以及如何在 HDInsight 群集上使用 R。HDInsight 的[高级级别](/home/features/hdinsight/#price)产品/服务包括 R Server 作为 HDInsight 群集的一部分。这样将允许 R 脚本使用 MapReduce 和 Spark 运行分布式计算。有关详细信息，请参阅[Get started using R Server on HDInsight（开始使用 HDInsight 上的 R Server）](/documentation/articles/hdinsight-hadoop-r-server-get-started)。
