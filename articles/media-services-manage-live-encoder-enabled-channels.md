...
#使用能够通过 Azure 媒体服务（预览版）执行实时编码的频道

##概述

在 Azure 媒体服务中，**频道**表示用于处理实时流内容的管道。**频道**通过以下两种方式之一接收实时输入流：

- 本地实时编码器将多比特率 **RTMP** 或**平滑流式处理**（分片 MP4）发送到频道。可以使用以下输出多比特率平滑流式处理的实时编码器：Elemental、Envivio、Cisco。以下实时编码器输出 RTMP：Adobe Flash Live、Telestream Wirecast 和 Tricaster 转码器。引入流将通过**频道**，而不会进行任何进一步处理。收到请求时，媒体服务会将该流传递给客户。
- 将单比特率流（采用以下格式之一：**RTP** (MPEG-TS)、**RTMP** 或**平滑流式处理**（分片 MP4）发送到能够使用媒体服务执行实时编码的**频道**。然后，**频道**将对传入的单比特率流执行实时编码，使之转换为多比特率（自适应）视频流。收到请求时，媒体服务会将该流传递给客户。 
...
可以选择通过 SSL 连接引入分片 MP4 （平滑流式处理）实时流。若要通过 SSL 进行摄取，请确保将摄取 URL 更新为 HTTPS。
...
**注意**：当前，不管指定了哪种输入类型，都只能以分片 MP4（平滑流式处理）格式来传送预览流。你可以使用 [http://smf.cloudapp.net/healthmonitor](http://smf.cloudapp.net/healthmonitor) 播放器来测试平滑流。你还可以使用 Azure 管理门户中承载的播放器来查看你的流。
...
当频道已启用实时编码时，你在管道中有一个组件正在处理视频，并可以操作它。你可以向频道发出信号，以将石板和/或广告插入到传出自适应比特率流中。石板是在某些情况下（例如，在商业广告期间）可用于覆盖输入实时源的静止图像。广告信号是嵌入到传出流中的时间同步信号，用于指示视频播放机执行特殊操作（例如，在适当时间切换到广告）。有关用于此目的的 SCTE-35 信号机制的概述，请参阅此[博客](https://codesequoia.wordpress.com/2014/02/24/understanding-scte-35/)。下面是可以在实时事件中实现的典型方案。
...
可选。指示实时编码器在商业广告期间切换到 [默认石板](/documentation/articles/media-services-manage-live-encoder-enabled-channels#default_slate)图像并隐藏传入视频源。在石板期间音频也会静音。默认值为 **false**。
 
所用图像将是在创建频道时通过默认石板资产 ID 属性指定的图像。石板将进行拉伸以适合显示图像大小。


##插入石板图像

可以向频道中的实时编码器发出信号，以切换到石板图像。它还可以指示实时编码器结束正在显示的石板。

在某些情况下（例如，在广告期间）可以配置实时编码器以切换到石板图像，并隐藏传入的视频信号。如果未配置此类石板，则在该广告期间将不会屏蔽输入视频。

###持续时间

石板的持续时间（以秒为单位）。这必须是非零正值，才能启动石板。如果正在显示石板，并且指定的持续时间为零，则将终止正在显示的石板。

###在 Ad 标记上插入石板

当设为 true 时，此设置会将实时编码器配置为在广告期间插入石板图像。默认值为 true。

###<a id="default_slate"></a>默认石板资产 ID

可选。指定媒体服务资产（包含石板图像）的资产 ID。默认值为 null。

**注意**：在创建频道之前，具有以下约束的石板图像应当作为专用资产上载（该资产中不应有其他文件）。

- 分辨率最大为 1920 x 1080。
- 大小最大为 3 兆字节。
- 文件名必须具有 *.jpg 扩展名。
- 必须将该图像作为资产中的唯一 AssetFile 上载到资产，此 AssetFile 应标记为主文件。此资产不能加密存储。

如果未指定**默认石板资产 ID**，并且**“在 ad 标记上插入石板”**设为 **true**，则将使用默认 Azure 媒体服务图像隐藏输入视频流。在石板期间音频也会静音。
...
- 通道启动可能需要 20 多分钟。
- RTP 支持迎合专业的广播装置。请查看[本](http://azure.microsoft.com/blog/2015/04/13/an-introduction-to-live-encoding-with-azure-media-services/)博客中有关 RTP 的说明。
- 石板图像应符合[此处](/documentation/articles/media-services-manage-live-encoder-enabled-channels#default_slate)所述的限制。如果您想要尝试创建默认石板大于 1920 x 1080 的频道，最终请求将会出错。
