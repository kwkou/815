...
在你为虚拟机和物理服务器设置保护后，它们将开始复制到辅助位置。复制正常进行后，你可以根据需要运行故障转移。Site Recovery 支持多种类型的故障转移。
...
VMware 站点到 Azure | 不支持 |此方案使用连续复制，因此在计划和非计划故障转移之间没有区别。选择“故障转移” | 不可用
...
2. **导入和导出** — 你可以通过导出，然后使用新的 GUID 导入，来创建 Active Directory 虚拟机的副本。
