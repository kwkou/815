<properties 
    pageTitle="使用弹性数据库拆分/合并工具进行缩放 | Windows Azure" 
...
拆分/合并服务使用基于 Azure SDK 2.5 的 Azure Diagnostics 进行监视与诊断。可以根据此处所述控制诊断配置：[在 Azure 云服务和虚拟机器中启用诊断](/documentation/articles/cloud-services-dotnet-diagnostics)。下载包包含两个诊断配置 – 一个用于 Web 角色，另一个用于辅助角色。这些服务诊断配置遵循 [Windows Azure 中的云服务基础知识](https://code.msdn.microsoft.com/windowsazure/Cloud-Service-Fundamentals-4ca72649)中的指导原则。它包括用于记录性能计数器、IIS 日志、Windows 事件日志和拆分/合并应用程序事件日志的定义。
