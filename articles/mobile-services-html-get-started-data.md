...
6. 在**"配置"**选项卡中，验证 `localhost` 是否已列在**"跨域资源共享(CORS)"**下的**"允许来自以下主机名的请求"**列表中。如果未列出，请在**"主机名"**字段中键入 `localhost`，然后单击**"保存"**。
	...
	> [AZURE.NOTE] 如果将快速入门应用程序部署到除 localhost 以外的 Web 服务器，则必须将该 Web 服务器的主机名添加到**"允许来自以下主机名的请求"**列表中。有关详细信息，请参阅[跨域资源共享](https://msdn.microsoft.com/zh-CN/library/windowsazure/dn155871.aspx"%20target="_blank)。
