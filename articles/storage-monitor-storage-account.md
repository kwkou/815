<properties
	...
	description="了解如何使用 Azure 经典门户在 Azure 中监视存储帐户。"
	services="storage"
	documentationCenter=""
	authors="tamram"
	manager="adinah"
...
你可以从 [Azure 经典门户](https://manage.windowsazure.cn)监视你的存储帐户。在配置您的存储帐户用于通过门户进行监视时，Azure 存储空间使用[存储分析](http://msdn.microsoft.com/zh-cn/library/azure/hh343270.aspx)跟踪帐户和日志请求数据的指标。

> [AZURE.NOTE] 在 [Azure经典门户](https://manage.windowsazure.cn)中查看监视数据会产生相关的额外费用。有关详细信息，请参阅<a href="http://msdn.microsoft.com/zh-cn/library/azure/hh360997.aspx">存储分析和计费</a>。<br />

> Azure 文件存储目前支持存储分析指标，但尚不支持日志记录。可以通过 [Azure 经典门户](https://manage.windowsazure.cn)为 Azure 文件存储启用指标。
...
1. 在 [Azure 经典门户](https://manage.windowsazure.cn/)中，单击“存储”，然后单击存储帐户名称以打开仪表板。
...
在设置监视级别和保留策略后，你可以选择要在 [Azure 经典门户](https://manage.windowsazure.cn)中监视哪些可用指标，以及要在指标图表上显示哪些指标。将在每个监视级别显示一组默认度量值。您可以使用“添加度量值”在度量值列表中添加或删除度量值。
...
5. 单击度量值旁边的向下箭头，以展开该度量值范围内所包括的操作的列表。选择要在 [Azure 经典门户](https://manage.windowsazure.cn)的度量值表中查看的每项操作。
...
1. 在[经典门户](https://manage.windowsazure.cn)中, 单击“存储”，然后单击存储帐户名称以打开仪表板。
