...
可以在任务的多实例设置中指定一个或多个通用资源文件。主要任务及所有子任务从 [Azure 存储空间](/documentation/articles/storage-introduction/)将这些通用资源文件下载到每个节点的任务共享目录。可以使用 `AZ_BATCH_TASK_SHARED_DIR` 环境变量从应用程序命令和协调命令行访问任务共享目录。
