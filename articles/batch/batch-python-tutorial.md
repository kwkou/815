...
若要实际执行工作，必须将任务添加到作业。每个 [CloudTask][py_task] 使用命令行属性和 [ResourceFiles][py_resource_file]（如同池的 StartTask）进行配置，ResourceFiles 已在其命令行自动执行前由任务下载到节点。在本示例中，每个任务只处理一个文件。因此，其 ResourceFiles 集合包含单个元素。
...
完成作业后，可以从 Azure 存储下载任务的输出。可通过在 *python\_tutorial\_client.py* 中调用 `download_blobs_from_container` 来实现此目的：
