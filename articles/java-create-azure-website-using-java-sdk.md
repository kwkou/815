<properties 
	...
	description="了解如何使用 Azure SDK for Java 以编程方式创建 Web 应用。" 
...
<tags 
	ms.service="multiple" 
	ms.date="08/31/2015" 
	wacn.data="10/17/2015"/>
# 使用 Azure SDK for Java 在 Azure 上创建 Web 应用

<!-- Azure Active Directory workflow is not yet available on the Azure Portal -->

## 概述

本演练演示如何创建一个 Azure SDK for Java 应用程序，以便在 Azure 上创建网站，然后将一个应用程序部署到该网站。它由两个部分组成：
...
若要将自签名证书上载到 Azure，请转到 Azure 管理门户中的“设置”页，然后单击“管理证书”选项卡。单击页面底部的“上载”，然后导航到你创建的 CER 文件的所在位置。
...
因为 AzureWebDemo 应用程序的目的是创建 Web 应用，所以请将此应用程序的主类命名为 `WebAppCreator`。此类提供调用 Azure 服务管理 API 的主入口点代码，以创建 Web 应用。
...
为 Web 应用和 Web 空间添加以下参数定义。你将需要提供你自己的 Azure 订阅 ID 和证书信息。
    ...
    public class WebAppCreator {
      ...
      private static String domainName = "DemoWebsite";
...
- `webAppName` 可以是你选择的任何名称；此过程使用名称 `WebDemoWebApp`。完整的域名称是 `webAppName`，并追加了 `domainName`，因此在这种情况下，完整的域是 `demowebsite.chinacloudsites.cn`。
...
接下来，定义用于创建 Web 应用的方法。此方法（`createWebApp`）指定 Web 应用的参数和 Web 空间。它还会创建并配置 Azure 网站管理客户端，该客户端由 WebSiteManagementClient 对象进行定义。管理客户端对于创建 Web Apps 至关重要。它提供 RESTful web 服务，使应用程序能够通过调用服务管理 API 来管理 Web Apps（执行创建、更新和删除等操作）。
    ...
    private static void createWebApp() throws Exception {
        ...
        // Specify configuration settings for Service management client.
      ...
      // Create the website management client to call Azure APIs;
        ...
        // Create an Azure Websites plan for the web app with the specified parameters.
        ...
        // Note that the server farm name takes the Azure Websites plan name.
...
登录 AMP，然后单击“Web Apps”。在数分钟内，新 Web 应用应会出现在“Web Apps”列表中。


## 将应用程序部署到 Web 应用

运行 AzureWebDemo 并创建新网站后，请登录 AMP，单击“网站”，然后在“网站”列表中选择“WebDemoWebsite”。在网站的仪表板页上，单击“浏览”（或单击网站 URL `webdemowebsite.chinacloudsites.cn`）导航到该网站。你将会看到一个空白的占位符页，因为尚未将任何内容发布到网站。
...
为了演示如何将应用程序部署到 Web，以下过程说明了如何创建简单的 "Hello World" Java 应用程序，并将其上载到应用程序创建的 Web 应用。
...
有关部署 WAR 文件的详细信息，请参阅[将 Java 应用程序添加到 Azure Web Apps](/documentation/articles/web-sites-java-add-app/)。
...
> **注意：**Azure Plugin for Eclipse with Java 2.4 支持部署到存储帐户和云服务，但当前不支持部署到 Web Apps。你可以按照[在 Eclipse 中为 Azure 创建 Hello World 应用程序](http://msdn.microsoft.com/zh-cn/library/azure/hh690944.aspx)中所述，使用 Azure 部署项目部署到存储帐户和云服务，但不能部署到 Web Apps。使用其他方法（例如 FTP 或 GitHub）将文件传输到 Web 应用。
...
有关使用 FTP 部署道 Web 应用的详细信息，请参阅以下主题：
...
1. 登录 AMP，然后单击“Web Apps”。确保“WebDemoWebApp”已显示在 Web Apps 列表中，并确保它正在运行。单击“WebDemoWebApp”以打开其“仪表板”页。
...
1. 在 AMP 中，转到 Web 应用的“仪表板”页，然后单击“配置”。在“配置”页上指定以下设置。
...
发布应用程序的一种方法是使用 Azure 中内置的 Kudu 调试控制台。已知 Kudu 很稳定并与 Web Apps 和 Tomcat 服务器一致。可以通过浏览到以下形式的 URL 来访问 Web 应用的控制台：
...
2. 在“站点管理员”对话框中，单击“新建站点”。随后，“选择条目”中会出现新的空白 FTP 站点，提示你提供名称。对于此过程，请将它命名为 `AzureWebDemo-FTP`。
    ...
    在“常规”选项卡上，指定以下设置： -“主机”：输入从仪表板中复制的“FTP 主机名” -“端口”：（将其留空，因为这是被动传输并且服务器会确定使用的端口。）-“协议”：FTP 文件传输协议 -“加密”：使用普通的 FTP -“登录类型”： 普通 -“用户”：输入从仪表板中复制的部署/FTP 用户。这是完整的 FTP 用户名，其形式为 *webappname\\username*。-“密码”：输入在设置部署凭据时指定的密码。
...
1. 上载 WAR 文件并确认 Tomcat 服务器已创建解包的 `JSPHello` 目录后，请浏览到 `http://webdemowebsite.chinacloudsites.cn/JSPHello` 以运行该应用程序。

    > **注意：**如果从 AMP 单击“浏览”，则可能获得默认网页，网页显示“已成功创建此基于 Java 的 Web 应用程序。” 你可能需要刷新网页才能查看应用程序输出，而不是默认网页。
...
此过程创建 Web 应用。只要 Web 应用存在，你就要支付资源的费用。除非你打算继续使用该 Web 应用进行测试或开发，否则应考虑停止或删除它。已停止的 Web 应用仍会产生较小的费用，但你随时可以重新启动它。删除某个 Web 应用会清除已上载到该 Web 应用的所有数据。
