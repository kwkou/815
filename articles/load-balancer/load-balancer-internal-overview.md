...
该基础结构限制了可访问性，在云服务或虚拟网络的经过负载均衡的虚拟 IP 地址之间创建信任边界，不会直接公开给 Internet 终结点。这样就可以让内部业务线应用程序在 Azure 中运行，只能在云中或本地进行访问。
...
现有的 Azure 负载均衡仅在基于 Internet 的计算机与云服务中的虚拟机之间进行负载均衡。可通过 ILB 启用在 Azure 中托管虚拟机的新功能。
