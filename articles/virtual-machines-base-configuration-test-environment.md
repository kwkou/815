...
基本配置测试环境由仅限云的 Azure 虚拟网络中名为 TestLab 的公司网络子网组成，它模拟连接到 Internet 的简化专用 Intranet。 
...
首先，你可以创建将托管基本配置的公司网络子网的 TestLab Azure 虚拟网络。
...
首先，使用以下命令选择相应的 Azure 订阅。将引号内的所有内容（包括 < 和 > 字符）替换为相应的名称。
...
你必须为云服务选择唯一名称。 *云服务名称只能包含字母、数字和连字符。该字段中的第一个和最后一个字符必须是字母或数字。* 
...
接下来，可以配置将包含虚拟机的磁盘和额外数据磁盘的存储帐户。 *必须选择仅包含小写字母和数字的唯一名称。*你可以使用此 Azure PowerShell 命令测试存储帐户名称的唯一性。
...
Azure 中的基本配置现已可用于应用程序开发和测试或其他测试环境，如[模拟的混合云环境](/documentation/articles/virtual-networks-setup-simulated-hybrid-cloud-environment-testing)。 
