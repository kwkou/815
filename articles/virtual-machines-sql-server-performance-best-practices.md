...
|**存储**|使用[高级存储](../storage/storage-premium-storage-preview-portal.md)。<br/><br/>使[存储帐户](../storage/storage-create-storage-account.md)和 SQL Server VM 保存在同一个区域中。<br/><br/>禁用存储帐户上的 Azure [地域冗余存储](../storage/storage-redundancy.md)（异地复制）。|
|**磁盘**|至少使用 2 个 [P30 磁盘](../storage/storage-premium-storage-preview-portal.md#scalability-and-performance-targets-whzh-CNing-premium-storage)（1 个用于日志文件；1 个用于数据文件和 TempDB）。<br/><br/>避免使用操作系统磁盘或临时磁盘用于数据库存储或日志记录。<br/><br/>在托管数据文件和 TempDB 的磁盘上启用读缓存。<br/><br/>请勿在托管日志文件的磁盘上启用缓存。<br/><br/>条带化多个 Azure 数据磁盘以获得更高的 IO 吞吐量。<br/><br/>用已归档的分配大小格式化。|
