...
3. 在“Azure 存储帐户配置”部分：
    1. 输入你的存储帐户名称，可以通过[Azure 经典门户](https://manage.windowsazure.cn)获取该帐户名称。
    2. 输入你的存储帐户密钥，同样可以从 Azure 经典门户获取该密钥。
    3. 如果你在使用公共 Azure 云，对于“Blob 服务终结点 URL”，请使用默认值。如果你在使用其他 Azure 云，则使用在 Azure 经典门户中为你的存储帐户指定的终结点。 
...
- **URL 格式**：可使用以下 URL 格式对 Blob 寻址：
    ...
    （以上格式适用于公共 Azure 云。如果你在使用其他 Azure 云，请使用 Azure 经典门户中的终结点来确定你的 URL 终结点。）
