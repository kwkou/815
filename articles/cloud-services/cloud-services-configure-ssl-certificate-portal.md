...
现在已更新服务定义和服务配置文件，请打包部署以上载到 Azure。如果你使用的是 **cspack**，请不要使用 **/generateConfigurationFile** 标志，因为这将覆盖你刚插入的证书信息。

## 步骤 3：上载证书

连接到该门户预览和...

1. 选择云服务：在门户预览中，选择“云服务”。（位于“所有资源”部分中。）
...
2.  在 Web 浏览器中，修改链接以使用 **https** 而不是 **http**，然后访问该页。
    ...
    >[AZURE.TIP] 若要对过渡部署而非生产部署使用 SSL，你首先需要确定用于过渡部署的 URL。在部署云服务后，过渡环境的 URL 由**部署 ID** GUID 决定，其格式为：`https://deployment-id.chinacloudapp.cn/`
      
    >创建一个证书，使其公用名 (CN) 等于基于 GUID 的 URL（例如，**328187776e774ceda8fc57609d404462.chinacloudapp.cn**），然后使用门户将该证书添加到你的过渡云服务。然后，将证书信息添加到 CSDEF 和 CSCFG 文件，重新打包应用程序，并更新过渡部署以使用新包。
