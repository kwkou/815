...
使用 Windows Azure 流量管理器可以控制向指定的终结点（可能包括 Azure 云服务、 Web 应用和其他终结点）分配用户流量。流量管理器的工作方式是将智能策略引擎应用到对 Internet 资源域名的域名系统 (DNS) 查询。Azure 云服务或 Web 应用可以在世界各地不同的数据中心内运行。
