...
2. 连接到媒体服务 URI。  
    ...
    >[AZURE.NOTE] 
    >- 上海 DC URL（中国东部）：https://wamsshaclus001rest-hs.chinacloudapp.cn/API/
    >- 北京 DC URL（中国北部）：https://wamsbjbclus001rest-hs.chinacloudapp.cn/API/

3. 将 API 调用直接发布到上面的 URL（在其中创建媒体服务）。 
...
请在此请求的正文中提供 client_id 和 client_secret 值；client_id 和 client_secret 分别对应于 AccountName 和 AccountKey 值。 在你设置帐户时，媒体服务将向你提供这些值。
...
> [AZURE.NOTE]
> 建议在外部存储中缓存“access_token”和“expires_in”（访问令牌的有效期，以秒为单位）值。 以后可以从存储中检索令牌数据，并在媒体服务 REST API 调用中重新使用。 这对于令牌可以在多个进程或多台计算机之间安全共享的方案尤其有用。
>
>
...
[AssetFile](https://docs.microsoft.com/zh-cn/rest/api/media/operations/assetfile) 实体表示 blob 容器中存储的视频或音频文件。 一个资产文件始终与一个资产关联，而一个资产则可能包含一个或多个 AssetFiles。 如果资产文件对象未与 BLOB 容器中的数字文件关联，则媒体服务 Encoder 任务将失败。
...
有关使用 Azure 存储 blob 的详细信息，请参阅 [Blob 服务 REST API](https://docs.microsoft.com/zh-cn/rest/api/storageservices/fileservices/Blob-Service-REST-API)。
...
将资产引入媒体服务后，即可对媒体进行编码、传输复用、打水印等处理，然后将其传送至客户端。 将根据多个后台角色实例调度把那个运行这些活动，以确保较高的性能和可用性。 这些活动称为作业，每个作业由原子任务构成，这些原子任务在资产文件上完成具体的工作（有关详细信息，请参阅[作业](https://docs.microsoft.com/zh-cn/rest/api/media/operations/job)、[任务](https://docs.microsoft.com/zh-cn/rest/api/media/operations/task)描述）。 
...
以下示例说明了如何使用一个任务集来创建和发布一个作业，从而以特定分辨率和质量来编码某个视频。 以下文档部分包含 Media Encoder Standard 处理器支持的所有[任务预设](http://msdn.microsoft.com/zh-cn/library/mt269960)的列表。  
