...
将自定义 DNS 服务器作为 Azure VM 托管时，可将主机名查询（针对同一 VNet）转发到 Azure 以解析内部 IP。如果不希望使用此路由，可使用动态 DNS 在 DNS 服务器中注册 VM。Azure 不具备直接在你的 DNS 服务器中创建记录的功能（如凭据），因此通常需要替代方案。以下是一些常见的替代方案。
...
## Linux 客户端
Linux 客户端通常在启动时注册到 DNS 服务器，而是假定 DHCP 服务器将执行此操作。Azure 的 DHCP 服务器不能在你的 DNS 服务器中注册记录。可以使用名为 nsupdate 的工具发送动态 DNS 更新，该工具包含于绑定包。由于动态 DNS 协议是标准化的，所以即使在 DNS 服务器上未使用绑定时，也可以使用 nsupdate。
