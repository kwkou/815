...
Azure 多重身份验证可以使 Azure 管理员安全访问 Azure 经典管理门户。然而, Azure 经典管理门户的登录协议和终结点不同于 Azure 账户门户（在里面可以查看账单）和 Office 365的登录协议和终结点。因此, 这些产品和门户目前不支持启用多重身份验证的帐户来登录。整合其他产品和门户使其使用和 Azure 经典管理门户中相同的登录协议和终结点的工作正在进行中。在那之前,任何启用多重身份验证的帐户只能访问 Azure 经典管理门户，如果试图访问其他地方则会报错。
