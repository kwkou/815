...
本文讲述如何使用 Azure 备份配置 SQL Server 数据库备份。

若要将 SQL Server 数据库备份到 Azure，你需要一个 Azure 帐户。如果你没有帐户，只需花费几分钟就能创建一个试用帐户。详细信息请参阅 [Azure 试用](/pricing/free-trial/)。

Azure 上备份和 恢复 SQL Server 数据库步骤如下：
...
7. 在“指定短期目标”屏幕中，提供创建磁盘备份点所需的输入。
    ...
    >[AZURE.NOTE] 在每天晚上 8:00（根据屏幕输入），将会创建一个备份点，传输自前一天晚上 8:00 的备份点以来。此过程称为**快速完整备份**。虽然事务日志每 15 分钟同步一次，但如果需要在晚上 9:00 恢复数据库，则会重播自上一个快速完整备份点（在本示例中为晚上 8 点）以来的日志，从而创建备份点。
...
8. 单击“下一步”
    ...
    DPM 为每个数据源（SQL Server 数据库）创建一个卷，用来创建初始备份副本。逻辑磁盘管理器 (LDM) 限制 DPM 最多只能保护 300 个数据源（SQL Server 数据库）。为了避免出现这种情况，DPM 实现了另一个方法，对多个数据源使用单个卷。“在 DPM 存储池中共置数据”即可启用此功能。使用此方法时，DPM 最多可以保护 2000 个 SQL 数据库。
...
9. 管理员可以选择手动传输此初始备份（脱离网络），以免网络出现带宽拥塞现象。管理员还可以配置初始传输发生的时间。单击**“下一步”**。
    ...
    初始备份副本需要将整个数据源（SQL Server 数据库）从生产服务器（SQL Server 计算机）传输到 DPM 服务器。此数据有时可能会非常大，通过网络传输可能会超出带宽限制。因此，管理员可对传输方式进行选择：手动传输或自动传输，手动可避免带宽拥堵。此外，当管理员选择“网络”时，他们可以选择是“立即”创建初始备份副本，还是“稍后”（在某个指定的时间）创建。
...
10. 选择需要运行一致性检查的时间，然后单击“下一步”。
    ...
    DPM 可以通过执行一致性检查来检查备份点的完整性。它会计算生产服务器（在本方案中为 SQL Server 计算机）上的备份文件和 DPM 上该文件的已备份数据的校验和。如果发生冲突，则推测 DPM 上的备份文件受损。DPM 会发送与校验和不匹配的块以纠正备份的数据。由于一致性检查是对性能影响很大的操作，因此系统允许管理员选择是按计划来运行它，还是自动运行它。
...
12. 管理员可以选择适合组织策略的备份计划和保留策略。
    ...
    本示例中每天中午 12:00 和晚上 8:00 各进行一次备份（参见屏幕底部）
...
13. 选择保留策略计划。有关保留策略工作原理的详细信息，请参阅[使用 Azure 备份来取代磁带基础结构文章](/documentation/articles/backup-azure-backup-cloud-as-tape/)。
    ...
    - 星期六中午 12:00 进行的备份会保留 104 周
    - 最后一个星期六中午 12:00 进行的备份会保留 60 个月
    - 3 月的最后一个星期六中午 12:00 进行的备份会保留 10 年
