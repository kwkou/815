...
这将以所有默认选项安装代理。安装将在几分钟内在后台完成。如果你没有指定 */nu* 选项，则安装结束时，会打开“Windows Update”窗口，以检查是否有任何更新。安装之后，代理将显示在已安装程序列表中。
...
## 注册到 Azure 备份服务
在可注册 Azure 备份服务之前，需要确保符合[先决条件](/documentation/articles/backup-configure-vault/)。必须具备以下条件：
...
- 应运行备份的“星期日期”。可以选择只在一周的某一天、一周的每一天或其中任意日期组合运行备份作业。
