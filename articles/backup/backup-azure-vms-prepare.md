...
如果确定环境满足这些条件，请转到[备份 VM 文章](/documentation/articles/backup-azure-vms/)。否则，请继续阅读本文，其中将会引导你逐步完成准备环境来备份 Azure VM 的过程。


## 备份和还原 VM 时的限制

>[AZURE.NOTE] Azure 有两种用于创建和使用资源的部署模型：[资源管理器部署模型和经典部署模型](/documentation/articles/resource-manager-deployment-model/)。以下列表提供了在经典模型中部署时的限制。
...
1. 登录到 [Azure 门户预览](http://manage.windowsazure.cn/)。
...
- [Whitelist the Azure datacenter IP ranges](http://www.microsoft.com/en-us/download/details.aspx?id=41653)（将 Azure 数据中心 IP 范围加入白名单）- 请参阅相关文章以获取有关如何将 IP 地址加入白名单的说明。
- 部署 HTTP 代理服务器来路由流量。
...
|选项|优点|缺点|
|------|----------|-------------|
|将 IP 范围加入允许列表| 无额外成本<br><br>若要在 NSG 中打开访问权限，请使用 <i>Set-AzureNetworkSecurityRule</i> cmdlet | 由于受影响的 IP 范围会不断变化，管理起来很复杂。<br><br>允许访问整个 Azure，而不只是存储。|
