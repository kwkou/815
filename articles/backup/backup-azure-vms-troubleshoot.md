...
| 备份操作 | 错误详细信息 | 解决方法 |
| -------- | -------- | -------|
|备份|	无法执行该操作，因为 VM 已不存在。- 停止保护虚拟机，但不删除备份数据。如需更多详细信息，请参阅 http://go.microsoft.com/fwlink/?LinkId=808124 |当主 VM 已删除，而备份策略仍继续查找用于执行备份的 VM 时，将会发生这种情况。若要修复此错误，请执行以下操作：<ol><li>使用相同的名称和相同的资源组名称[云服务名称]重新创建虚拟机，<br>（或者）</li><li>停止保护虚拟机（无论是否删除备份数据）。[更多详细信息](http://go.microsoft.com/fwlink/?LinkId=808124)</li></ol>|
...
|备份 |虚拟机不存在。- 请确保该虚拟机存在，或选择其他虚拟机。 | 当主 VM 已删除，而备份策略仍继续查找用于执行备份的 VM 时，将会发生这种情况。若要修复此错误，请执行以下操作：<ol><li>使用相同的名称和相同的资源组名称[云服务名称]重新创建虚拟机，<br>（或者）<br></li><li>停止保护虚拟机，但不删除备份数据。[更多详细信息](http://go.microsoft.com/fwlink/?LinkId=808124)</li></ol>|
...
| 备份 | 发生内部错误，备份失败 - 请在几分钟后重试操作。如果问题持续出现，请联系 Microsoft 支持 | 导致此错误发生的原因有 2 个：<ol><li>访问 VM 存储时发生暂时性问题。请检查“Azure 状态”，以确定区域中的计算/存储/网络是否存在任何相关问题。解决问题后，请重试备份。<li>已删除原始 VM，因此无法进行备份。若要保留已删除 VM 的备份数据，且要防止备份错误，请取消保护 VM 并选择保留数据选项。这样即可停止备份计划以及重复出现的错误消息。 |
...
| 备份 | 虚拟机上不存在虚拟机代理 - 请安装所需的系统必备组件和 VM 代理，然后重新启动操作。 | 详细了解如何安装 VM 代理以及如何验证 VM 代理安装。 |
...
1. 使用 NSG 阻止对存储进行网络访问<br>
	详细了解如何使用 IP 白名单或通过代理服务器对存储[启用网络访问](/documentation/articles/backup-azure-vms-prepare/)。
...
2. 为 HTTP 流量创建路径
    - 如果你指定了某种网络限制（例如网络安全组），请部署 HTTP 代理服务器来路由流量。可在[此处](/documentation/articles/backup-azure-vms-prepare/)找到部署 HTTP 代理服务器的步骤。
