...
2. 选择“创建新服务器”并输入服务器名称、用户名和密码。
	...
	如果你已有数据库服务器，则可以选择该服务器而无需创建服务器。数据库服务器是非常宝贵的资源，而且你通常会想要在同一台服务器上创建多个数据库来进行测试和开发，而不是在每个数据库中各创建一个数据库服务器。不过，在本教程中，你只是暂时需要服务器。通过在相同的资源组中创建服务器作为 WEB 应用，你可以在完成教程时轻松删除 WEB 应用和数据库资源。
...
4. 单击**“确定”**。
	...
	Visual Studio 将创建 ContactManager Web 项目、创建资源组和指定的 App Service 计划，并在 Azure 中创建具有指定名称的 WEB 应用。
...
4. 右键单击 WEB 应用并选择“停止”。
	...
	![stop WEB 应用portal](./media/web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database/stopweb.png)
