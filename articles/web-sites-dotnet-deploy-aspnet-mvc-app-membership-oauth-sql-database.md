...
本教程演示如何构建安全的 ASP.NET MVC 5网站，以便用户能够使用 Facebook 或 Google 凭据进行登录。该应用是一个简单的联系人列表，它使用 ADO.NET Entity Framework 进行数据库访问。你需要将该应用部署到 [Azure 网站](/documentation/services/web-sites/)。

完成本教程之后，你将能够在云中启动并运行安全的数据驱动的网站，以及使用云数据库。下图演示了已完成的应用程序的登录页。
...
* 如何部署 Web 项目，以便将数据库用于 Azure 网站中的 [网站](/documentation/services/web-sites/)。

>[AZURE.NOTE]这是一篇很长的教程。如果你想要快速了解 Azure 网站和 Visual Studio Web 项目，请参阅[在 Azure 网站中创建 ASP.NET网站](/documentation/articles/web-sites-dotnet-get-started)。有关疑难解答信息，请参阅[疑难解答](#troubleshooting)部分。
>
...
1. 在“新建项目”对话框中，展开“C#”并在“已安装的模板”下选择“Web”，然后选择“ASP.NET网站”。
...
3. 当“配置 Microsoft Azure 网站设置”对话框出现时，请确保已登录 Azure：如果还没有登录，现在请登录；如果登录已过期，请重新输入凭据。

	![重新输入凭据](./media/web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database/reentercredentials.png)

2. 如果想要指定网站的名称，请更改“网站名称”框中的值。

网站的 URL 将是 {name}.chinacloudsites.cn，因此该名称在 chinacloudsites.cn 域中必须是唯一的。配置向导建议在项目名称“ContactManager”的后面追加一个数字以使名称保持唯一，在本教程中你也可以这样做。

5. 在“App Service 计划”下拉列表中，选择“创建新的 App Service 计划”并输入名称，如下图中所示的“StandardWeb”。

	如果需要，你也可以选择已有的 App Service 计划。有关 App Service 计划的信息，请参阅 [Azure App Service 计划深入概述](/documentation/articles/azure-web-sites-web-hosting-plans-in-depth-overview)。

5. 在“资源组”下拉列表中，选择“创建新的资源组”并输入名称，如下图中所示的“ExampleMVC”。
...
7. 选择你附近的区域。
	...
	暂时不要单击“确定”。在下一步骤中，你将配置数据库资源。该对话框现在如下图所示。

	![新建计划和资源组](./media/web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database/newplanandgroup.png)
 
2. 选择“创建新服务器”并输入服务器名称、用户名和密码。

	服务器名称必须唯一。该名称可以包含小写字母、数字和短划线，但尾部不能包含短划线。用户名和密码是要为新服务器创建的新凭据。

	如果你已有数据库服务器，则可以选择该服务器而无需创建服务器。数据库服务器是非常宝贵的资源，而且你通常会想要在同一台服务器上创建多个数据库来进行测试和开发，而不是在每个数据库中各创建一个数据库服务器。不过，在本教程中，你只是暂时需要服务器。通过在相同的资源组中创建服务器作为网站，你可以在完成教程时轻松删除网站和数据库资源。

	如果选择现有的数据库服务器，请确保网站和数据库位于相同的区域。

	![使用新数据库](./media/web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database/newdb.png)

4. 单击**“确定”**。

	Visual Studio 将创建 ContactManager Web 项目、创建资源组和指定的 App Service 计划，并在 Azure 网站中创建具有指定名称的网站。
...
1. 按 Ctrl+F5 运行应用程序。
	...
	![网站在本地运行](./media/web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database/rr2.png)
...
4. 复制“SSL URL”。
	...
	SSL URL 将为 https://localhost:44300/，除非你之前已创建 SSL网站。
...
在本教程的后面，你会将应用程序部署到 Azure，在其中，你将使用 Google 或其他第三方身份验证提供程序进行登录。这会将你新注册的帐户添加到 *canEdit* 角色。发现网站的 URL 并且具有 Google ID 的任何人都能注册并更新你的数据库。若要阻止其他人这样做，你可以停止该网站。你可以通过检查数据库来验证具有 *canEdit* 角色的人员。
...
1. 在“服务器资源管理器”中，导航到“Azure > Azure 网站 > {你的资源组} > {你的网站}”。

4. 右键单击网站并选择“停止”。

	也可以从 [Azure 管理门户](https://manage.windowsazure.cn)中选择网站，然后单击页面底部的“停止”图标。
...
5. 从 Visual Studio 或门户启动网站。“网站停止时无法发布”。
	...
	![启动网站](./media/web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database/ss15.png)
...
* 部署时无法在“发布 Web”向导中选择连接字符串 - 如果你使用另一种方法来创建 Azure 资源（例如，尝试部署到在门户中创建的网站和 SQL 数据库），SQL 数据库可能与网站不相关联。最简单的解决方案是根据本教程中所示，使用 VS 创建新的网站和数据库。不需要从头开始学习本教程 - 在“发布 Web”向导中，可以选择创建新的网站，此时会出现你在创建项目时所看到的相同 Azure 资源创建对话框。
* 有关 Google 或 Facebook 开发人员门户的指导已过时 - 在本教程末尾的 Disqus 专门注释。

## 后续步骤

你已创建一个可对用户进行身份验证的基本 ASP.NET MVC网站。有关常见身份验证任务以及如何保护机密数据的详细信息，请参阅以下教程。

- [创建具有登录、电子邮件确认及密码重置功能的安全 ASP.NET MVC 5网站](http://www.asp.net/mvc/overview/getting-started/create-an-aspnet-mvc-5-web-app-with-email-confirmation-and-password-reset)
...
[Important information about ASP.NET in Azure 网站]: #aspnetwindowsazureinfo
