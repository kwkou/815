...
可以使用连接到未启用群集功能的缓存时使用的相同[终结点](/documentation/articles/cache-configure/#properties)、[端口](/documentation/articles/cache-configure/#properties)和[密钥](/documentation/articles/cache-configure/#access-keys)连接到你的缓存。Redis 在后端管理群集功能，因此不需要你通过客户端来管理它。
