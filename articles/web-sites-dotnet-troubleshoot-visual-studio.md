<properties 
	pageTitle="使用 Visual Studio 对 Azure 网站中的网站进行故障排除" 
	description="了解如何通过内置于 Visual Studio 2013 的远程调试、 跟踪和日志记录工具排除 Azure 网站的故障。" 
...
本教程将向你介绍如何使用 Visual Studio 工具，以通过远程运行[调试模式](http://www.visualstudio.com/zh-cn/get-started/debug-your-app-vs.aspx)或查看应用程序日志和 Web 服务器日志帮助调试 Azure 网站的网站。

学习内容：

* Visual Studio 中提供的 Azure 网站管理功能。
* 如何使用 Visual Studio 远程视图在远程网站中进行快速更改。
* 当项目在 Azure 中运行时，如何对网站和 Web 作业远程运行调试模式。
...
在本教程中所示的代码示例适用于 C# MVC网站，但对于 Visual Basic 和 Web 窗体应用程序，故障排除过程是一样的。
...
## <a name="sitemanagement"></a>网站配置和管理

通过 Visual Studio，用户可以访问管理门户中提供的网站管理功能和配置设置的子集。本节将向你介绍具体提供的功能。
...
3. 右键单击在 [Azure 和 ASP.NET 入门][GetStarted]中创建的网站的节点，然后单击“查看设置”。
	...
	此时将显示“Azure 网站”选项卡，你可在此处查看 Visual Studio 中提供的网站管理和配置任务。

	![“Azure 网站”窗口](./media/web-sites-dotnet-troubleshoot-visual-studio/tws-configtab.png)
...
## <a name="remoteview"></a>在服务器资源管理器中访问网站文件
...
1. 在“服务器资源管理器”中，展开“Azure”、“网站”、网站所在的资源组以及网站的节点。

	你将看到允许你访问网站的内容文件和日志文件的节点。

2. 展开“文件”节点，然后双击 *Web.config* 文件。

	![打开 Web.config](./media/web-sites-dotnet-troubleshoot-visual-studio/webconfig.png)

	Visual Studio 将从远程网站打开 Web.config 文件，并在标题栏中的文件名旁显示 [远程]。
...
4. 部署结束后，浏览器将打开网站的 Azure URL，关闭浏览器。

5. 对于 Visual Studio 2013：在“服务器资源管理器”中依次展开“Azure”和“网站”，右键单击你的网站，然后单击“附加调试器”。
...
6. 对于 Visual Studio 2012 Update 4：<a id="vs2012"></a>
	...
	* 在“限定符”框中，输入网站的 URL（不带前缀 `http://`）。

	* 选择“显示来自所有用户的进程”。

	* 当系统提示输入凭据时，请输入有权将内容部署到网站的用户的名称和密码。若要获取这些凭据，请转至管理门户中网站的“仪表板”选项卡，单击“下载发布配置文件”。在文本编辑器中打开该文件，可在 **userName=** 和 **userPWD=** 第一次出现的地方找到用户名和密码。
...
本部分说明如何使用你在 [Azure WebJobs SDK 入门](/documentation/articles/websites-dotnet-webjobs-sdk)中创建的项目和网站进行远程调试。本部分所示的功能只能在 Visual Studio 2013 Update 4 中使用。
...
3. 单击“设置”选项卡，将“配置”更改为“调试”，然后单击“发布”。
	...
	Visual Studio 将部署 Web 和 Web 作业项目，浏览器将打开你网站的 Azure URL。
...
* 不建议在生产环境中以调试模式运行。如果你的生产网站未进行扩展以容纳多个服务器实例，则调试会阻止 Web 服务器响应其他请求。如果你具有多个 Web 服务器实例，在附加至调试程序时将获得一个随机实例，而你无法确保后续浏览器请求将前往该实例。此外，调试版本一般不会部署到生产环境，针对版本生成的编译器优化可能可以逐行显示源代码中出现的情况。至于如何解决生产环境中出现的问题，可利用的最佳资源是应用程序跟踪和 Web 服务器日志。
...
* **Web 服务器日志**<br/>Web 服务器为每一个对网站的 HTTP 请求创建一个日志条目。
...
日志记录会影响网站性能，因此 Azure 允许你提供根据需要启用或禁用各种类型的日志。对于应用程序日志，可以指定只写入特定严重级别以上的日志。创建新的网站时，默认为禁用所有日志记录功能。
...
5. 在“应用程序跟踪”页面上，单击第一行中（不是浏览器链接中）的“查看详细信息”。
	...
	默认情况下，`trace.axd` 仅供本地使用。如果希望可以从远程网站使用，可将 `localOnly="false"` 添加到 `trace` 元素（位于 *Web.config* 文件中），如下所示：

		<trace enabled="true" writeToDiagnosticsTrace="true" localOnly="false" mostRecent="true" pageOutput="false" />

	然而，出于安全考虑，一般不建议在生产网站中启用 `trace.axd`，在随后的内容中你将了解到一种在 Azure 网站中更为便捷地读取跟踪日志的方法。
...
4. 在显示应用程序主页的浏览器窗口，单击“联系人”。
	...
	Visual Studio 仅显示错误级跟踪，因为这是启用日志监视服务时的默认设置。创建新的 Azure 网站时，默认禁用所有日志记录，正如之前打开设置页面时所见：
...
6. 在现在显示的是“联系人”页面的浏览器窗口中，单击“主页”，然后依次单击“关于”和“联系人”。
	...
	在本部分中，你已通过使用 Azure 网站设置启用和禁用了日志记录。你还可以通过修改 Web.config 文件启用和禁用跟踪侦听器。然而，修改 Web.config 文件会导致应用域回收，而通过网站配置启用日志记录则不会出此情况。如果问题重现需要花费较长时间或呈间歇性，回收应用域可能会“修正”该问题并强迫你一直等待直至其再次出现。在 Azure 中启用诊断功能不会如此，因此你可以立即开始捕获错误信息。
...
Web 服务器日志将记录网站上所有的 HTTP 活动。若要在“输出”窗口中查看这些日志，必须为网站启用日志并告知 Visual Studio 你希望对其进行监视。
...
详细的错误日志提供了有关导致错误响应代码（400或更高）的 HTTP 请求的一些额外信息。若要在“输出”窗口中查看这些日志，必须为网站启用日志并告知 Visual Studio 你希望对其进行监视。
...
5. 将“区域或地缘组”设置为离你最近的区域。
	...
	此设置指定哪个 Azure 数据中心将托管你的存储帐户。对于本教程，你做的选择不会带来明显的差异；但是，对于生产网站，你希望 Web 服务器和存储帐户处于同一区域，以最大程度地减少延迟和数据传出费用。（稍后要创建的）网站应在与访问网站的浏览器尽可能靠近的区域中运行，以最大程度地减少延迟。
...
2. 在 Visual Studio 中，在“Azure 网站”窗口的“”选项卡中，单击“在管理门户中打开”。
...
5. 在新的浏览器窗口，转至显示在网站的管理门户页面“仪表板”选项卡中“FTP 主机名”下的 URL。“FTP 主机名”的位置靠近“速览”部分内的“部署/FTP 用户”。

6. 使用之前创建的 FTP 凭据（包括作为用户名前缀的网站名称）登录。
...
* [你应该了解的 Windows Azure 网站联机工具](http://azure.microsoft.com/blog/2014/03/28/windows-azure-websites-online-tools-you-should-know-about-2/)。Amit Apple 发表的博客文章。
