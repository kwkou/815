<properties linkid="manage-linux-common-task-upload-vhd" urlDisplayName="Upload a VHD" pageTitle="在 Azure 中创建和上载 Linux VHD" metaKeywords="Azure VHD, uploading Linux VHD" description="了解如何创建和上载具有 Linux 操作系统的 Azure 虚拟硬盘 (VHD)。" metaCanonical="" services="virtual-machines" documentationCenter="" title="创建并上传包含 Linux 操作系统的虚拟硬盘" authors="kathydav" solutions="" manager="jeffreyg" editor="tysonn" />

# 创建并上传包含 Linux 操作系统的虚拟硬盘

Azure 中的虚拟机运行您在创建虚拟机时选择的操作系统。Azure 采用 VHD 格式（.vhd 文件）将虚拟机的操作系统存储在虚拟硬盘中。已为复制准备的操作系统的 VHD 称作映像。本文说明如何利用已安装且经过一般化的操作系统上载 .vhd 文件来创建您自己的映像。有关 Azure 中的磁盘和映像的详细信息，请参阅[管理磁盘和映像（可能为英文页面）][管理磁盘和映像（可能为英文页面）]。

**注意**：创建虚拟机时，您可以自定义操作系统设置以快速运行您的应用程序。您设置的配置存储在该虚拟机的磁盘上。有关说明，请参阅[如何创建自定义虚拟机][如何创建自定义虚拟机]。
...
-   **管理证书** - 您已为要为其上载 VHD 的订阅创建一个管理证书，并且已将该证书导出到 .cer 文件。有关创建证书的详细信息，请参阅[为 Azure 创建管理证书（可能为英文页面）][为 Azure 创建管理证书（可能为英文页面）]。

-   **安装在 .vhd 文件中的 Linux 操作系统。** - 您已将支持的 Linux 操作系统安装到某一虚拟硬盘。可使用多个工具创建 .vhd 文件。可使用虚拟化解决方案（例如 Hyper-V）创建 .vhd 文件并安装操作系统。有关说明，请参阅[安装 Hyper-V 角色和配置虚拟机][安装 Hyper-V 角色和配置虚拟机]。
...
-   **Linux Azure 命令行工具。**如果您正在使用 Linux 操作系统创建映像，请使用此工具上载 VHD 文件。若要下载此工具，请参阅[针对 Mac 和 Linux 的 Azure 命令行工具][针对 Mac 和 Linux 的 Azure 命令行工具]。
...
-   Azure Linux 代理 (waagent) 与 NetworkManager 不兼容。网络配置应可通过 ifup/ifdown 脚本来进行控制。由分布提供的大多数 RPM/Deb 包会将 NetworkManager 配置为 waagent 包的冲突，因此当您安装 Linux 代理包时将卸载 NetworkManager。Azure Linux 代理还需要安装 python-pyasn1 包。

-   由于低于 2.6.37 的 Linux 内核版本中存在错误，因此 NUMA 不支持更大的 VM 大小。手动安装 waagent 将在 Linux 内核的 GRUB 配置中自动禁用 NUMA。此问题主要影响使用上游 Red Hat 2.6.32 内核的分发。

-   建议您在安装时不要创建 SWAP 分区。可以通过使用 Azure Linux 代理配置 SWAP 空间。此外，建议不要将主流 Linux 内核用于不带 [Microsoft 网站（可能为英文页面）][Microsoft 网站（可能为英文页面）]上提供的修补程序（许多当前分发/内核可能已包含此修补程序）的 Azure 虚拟机。
...
8.  **仅限 CentOS 6.2 或 6.3**：安装适用于 Linux Integration Services 的驱动程序。
...
9.  通过运行以下命令安装 python-pyasn1：

        # sudo yum install python-pyasn1

10. 将其 /etc/yum.repos.d/CentOS-Base.repo 文件替换为以下文本
...
14. 对于 CentOS 6.2 和 6.3，通过运行以下命令更新正在运行的虚拟机的内核
...
15. 在 grub 配置中修改内核引导行，以使其包含 Azure 的其他内核参数。若要执行此操作，请在文本编辑器中打开 /boot/grub/menu.lst，并确保默认内核包含以下参数：

        console=ttyS0 earlyprintk=ttyS0 rootdelay=300 numa=off

    这还将确保所有控制台消息都发送到第一个串行端口，从而可以协助 Azure 支持人员调试问题。此外，由于 CentOS 6 所使用的内核版本中存在错误，因此这将禁用 NUMA。

16. 在 /etc/sudoers 中，如果以下行存在，则注释掉以下行：

        Defaults targetpw

17. 请确保已安装 SSH 服务器且已将其配置为在引导时启动。
...
3.  替换映像中的当前存储库以使用具有您升级 VM 所需的内核和代理包的 Azure 存储库。这些步骤可能会由于 Ubuntu 版本的不同而稍有差异。

    在编辑 /etc/apt/sources.list 之前，建议您
...
6.  修改 Grub 的内核引导行以使其包含 Azure 的其他内核参数。若要执行此操作，请在文本编辑器中打开 /etc/default/grub、找到名为“GRUB\_CMDLINE\_LINUX\_DEFAULT”的变量（或添加它，如果需要） 并对它进行编辑以使其包含以下参数：
...
9.  通过将以下命令与 sudo 一起运行来安装代理：
    ...
    请注意，如果已安装 NetworkManager 包和 NetworkManager-gnome 包，则安装 walinuxagent 包将删除它们。
...
9.  在 grub 配置中修改内核引导行，以使其包含 Azure 的其他内核参数。若要执行此操作，请在文本编辑器中打开 /boot/grub/menu.lst，并确保默认内核包含以下参数：

        console=ttyS0 earlyprintk=ttyS0 rootdelay=300 numa=off

    这还将确保所有控制台消息都发送到第一个串行端口，从而可以协助 Azure 支持人员调试问题。此外，由于 CentOS 6 所使用的内核版本中存在错误，因此这将禁用 NUMA。

10. 在 /etc/sudoers 中，如果以下行存在，则注释掉以下行：

        Defaults targetpw

11. 请确保已安装 SSH 服务器且已将其配置为在引导时启动。
...
15. 在 Hyper-V 管理器中单击“关闭”。

### 准备 SUSE Linux Enterprise Server 11 SP2 和 SP3

**注意：** [SUSE Studio][SUSE Studio] 可轻松地为 Azure 和 Hyper-V 创建和管理您的 SLES/opeSUSE 映像。此外，SUSE Studio 库中的以下正式映像可下载或克隆到您自己的 SUSE Studio 帐户中以便轻松地进行自定义：
...
3.  在 grub 配置中修改内核引导行，以使其包含 Azure 的其他内核参数。若要执行此操作，请在文本编辑器中打开 /boot/grub/menu.lst，并确保默认内核包含以下参数：

        console=ttyS0 earlyprintk=ttyS0 rootdelay=300

    这将确保所有控制台消息都发送到第一个串行端口，从而可以协助 Azure 支持人员调试问题。

4.  建议您将 /etc/sysconfig/network/dhcp 或等效项从 DHCLIENT\_SET\_HOSTNAME="yes" 设置为 DHCLIENT\_SET\_HOSTNAME="no"
...
> -   [SUSE Studio 库上的 openSUSE 13.1 for Azure][SUSE Studio 库上的 openSUSE 13.1 for Azure]
...
4.  在外壳程序上，运行命令“`zypper lr`”。如果此命令返回类似于以下内容的输出（请注意版本号可能会有所不同）：
...
-   从您在上一步中使用的 Azure PowerShell 窗口中，键入：
    ...
    有关详细信息，请参阅 [Add-AzureVhd][Add-AzureVhd]。
...
此列表并未涵盖所有信息，因为每个分发都是不同的；即使您满足以下所有条件，您也可能仍需显著调整您的映像以确保其在平台上正常运行。

正是出于这个原因，建议您从某个我们的[合作伙伴认可的映像][合作伙伴认可的映像]开始操作。

下面列出的内容将替换该过程的步骤 1 来创建您自己的 VHD：

1.  您将需要确保所运行的是包含最新的适用于 Hyper V 的 LIS 驱动程序的内核，或者确保已成功编译这些驱动程序（它们已进行开源）。可在[此处][此处]找到这些驱动程序
...
5.  建议您将 /etc/sysconfig/network/dhcp 或等效项从 DHCLIENT\_SET\_HOSTNAME="yes" 设置为 DHCLIENT\_SET\_HOSTNAME="no"

6.  您应确保您内核中安装的所有 SCSI 设备都包含 300 秒或更长时间的 I/O 超时。

7.  您将需要按照 [Linux 代理指南][Linux 代理指南]中的步骤操作来安装 Azure Linux 代理。已根据 Apache 2 许可发布代理，您可以在[代理 GitHub 位置][代理 GitHub 位置]获取最新的位数
...
12. 然后需要关闭虚拟机并继续上载。
  ...
  [Add-AzureVhd（可能为英文页面）]: http://msdn.microsoft.com/zh-cn/library/azure/dn495173.aspx
  ...
  [打开 Hyper-V 设置]: ./media/virtual-machines-linux-create-upload-vhd/settings.png
  [添加 DVD 驱动器与安装介质]: ./media/virtual-machines-linux-create-upload-vhd/installiso.png
  ...
  [SUSE Studio 库上的 openSUSE 13.1 for Azure]: https://susestudio.com/a/02kbT4/opensuse-13-1-for-windows-azure
  [创建存储帐户]: ./media/virtual-machines-linux-create-upload-vhd/create.png
  [快速创建存储帐户]: ./media/virtual-machines-linux-create-upload-vhd/storage-quick-create.png
  [输入存储帐户详细信息]: ./media/virtual-machines-linux-create-upload-vhd/storage-create-account.png
