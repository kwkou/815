...
**组件** | **详细信息** | **安装** | **部署方案**
--- | --- | --- | ---
**适用于 VMM 的 Azure Site Recovery 提供程序** | 处理 VMM 服务器与 Site Recovery 服务之间的通信。 | 在 VMM 服务器上安装 | 在您设置两个 VMM 站点之间或 VMM 站点与 Azure 之间的保护时使用
**适用于 Hyper-V 的 Azure Site Recovery 提供程序** | 在未部署 VMM 的情况下处理 Hyper-V 主机与 Site Recovery 服务之间的通信。 | 在 Hyper-V 主机服务器上安装 | 在设置 Hyper-V 站点与 Azure 之间的保护时使用   
**Microsoft 恢复服务代理** | 处理 Hyper-V 主机服务器与 Site Recovery 服务之间的通信 | 在 Hyper-V 主机服务器上安装 | <p>在您设置 Hyper-V 站点与 Azure 之间的保护时使用。</p><p>您下载单个提供程序，其中包括适用于 Hyper-V 的 Azure Site Recovery 提供程序以及 Microsoft 恢复服务代理。</p>
...
**配置服务器** | <p>在 Azure 中协调受保护的计算机、进程服务器和主目标服务器之间的通信</p><p>发生故障转移时在 Azure 中安排复制并协调恢复</p> | 安装在与 Site Recovery 相同的 Azure 订阅中的 Azure 标准 A3 虚拟机上。 | 在您设置本地物理服务器或 VMware 虚拟机与 Azure 之间的保护时使用
...
该提供程序在 VMM 服务器、HYPER-V 主机服务器（如果您的部署中没有 VMM 服务器）或配置服务器上运行。它通过 Internet 使用加密的 HTTPS 连接连接到 Site Recovery 服务。请注意：

- 不需要添加特定的防火墙例外来连接 Site Recovery 提供程序。
...
该代理通过 Internet 使用加密的 HTTPS 连接连接到 Site Recovery 服务。无需指定防火墙例外。
...
- 请注意，附加到主目标服务器的一个磁盘作为保留驱动器保留。Azure Site Recovery 让您可以定义保留窗口，并在该保留窗口内将受保护的计算机恢复到一个恢复点。保留驱动器为窗口时段内的磁盘更改保留一个日志。这会将 A4 上可用于复制的最大磁盘数量减少到 15 个，将 D14 上的减少到 31 个。

#### 进程服务器 

- 进程服务器使用基于磁盘的缓存。确保有足够的可用空间 C:/ 用于缓存。缓存大小调整将受您要保护的计算机的数据更改率的影响。通常，我们建议对中型部署使用 600 GB 的缓存目录大小。
- 如果受保护的计算机的数据更改率超过现有处理服务器的能力，则您应该部署其他处理服务器。
- 若要扩展部署，您可以添加多个处理服务器和主目标服务器。如果现有的主目标服务器上没有足够的可用磁盘，则您应该部署第二个主目标服务器。
-  请注意，处理服务器和主目标服务器不需要一对一映射。您可以使用第二个主目标服务器部署第一个处理服务器，依此类推。

#### 配置服务器

- 配置服务器是基于 Azure Site Recovery 的标准 A3 虚拟机，系统将在您的订阅中为配置服务器创建 Windows Server 2012 R2 库映像。它将使用保留的公用 IP 地址作为第一个实例在新的云服务中创建。
- 安装路径仅限使用英文字符。
...
**组件** | **如何更新** 
--- | --- 
<p>**适用于 VMM 的 Azure Site Recovery 提供程序**</p><p>**Azure 恢复服务代理**</p> | <p></p>**首次安装**：从“快速启动”页下载最新版本<p></p>**后续安装**：您可以从 Site Recovery 的“仪表板”下载最新（和上一个）版本。或者，如果您选择启用 Microsoft 更新，最新版本的提供程序和代理将会自动安装在服务器上。
<p>**进程服务器**</p><p>**配置服务器**</p><p>**主目标服务器**</p> | 检查 Site Recovery 仪表板上的更新。 
