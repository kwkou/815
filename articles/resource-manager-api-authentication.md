...
1. **仅限应用的访问**：适用于运行守护程序服务和计划作业的应用。应用的标识获得资源的直接访问权限。此方法适用于需要长期“脱机访问”Azure 的应用。
...
本主题的所有代码都将作为 Web 应用运行，可以在 [http://vipswapper.azurewebsites.net/cloudsense](http://vipswapper.azurewebsites.net/cloudsense) 上试用。
...
[request an authorization code](/documentation/articles/active-directory-protocols-oauth-code/)（授权代码授予流）主题中介绍了适用于此请求的查询字符串参数。
...
若要检查应用程序是否具有 Azure 订阅上的所需访问权限，也可以调用 [Resource Manager 权限](https://msdn.microsoft.com/zh-cn/library/azure/dn906889.aspx) API。此方式类似于用于确定用户是否具有订阅访问管理权限的方式。不过，这次请使用上一步骤中收到的仅限应用的访问令牌来调用权限 API。
