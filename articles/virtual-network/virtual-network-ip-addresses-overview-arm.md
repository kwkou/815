<properties
   ...
   description="了解如何在 Azure Resource Manager 中实施公共 IP 和专用 IP 寻址"
...
> [AZURE.NOTE] Azure 提供用于创建和处理资源的两种部署模型：[Resource Manager 模型和经典模型](/documentation/articles/resource-manager-deployment-model/)。本文介绍如何使用 Resource Manager 部署模型。Azure 建议对大多数新的部署使用该模型，而不是[经典部署模型](/documentation/articles/virtual-network-ip-addresses-overview-classic/)。
...
在 Azure Resource Manager 中，公共 IP 地址是具有其自身属性的资源。可以将公共 IP 地址资源与以下任意资源相关联：
...
>[AZURE.IMPORTANT] 所创建的每个域名标签在其 Azure 位置必须是唯一的。

### 虚拟机
将公共 IP 地址分配到其**网络接口**，可将其与 [Windows](/documentation/articles/virtual-machines-windows-about/) 或 [Linux](/documentation/articles/virtual-machines-linux-about/) VM 相关联。对于多网络接口 VM，只能将它分配到*主要*网络接口。可以向 VM 分配动态或静态公共 IP 地址。
...
## 后续步骤
- [使用 Azure 门户预览为 VM 部署静态公共 IP](/documentation/articles/virtual-network-deploy-static-pip-arm-portal/)
- [使用模板为 VM 部署静态公共 IP](/documentation/articles/virtual-network-deploy-static-pip-arm-template/)
- [通过 Azure 门户预览为 VM 部署静态专用 IP 地址](/documentation/articles/virtual-networks-static-private-ip-arm-pportal/)
