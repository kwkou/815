...
在本教程中，您将了解有关使用 Azure 管理门户执行 Windows Azure SQL数据库 管理任务的基础知识。如果您对数据库管理不熟悉，则可以通过这些课程在大约 30 分钟的时间内学习一些基本技能。
...
通过选择此选项，您可以以管理员身份同时创建新的服务器和 SQL数据库。作为系统管理员，您可以执行更多任务，包括连接到 SQL数据库 管理门户，本教程的后面将会介绍该任务。

1.  单击“自定义创建”后，将显示“数据库设置”页。在此页面中，需要提供在服务器上创建空 SQL数据库 的基本信息。将在后面的步骤中添加表和数据。
...
-   输入一个没有空格的单词作为管理员名称。SQL数据库 在加密连接中使用 SQL 身份验证来验证用户身份。将使用您提供的名称创建一个具有管理员权限的新 SQL Server 身份验证登录名。管理员名称既不能是 Windows 用户，也不能是 Live ID 用户名。SQL数据库 不支持 Windows 身份验证。
...
若要查看刚创建的用于托管 **School** 数据库的服务器名称，请在左侧导航窗格中单击“SQL数据库”，然后在单击“SQL数据库”列表视图中单击 **School** 数据库。在“快速启动”页上，向下滚动该页面以查看该服务器名称。
...
现在，您已在 Azure 上拥有了 SQL数据库 服务器、允许访问该服务器的防火墙规则、数据库对象以及管理员登录名。但是，您仍然没有可查询的工作数据库。为此，您的数据库必须具有架构和实际数据。

由于本教程中仅使用已有的工具，因此您将使用 SQL数据库 管理门户中的查询窗口来运行 Transact-SQL 脚本，以便生成预定义的数据库。
...
2.  单击 **School** 以将其选中，然后单击页面底部的“管理”。这将打开 SQL数据库 管理门户。此门户与 Azure 管理门户不同。您将使用此门户运行 Transact-SQL 命令和查询。

3.  输入用于登录到 **School** 数据库的管理员登录名和密码。这是您在创建服务器时所指定的管理员登录名。

4.  在 SQL数据库 管理门户的功能区中，单击“新建查询”。将在工作区中打开一个空查询窗口。在下一步中，您将使用此窗口来复制用于将结构和数据添加到空数据库的一系列预定义脚本。
...
在 SQL数据库 中，您可以使用 Transact-SQL 创建登录名和授予权限。在本课程中，您将使用 Transact-SQL 执行以下三项操作：
...
4.  将在新浏览器窗口中打开 SQL数据库 管理门户并且会将您连接到 **master**。
...
在 Azure 管理门户上，您可以在 SQL数据库“服务器”页的“仪表板”上的“管理 URL”中找到该服务器名称。服务器名称由一串字母和数字组成，后跟“.database.chinacloudapi.cn”。在数据库连接向导中指定此名称。在指定该名称时不要包含 http:// 或 https:// 前缀。
