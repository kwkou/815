...
可在[此文][virtual-machines-linux-capture-image-resource-manager]中找到有关从虚拟机创建映像详细信息。
...
可以通过许多不同的方式配置 Azure 中的名称解析和 IP 解析。在本文档中，仅限云的方案依赖于使用 Azure DNS 的默认方式（与定义自己的 DNS 服务不同）。此外，还可使用新的 Azure DNS 服务，而不是设置自己的 DNS 服务器。有关详细信息，请参阅[此文][virtual-networks-manage-dns-in-vnet]和[此页](/home/features/dns/)。
...
#### VNet 到 VNet 连接
使用多站点 VPN 时，需要在每个区域中配置不同的 Azure 虚拟网络。但是，通常必须让不同区域中的软件组件能够彼此通信。在理想情况下，此通信应该从一个 Azure 区域路由到本地，再由此路由到其他 Azure 区域。简而言之，Azure 可让设置从某个区域中的一个 Azure 虚拟网络到托管在另一个区域中的另一个 Azure 虚拟网络的连接。此功能称为 VNet 对 VNet 连接。有关此功能的详细信息，请参阅：</documentation/articles/vpn-gateway-vnet-vnet-rm-ps/>。
...
若要避免此问题，可以在本地与 Azure 之间配置具有站点到站点连接的强制隧道。有关强制隧道功能的详细说明，请参阅 </documentation/articles/vpn-gateway-forced-tunneling-rm/>
...
对于使用 Linux 并想要管理 Azure 资源的客户而言，可能不适合使用 Powershell。Microsoft 提供了 Azure CLI 作为替代方案。Azure CLI 提供了一组开源且跨平台的命令，这些命令可以用于 Azure 平台。Azure CLI 提供许多与 Azure 门户相同的功能。
...
> ![Linux][Logo_Linux] Linux
>
> [如何捕获 Linux 虚拟机以用作资源管理器模板][virtual-machines-linux-capture-image-resource-manager]
...
也可以在订阅之间复制 VHD。可在 <http://gallery.technet.microsoft.com/scriptcenter/Copy-all-VHDs-in-Blog-829f316e> 中下载或查看用于执行此操作的示例。
...
> ![Windows][Logo_Windows] Windows
>
> 共享本地打印机。在 Azure VM 中打开 Windows 资源管理器，然后键入打印机共享名称。打印机安装向导将引导你完成整个安装过程。
>
> ![Linux][Logo_Linux] Linux
>
...
如果未建立直接 IP 连接，SAProuter 可以在参与系统之间实现 TCP/IP 通信。它提供的优势是，无需在通信合作伙伴之间建立网络级的端到端连接。默认情况下，SAProuter 将会侦听端口 3299。若要通过 SAProuter 连接 SAP 实例，每次尝试连接时，都需要提供 SAProuter 字符串和主机名。
...
可在以下文章中找到更多详细信息：</documentation/articles/storage-redundancy/>
