...
* **数据移动** - 对于处理大量数据的应用程序，需要使用策略将数据移至云存储和计算资源，并可能需要高速跨界网络，例如 [Azure ExpressRoute](https://azure.microsoft.com/services/expressroute/)。另外，还要考虑到法律、法规或政策对存储或访问该数据的限制。
...
• **大数据**可解决数据分析问题，这些问题涉及无法由单个计算机或数据库管理系统管理的大量数据（例如大量 Web 日志或其他商业智能数据）。大数据往往更多地依赖于磁盘容量和 I/O 性能而不是 CPU 性能，通常使用 Apache Hadoop 等专用工具来管理群集以及对数据分区。（有关 Azure HDInsight 和其他 Azure Hadoop 解决方案的信息，请参阅 [Hadoop](https://azure.microsoft.com/solutions/hadoop/)。）
