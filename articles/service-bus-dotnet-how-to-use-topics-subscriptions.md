...
本指南介绍如何使用服务总线主题和订阅。相关示例用 C# 编写且使用 .NET API。涉及的任务包括**创建主题和订阅**、**创建订阅筛选器**、**将消息发送到主题**、**从订阅接收消息**以及**删除主题和订阅**。有关主题和订阅的详细信息，请参阅[后续步骤](#Next-steps)部分。
...
2.  搜索“服务总线”并选择“Windows Azure 服务总线”项。单击“安装”以完成安装，然后关闭此对话框。
...
在使用 Azure 网站或虚拟机时，建议你使用 .NET 配置系统（如 **Web.config**）。你可以使用 `<appSettings>` 元素存储连接字符串：
...
你可以使用 [`NamespaceManager`](https://msdn.microsoft.com/zh-cn/library/microsoft.servicebus.namespacemanager.aspx) 类对服务总线主题和订阅执行管理操作。此类提供了创建、枚举和删除主题的方法。

以下示例使用带连接字符串的 Azure `CloudConfigurationManager` 类构造 `NamespaceManager` 对象，此连接字符串包含服务总线服务命名空间的基址和有权管理该命名空间的相应 SAS 凭据。此连接字符串的形式为
...
[`CreateTopic`](https://msdn.microsoft.com/zh-cn/library/microsoft.servicebus.namespacemanager.createtopic.aspx) 方法存在一些重载，允许你调整主题的属性，例如，将默认的“生存时间”值设置为应用于发送到主题的消息。使用 [`TopicDescription`](https://msdn.microsoft.com/zh-cn/library/microsoft.servicebus.messaging.topicdescription.aspx) 类应用这些设置。以下示例演示如何创建名为 **TestTopic**、最大大小为 5 GB、默认消息生存时间为 1 分钟的主题。
...
> [AZURE.NOTE]你可以对 [`NamespaceManager`](https://msdn.microsoft.com/zh-cn/library/microsoft.servicebus.namespacemanager.aspx) 对象使用 [`TopicExists`](https://msdn.microsoft.com/zh-cn/library/microsoft.servicebus.namespacemanager.topicexists.aspx) 方法，以检查具有指定名称的主题是否已位于服务命名空间中。

## 如何创建订阅

你还可以使用 [`NamespaceManager`](https://msdn.microsoft.com/zh-cn/library/microsoft.servicebus.namespacemanager.aspx) 类创建主题订阅。订阅已命名，并且具有一个限制传递到订阅的虚拟队列的消息集的可选筛选器。
...
以下示例创建了一个名为 **HighMessages** 的订阅（带有只选择具有大于 3 的 **MessageNumber** 属性的 [SqlFilter] 对象）：
...
以下代码演示了如何使用 [`CreateFromConnectionString`](https://msdn.microsoft.com/zh-cn/library/microsoft.servicebus.messaging.topicclient.createfromconnectionstring.aspx) API 调用为上面创建的 **TestTopic** 主题创建 [`TopicClient`](https://msdn.microsoft.com/zh-cn/library/microsoft.servicebus.messaging.topicclient.aspx) 对象：
...
从订阅接收消息的建议方法是使用 [`SubscriptionClient`](https://msdn.microsoft.com/zh-cn/library/azure/microsoft.servicebus.messaging.subscriptionclient.aspx) 对象。**SubscriptionClient** 对象可在两种不同模式下工作：[`ReceiveAndDelete` 和 `PeekLock`](https://msdn.microsoft.com/zh-cn/library/azure/microsoft.servicebus.messaging.receivemode.aspx)。
...
此示例使用 [`OnMessageOptions`](https://msdn.microsoft.com/zh-cn/library/azure/microsoft.servicebus.messaging.onmessageoptions.aspx) 对象配置 [`OnMessage`](https://msdn.microsoft.com/zh-cn/library/azure/microsoft.servicebus.messaging.subscriptionclient.onmessage.aspx) 回调。将 [`AutoComplete`](https://msdn.microsoft.com/zh-cn/library/azure/microsoft.servicebus.messaging.onmessageoptions.autocomplete.aspx) 设置为 **false** 时，以便在对接收的消息调用 [`Complete`](https://msdn.microsoft.com/zh-cn/library/azure/microsoft.servicebus.messaging.brokeredmessage.complete.aspx) 时启用手动控制。将 [`AutoRenewTimeout`](https://msdn.microsoft.com/zh-cn/library/azure/microsoft.servicebus.messaging.onmessageoptions.autorenewtimeout.aspx) 设为 1 分钟，使客户端等待消息调用超时最多一分钟，然后客户端执行新的调用以检查消息。此属性值会减少客户端无法检索消息时产生的应计费调用次数。

## 如何处理应用程序崩溃和不可读消息

Service Bus 提供了相关功能来帮助你轻松地从应用程序错误或消息处理问题中恢复。如果接收方应用程序出于某种原因无法处理消息，它可以对收到的消息调用 [`Abandon`](https://msdn.microsoft.com/zh-cn/library/azure/microsoft.servicebus.messaging.brokeredmessage.abandon.aspx) 方法（而不是 [`Complete`](https://msdn.microsoft.com/zh-cn/library/azure/microsoft.servicebus.messaging.brokeredmessage.complete.aspx) 方法）。这会导致服务总线解锁订阅中的消息并使其能够重新被同一个正在使用的应用程序或其他正在使用的应用程序接收。
...
以下示例演示如何从 **HowToSample** 服务命名空间中删除名为 **TestTopic** 的主题：
...
-   服务总线示例：从 [Azure 示例][]下载，或参阅 [MSDN][] 上的概述。
  ...
  [服务总线中转消息传送 .NET 教程]: http://msdn.microsoft.com/zh-cn/library/hh367512.aspx
