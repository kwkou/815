...
-   [步骤 1：使用 Azure PowerShell 配置远程桌面访问服务][步骤 1：使用 Azure PowerShell 配置远程桌面访问服务]
-   [步骤 2：连接到角色实例][步骤 2：连接到角色实例]
-   [步骤 3：使用 Azure PowerShell 配置禁用远程桌面访问的服务][步骤 3：使用 Azure PowerShell 配置禁用远程桌面访问的服务]

## <a name="step1"> </a>步骤 1：使用 Azure PowerShell 配置远程桌面访问服务

若要使用远程桌面，你需要使用用户名、密码和证书配置服务定义和服务配置，以向云中的角色实例进行身份验证。[Azure PowerShell][Azure PowerShell] 中包含 **Enable-AzureServiceProjectRemoteDesktop** cmdlet，它会为你完成此配置。

在创建服务定义的计算机上执行以下步骤。

1.  从“开始”菜单中，选择“Azure PowerShell”。

    ![Azure PowerShell“开始”菜单项][Azure PowerShell“开始”菜单项]
...
1.  在 [Azure 管理门户][Azure 管理门户]中，选择“云服务”，然后选择在上面的步骤 1 中部署的服务
...
3.  单击“连接”后，Web 浏览器会提示你保存 .rdp 文件。如果你使用的是 Internet Explorer，请单击“打开”。
    ...
    ![提示打开或保存 .rdp 文件][提示打开或保存 .rdp 文件]

4.  打开该文件时，会显示以下安全提示：

    ![Windows 安全性提示][Windows 安全性提示]
...
当你不再需要与云中角色实例的远程桌面连接时，可使用 [Azure PowerShell][Azure PowerShell] 禁用远程桌面访问
...
-   [远程访问 Azure 中的角色实例][远程访问 Azure 中的角色实例]
-   [将远程桌面与 Azure 角色一起使用][将远程桌面与 Azure 角色一起使用]
