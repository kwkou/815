...
1. 打开你的 Web 浏览器并转到 [Azure 经典门户]。如果出现提示，请使用你的 Azure 订阅信息进行登录。
...
> [AZURE.NOTE]还可以使用 [Azure 经典门户](https://manage.windowsazure.cn/)执行这些步骤。请参阅[在 Azure 中生成和部署 Node.js Web 应用]。
>
> 如果这是你创建的第一个 Web 应用，则你必须使用 Azure 经典门户部署此应用程序。
...
2. 使用以下命令创建新的 Web 应用。
	...
	> [AZURE.NOTE]如果这是你的订阅的第一个 Web 应用，系统会指示你使用 Azure 经典门户创建 Web 应用。有关详细信息，请参阅[在 Azure 中生成和部署 Node.js Web 应用]。

### 切换到环境变量

前面我们实现了用于查找环境变量或从 **config.json** 文件中加载值的代码。在接下来的步骤中，你将在 Web 应用配置中创建应用程序通过环境变量实际访问的键值对。

1. 从经典门户中，单击“ Web 应用”，然后选择你的 Web 应用。
