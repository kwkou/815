...
若要确定访问密钥，请转到 [Azure 管理门户][Azure 管理门户]、单击左侧的“存储”选项卡、选择与您的存储帐户对应的名称，然后在底部单击“管理访问密钥”。单击“主访问密钥”的“复制”按钮。

![管理访问密钥][管理访问密钥]

1.  输入存储帐户的名称和在存储连接字符串的占位符内提供的访问密钥之一。此连接字符串将同时在 **Microsoft.WindowsAzure.Plugins.Diagnostics.ConnectionString** 设置的 **SplitMergeWeb** 和 **SplitMergeWorker** 角色部分下使用。您可以对不同的角色使用不同的存储帐户。

### 配置安全

有关配置服务安全的详细说明，请参阅[灵活扩展安全配置][灵活扩展安全配置]。
...
请注意，对于生产部署，应针对 CA 使用单独的证书（服务器证书和客户端证书）。有关此内容的详细说明，请参阅[安全配置][灵活扩展安全配置]。

### 部署您的拆分/合并服务

1.  转到 [Azure 管理门户][Azure 管理门户]。
2.  单击左侧的“云服务”选项卡，然后选择您之前创建的云服务。
3.  单击**“仪表板”**。
4.  选择过渡环境，然后单击“上载新的过渡部署”。

    ![过渡][过渡]

5.  在对话框中，输入一个部署标签。对于“程序包”和“配置”，单击“从本地”，然后选择“SplitMergeService.cspkg”文件和您之前配置的 .cscfg 文件。
