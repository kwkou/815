...
连接到 Azure SQL 数据库所需的连接字符串与连接到 Microsoft SQL Server 所需的字符串稍有不同。可以从 [Azure 经典门户](https://manage.windowsazure.cn)复制数据库的连接字符串。
...
必须将 SQL 数据库服务器配置为接受来自托管客户端程序的计算机的通信。为此，可以通过 [Azure 经典门户](https://manage.windowsazure.cn)编辑防火墙设置。
