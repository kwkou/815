<properties
    pageTitle="使用 Apache Storm 从 Azure 事件中心接收事件 | Azure"
...
## 使用 Apache Storm 从 Azure 事件中心接收事件
...
11. 使用以下代码创建一个名为 **LogTopology** 的类：
    ...
    此类创建新事件中心 Spout，并使用配置文件中的属性对它进行实例化。请务必注意：此示例创建喷管的数量与事件中心中分区的数量相同，以便使用该事件中心允许的最大并行度。
...
<!--Update_Description:new article about recieving event from Azure event hubs with apache storm-->