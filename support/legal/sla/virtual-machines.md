...
1. “**可用性组**”是指为了避免单点故障在不同的故障域中部署的两台或两台以上的虚拟机。

2. “**虚拟机**”是指可以单独部署也可以作为可用性组的一部分部署的永久性实例类型。

3. “**故障域**”是指一组共享公共资源（例如电源和网络连接）的服务器的集合。

4. “**更新域**”是指同时应用平台更新的一组 Azure 实例。

### 虚拟机的每月正常服务时间计算和服务级别

1. “**最大可用分钟数**”是指在一个帐单月份期间在同一可用性组中具有两个或多个实例的所有面向互联网虚拟机的总累计分钟数。最大可用分钟数的测量时间从因客户发起的操作而启动同一可用性组中至少两台虚拟机开始，直到客户发起了可能导致停止或删除虚拟机的操作时为止。
