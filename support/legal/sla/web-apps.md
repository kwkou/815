...
“**Web 应用**”是指在 Web 应用服务内客户部署的 Web 应用，不包括免费和共享 Azure Web 应用层级中的 Web 应用。

### Web 应用服务的每月正常服务时间计算和服务级别

1. “**部署分钟数**”是指在一个帐单月份期间指定的 Web 应用已设置为在 Azure 中运行的总分钟数。部署分钟数的测量时间从创建 Web 应用之时或者客户发起了使 Web 应用运行的操作时开始，直到客户发起了可能导致停止或删除该 Web 应用操作时为止。

2. “**最大可用分钟数**”是指在一个帐单月份期间指定的 Azure 订购中客户部署的所有 Web 应用所用的总部署分钟数。

3. “**停机时间**”是指在 Web 应用不可用期间指定的 Azure 订购中客户部署所有 Web 应用所用的总累计部署分钟数。当在某一分钟内，在某个网站和世纪互联 Internet 网关之间没有连接时，则将视为此指定 Web 应用在该分钟内不可用。

4. Web 应用服务的“**每月正常服务时间百分比**”按如下方式计算：指定的 Azure 订购在一个帐单月份期间的最大可用分钟数减去停机时间，再除以最大可用分钟数。每月正常服务时间百分比计算公式如下所示：

	每月正常服务时间百分比 % = (最大可用分钟数 − 停机时间) ÷ 最大可用分钟数 

5. 以下服务级别和服务费抵扣适用于客户对 Web 应用服务的使用：
