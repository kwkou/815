...
# MySQL Database on Azure 的服务级别协议
...
“**数据库服务器**”是指客户在 MySQL Database on Azure 服务中部署的 MySQL 数据库服务器。



### MySQL Database on Azure 服务的月度正常运行时间计算和服务级别

1. “**部署分钟数**” 是指给定数据库服务器在计费月份期间部署在 Azure 中的总分钟数。 

2. “**最大可用分钟数**” 是指给定 Azure 订阅在计费月份期间跨所有数据库服务器的全部部署分钟数的总和。

3. “**停机时间**” 是指数据库服务器不可用期间，客户在给定 Azure 订阅中部署的跨所有数据库服务器的累计总部署分钟数。如果客户在一分钟内与给定数据库服务器的所有连接尝试均失败，则该分钟对于给定数据库服务器而言视为不可用。 

4. MySQL Database on Azure 服务的“**月度正常运行时间百分比**”计算方法为：给定 Azure 订阅在计费月份的最大可用分钟数减去停机时间，然后除以最大可用分钟数。月度正常运行时间百分比的计算公式如下：


	每月正常服务时间百分比 % = (最大可用分钟数 − 停机时间) ÷ 最大可用分钟数 

5. 客户在使用 MySQL Database on Azure 服务时适用以下服务级别和服务费抵扣：
