...
### 流分析 API 调用的每月正常运行时间计算

1. “**总事务尝试次数**”是指在指定 Azure 订阅的计费月份内，可以让客户在流分析服务内管理流作业的通过身份验证的 REST API 请求总数。

2. “**失败事务数**”是指“**总事务尝试次数**”中返回错误代码或者自世纪互联收到请求起的五分钟内未返回成功代码的所有请求集。

3. 流分析服务中 API 调用的“**每月正常运行时间百分比**”计算方法为：总事务尝试次数减去失败事务数，两者的差值除以指定 Azure 订阅的计费月份内的总事务尝试次数。每月正常运行时间百分比的计算公式如下：

		每月正常运行时间百分比 = (总事务尝试次数 - 失败事务数) / 总事务尝试次数

4. 客户在流分析服务中使用 API 调用时适用以下服务费抵扣。
...
1.“**部署分钟数**”是指给定作业在计费月份内部署在流分析服务中的总分钟数。

2.“**最大可用分钟数**”是指客户在指定 Azure 订阅的计费月份内跨所有作业部署的全部部署分钟数的总和。

3.“**停机时间**”是指作业不可用期间，客户在指定 Azure 订阅中部署的跨所有作业的累计总部署分钟数。如果已部署的作业在一分钟内既不处理数据也无法处理数据，则对于此作业而言，该分钟视为不可用。

4. 流分析服务中作业的 “**每月正常运行时间百分比**” 计算方法为：最大可用分钟数减去停机时间，两者的差值除以在指定 Azure 订阅的计费月份内的最大可用分钟数。每月正常运行时间百分比的计算公式如下：

		每月正常运行时间百分比 = (最大可用分钟数 - 停机时间) / 最大可用分钟数

5. 客户在流分析服务中使用作业时适用以下服务费抵扣。
