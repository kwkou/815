...
> [AZURE.LANGUAGE]
- [中文](/support/legal/sla/web-apps/)
- [English](/support/legal/sla/web-apps-en/)
# SLA for Web Apps
...
"**Web Apps**" is a Web Apps deployed by Customer within the Web Apps Service, excluding Web Apps in the Free and Shared Azure Web Apps tiers.

### Monthly Uptime Calculation and Service Levels for Web Apps Service

1. "**Deployment Minutes**" is the total number of minutes that a given Web Apps has been set to running in Azure during a billing month. Deployment Minutes is measured from when the Web Apps was created or the Customer has initiated an action that would result in running the Web Apps to the time the Customer has initiated an action that would result in stopping or deleting the Web Apps.

2. "**Maximum Available Minutes**" is the sum of all Deployment Minutes across all Web Apps deployed by Customer in a given Azure subscription during a billing month.

3. "**Downtime**" is the total accumulated Deployment Minutes, across all Web Apps s deployed by Customer in a given Azure subscription, during which the Web Apps is unavailable. A minute is considered unavailable for a given Website when there is no connectivity between the Website and 21Vianet’s Internet gateway.

4. "**Monthly Uptime Percentage**" for the Web Apps Service is calculated as Maximum Available Minutes less Downtime divided by Maximum Available Minutes in a billing month for a given Azure subscription. Monthly Uptime Percentage is represented by the following formula:

	Monthly Uptime % = (Maximum Available Minutes − Downtime) / Maximum Available Minutes

5. The following Service Levels and Service Credits are applicable to Customer's use of the Web Apps Service:
