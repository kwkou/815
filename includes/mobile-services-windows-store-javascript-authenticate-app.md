...
1. 打开项目文件 default.js，并在 **app.OnActivated** 方法重载中，将对 **refreshTodoItems** 方法的最后一个调用替换为以下代码： 
    ...
    >[WACOM.NOTE]如果您向移动服务注册了 Windows 应用商店应用程序包信息，则应通过为 <em>useSingleSignOn</em> 参数提供值 <strong>true</strong> 来调用 <a href="http://go.microsoft.com/fwlink/p/?LinkId=322050" target="_blank">login</a> 方法。如果您不执行此操作，则每次调用 login 方法时，系统仍会向您的用户提供登录提示。

2. 按 F5 键运行应用，并使用您选择的标识提供程序登录应用。 
