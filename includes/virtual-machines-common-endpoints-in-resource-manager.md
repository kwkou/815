Azure 终结点的工作方法在经典部署模型和 Resource Manager 部署模型之间略有不同。在 Resource Manager 部署模型中，现可灵活地创建用于控制出入 VM 的流量的网络筛选器。在经典部署模型中，只能创建简单终结点；而使用这些筛选器，可进一步创建复杂网络环境。本文提供了网络安全组的概述以及它们与使用经典终结点、创建这些筛选规则有什么不同，并提供了示例部署方案。
