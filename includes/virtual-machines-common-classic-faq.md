...
Azure 存储帐户提供可用于操作系统磁盘和任意数据磁盘的存储。每个磁盘都是一个 .vhd 文件，以页 blob 形式存储。有关定价详细信息，请参阅 [Storage Pricing Details（存储定价详细信息）](/pricing/details/storage/)。
...
有关更多详细信息，请参阅[虚拟机定价](/pricing/details/virtual-machines/)。
