...
> [WACOM.NOTE]
> Azure 存储服务支持最大为 1 TB 的 Blob，其中可容纳最大虚拟大小为 999 GB 的 VHD。不过，如果你使用 Hyper-V 创建新的 VHD，则你指定的大小表示虚拟大小。若要在 Azure 中使用 VHD，请指定不超过 999 GB 的大小。

**数据磁盘与资源磁盘**
数据磁盘驻留在 Azure 存储服务中并且可以用于持久存储文件和应用程序数据。
