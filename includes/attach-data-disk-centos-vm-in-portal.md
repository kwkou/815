1. 在 [Azure 管理门户][AzurePortal] 中，单击"虚拟机"，然后选择刚才创建的虚拟机 (**testlinuxvm**)。

2. 在命令栏上，单击"附加"，然后单击"附加空磁盘"。

	将显示"附加空磁盘"对话框。

3. 已经为您定义了"虚拟机名称"、"存储位置"和"文件名"。您只需要输入所需的磁盘大小。在"大小"字段中键入 **5**。

	![附加空磁盘 ][Image2]

	**注意：**所有磁盘都从 Azure 存储中的 VHD 文件创建。您可以为添加到存储中的 VHD 文件提供名称，但 Azure 会自动生成磁盘的名称。
