...
创建特定资源时，Azure 使用某些默认设置来简化与这些资源关联的资源的管理。例如，在为新云服务创建第一个虚拟机时，Azure 经典门户会尝试将虚拟机的名称用于虚拟机的新云服务的名称。
...
- 使用命名约定创建存储帐户集。可以使用 Azure 经典门户或 **New-AzureStorageAccount** PowerShell cmdlet。
...
- 使用命名约定定义每个虚拟机名称。
- 使用 Azure 经典门户、**New-AzureVM** PowerShell cmdlet 或使 Azure CLI。
...
他们通过 Azure 经典门户使用以下设置创建了仅限云的虚拟网络：
